...
class UnitTests(unittest.TestCase):
    def test_auc(self):
        ...
        spu_fn = spsim.sim_jax(sim, bin_count, static_argnums=(2,), copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #bin_count
        ...
        spu_fn = spsim.sim_jax(sim, thresholds_digitize, static_argnums=(1,), copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #thresholds_digitize
        ...
        spu_fn = spsim.sim_jax(sim, roc_auc_score, copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #roc_auc_score
    ...
    def test_classification(self):
        ...
        spu_fn = spsim.sim_jax(sim, proc, static_argnums=(2, 5), copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #test_classification_binary
        spu_result = spu_fn(
        ...
        spu_fn = spsim.sim_jax(sim, proc, static_argnums=(2, 5), copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #test_classification_multiclass
        spu_result = spu_fn(
    ...
    def test_average_precision_score(self):
        ...
        check(*proc(y_true, y_score, 0, {pphlo_dict}{skip_control})) #average_precision_score_test0
        # 0-1 labels, with tied value, even length
        y_true = jnp.array([0, 0, 1, 1], dtype=jnp.int32)
        y_score = jnp.array([0.4, 0.4, 0.4, 0.25], dtype=jnp.float32)
        check(*proc(y_true, y_score, 1, {pphlo_dict}{skip_control})) #average_precision_score_test1
        ...
        check(*proc(y_true, y_score, 2, {pphlo_dict}{skip_control})) #average_precision_score_test2
        # customized labels
        y_true = jnp.array([2, 2, 3, 3], dtype=jnp.int32)
        y_score = jnp.array([0.1, 0.2, 0.3, 0.4], dtype=jnp.float32)
        check(*proc(y_true, y_score, 3, {pphlo_dict}{skip_control}, pos_label=3)) #average_precision_score_test3
        ...
        check(*proc(y_true, y_score, 4, {pphlo_dict}{skip_control})) #average_precision_score_test4
        # single label edge case
        y_true = jnp.array([0, 0, 0, 0], dtype=jnp.int32)
        y_score = jnp.array([0.4, 0.25, 0.4, 0.25], dtype=jnp.float32)
        check(*proc(y_true, y_score, 5, {pphlo_dict}{skip_control})) #average_precision_score_test5
        y_true = jnp.array([1, 1, 1, 1], dtype=jnp.int32)
        y_score = jnp.array([0.4, 0.25, 0.4, 0.25], dtype=jnp.float32)
        check(*proc(y_true, y_score, 6, {pphlo_dict}{skip_control})) #average_precision_score_test6
        # zero score edge case
        y_true = jnp.array([0, 0, 1, 1, 1], dtype=jnp.int32)
        y_score = jnp.array([0, 0, 0, 0.25, 0.25], dtype=jnp.float32)
        check(*proc(y_true, y_score, 7, {pphlo_dict}{skip_control})) #average_precision_score_test7
        # score > 1 edge case
        y_true = jnp.array([0, 0, 1, 1, 1], dtype=jnp.int32)
        y_score = jnp.array([1.5, 1.5, 1.5, 0.25, 0.25], dtype=jnp.float32)
        check(*proc(y_true, y_score, 8, {pphlo_dict}{skip_control})) #average_precision_score_test8
        ...
        for average in ["macro", "micro", None]:
            ...
            if average == "macro":
                ...
                spu_fn = spsim.sim_jax(sim, average_precision_score, static_argnums=(3,), copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #average_precision_score_testmacro
            elif average == "micro":
                average_precision_score.__name__ = f"average_precision_score_testmicro"
                spu_fn = spsim.sim_jax(sim, average_precision_score, static_argnums=(3,), copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #average_precision_score_testmicro
            else:
                average_precision_score.__name__ = f"average_precision_score_testNone"
                spu_fn = spsim.sim_jax(sim, average_precision_score, static_argnums=(3,), copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #average_precision_score_testNone
            spu_res = spu_fn(
