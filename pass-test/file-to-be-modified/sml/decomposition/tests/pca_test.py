...
class UnitTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        config128 = spu_pb2.RuntimeConfig(
            protocol=spu_pb2.ProtocolKind.{protocalchosen},
            field=spu_pb2.FieldType.FM128,
            fxp_fraction_bits=30,
        )
        cls.sim128 = spsim.Simulator({partynum}, config128)
    ...
    def test_power(self):
        ...
        spu_fn = spsim.sim_jax(self.sim64, proc_transform, copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #test_power
        result = spu_fn(X)
        try:
            if result == "skipped":
                return True
    ...
    def test_rsvd(self):
        ...
        spu_fn = spsim.sim_jax(self.sim128, proc_transform, copts=copts, pphlo_ref={pphlo_dict}{skip_control}) #test_rsvd
