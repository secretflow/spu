...
class reducer():
    def __init__(self, data, log_path_all, partynum=2, protocalchosen="SEMI2K", pass_option_mut=None, matrix="send_bytes", inline_call=False, verbose=False):
        self.data = data
    ...
    @staticmethod
    def _compute_cdd_chunk_size(round_count, initial_p):
    ...
    def reduce(self, mod="ddmin", onlycomplement=False, initial_p=0.1):
        if mod not in ["ddmin", "CDD", "ProfDD"]:
            raise ValueError(
                "Error: mod should be one of ddmin, CDD, or ProfDD."
            )
        ...
        if mod == "ddmin":
            ...
            self.ProfDD([i for i in range(len(self.IR_sequnce))], record_log_path, trigger_log_path)
...
ddmethod_list = ["ddmin", "onlycomplement", "CDD", "ProfDD"]
...
for case_name in case_name_list:
    for ddmethod in ddmethod_list:
        ...
        print("matrix:", matrix)
        reducer_instance = reducer(input_test_case, log_path_all, partynum=partynum, protocalchosen=protocalchosen, pass_option_mut=pass_option_mut, matrix=matrix, inline_call=True, verbose=True)
        if ddmethod == "ProfDD":
        ...
        elif ddmethod == "ProfDD":
            reducer_instance.reduce(mod="ProfDD")
        else:
            raise ValueError(
                "Invalid ddmethod. Choose from ddmin, onlycomplement, CDD, or ProfDD."
            )
