  ...
  if (smallConst && smallConst.getValue().isSplat()) {
    ...
    if (elType.isF32() || elType.isF64()) {
      ...
      if (fValue < eps && fValue > 0) {
        ...
        // n/N
        auto newRhs = kernel::hlo::Constant(
            sctx, static_cast<float>(static_cast<float>(n) / N),
        ...
        // truncate(x*n/N, k)
        auto f = getFieldTypeFromDataType(mulRet.dtype());
        addValue(sscope, op.getResult(),
                 kernel::hal::_trunc(sctx, mulRet, sctx->getFxpBits(f) + k)
