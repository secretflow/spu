...
class IoClient {
 ...
 public:
  ...
  std::vector<spu::MemRef> makeShares(const PtBufferView &bv, Visibility vtype,
                                      int owner_rank = -1);
  ...
  void combineShares(const std::vector<MemRef> &values, PtBufferView *out);
};
...
class ColocatedIo {
 ...
 public:
  ...
  yacl::Buffer hostGetVar(const std::string &name) const;

  SymbolTable &deviceSymbols() { return symbols_; }

  //
  void deviceSetVar(const std::string &name, const spu::MemRef &var,
                    PtType pt_type);

  //
  spu::MemRef deviceGetVar(const std::string &name) const;
