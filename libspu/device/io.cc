...
size_t IoClient::getShareSize(const PtBufferView &bv, Visibility vtype,
  ...
  if (bv.pt_type == PT_BOOL && vtype == VIS_SECRET &&
    ...
    return base_io_
               ->getShareType(vtype, getFieldFromPlainTextType(bv.pt_type),
                              owner_rank)
               .size() *
           bv.shape.numel();
  }
...
std::vector<spu::Value> IoClient::makeShares(const PtBufferView &bv,
  ...
  FieldType field = getFieldFromPlainTextType(bv.pt_type);
  const size_t fxp_bits = config_.fxp_fraction_bits().at(FieldType_Name(field));
  ...
  DataType dtype;
  NdArrayRef encoded =
      encodeToRing(convertToNdArray(bv), field, fxp_bits, &dtype);
...
NdArrayRef IoClient::combineShares(absl::Span<spu::Value const> values) {
  ...
  const size_t fxp_bits = config_.fxp_fraction_bits().at(
      FieldType_Name(getFieldTypeFromDataType(values.front().dtype())));
  SPU_ENFORCE(fxp_bits != 0, "fxp should never be zero, please check default");
