...
struct ErrorHandlerGuard {
  ...
  ~ErrorHandlerGuard() { llvm::remove_fatal_error_handler(); }
};
...
CompilationContext::CompilationContext(CompilerOptions options,
  ...
  if (ctx != nullptr) {
    context_ = ctx;
    owing_context_ = false;
  } else {
    context_ = new mlir::MLIRContext;
  }

  // Set an error handler
  [[maybe_unused]] static ErrorHandlerGuard _;
}
...
bool CompilationContext::emitByteCode() const {
  return !options_.disable_emit_bytecode();
}
