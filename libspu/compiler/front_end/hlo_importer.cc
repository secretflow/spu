...
void runHloPasses(xla::HloModule *module,
  ...
  if (compiler_options.settrue_islayoutsensitive()) {
    options.set_is_layout_sensitive(true);
  }
  if (compiler_options.settrue_enabledotstrengthreduction()) {
    options.set_enable_dot_strength_reduction(true);
  }
  if (compiler_options.setfalse_supportsnoncanonicaldots()) {
    options.set_supports_non_canonical_dots(false);
  }
  if (compiler_options.setfalse_enabledottomultiplyrewrite()) {
    options.set_enable_dot_to_multiply_rewrite(false);
  }
  if (compiler_options.settrue_enablemovedotparamtorhs()) {
    options.set_enable_move_dot_param_to_rhs(true);
  }
  if (compiler_options.setfalse_enableconvsimplification()) {
    options.set_enable_conv_simplification(false);
  }
  if (compiler_options.setfalse_enableconvoperandswap()) {
    options.set_enable_conv_operand_swap(false);
  }
  if (compiler_options.settrue_enablescalarmultiplyreduction()) {
    options.set_enable_scalar_multiply_reduction(true);
  }
  if (compiler_options.settrue_enablefloatsarereal()) {
    options.set_enable_floats_are_real(true);
  }
  if (compiler_options.setfalse_enablewindowreducetoreducereplacement()) {
    options.set_enable_window_reduce_to_reduce_replacement(false);
  }
  if (compiler_options.setfalse_enablereduceofreshape()) {
    options.set_enable_reduce_of_reshape(false);
  }
  if (compiler_options.setfalse_enablenegativepaddingreplacement()) {
    options.set_enable_negative_padding_replacement(false);
  }
  if (compiler_options.setfalse_enablesinkbroadcast()) {
    options.set_enable_sink_broadcast(false);
  }
  if (compiler_options.setfalse_unconditionallysimplifyreduceoftransposeorreshape()) {
    options.set_unconditionally_simplify_reduce_of_transpose_or_reshape(false);
  }
  if (compiler_options.setless_verysmallgathersize()) {
    options.set_very_small_gather_size(3);
  }
  if (compiler_options.setmore_verysmallgathersize()) {
    options.set_very_small_gather_size(5);
  }
  if (compiler_options.settrue_minmaxpropagatenan()) {
    options.set_minmax_propagate_nan(true);
  }
  if (compiler_options.setfalse_enableunconditionalreduceofconcatreplacement()) {
    options.set_enable_unconditional_reduce_of_concat_replacement(false);
  }
  if (compiler_options.settrue_executingoncpu()) {
    options.set_executing_on_cpu(true);
  }
  if (compiler_options.settrue_useassociativereordering()) {
    options.set_use_associative_reordering(true);
  }
  if (compiler_options.setless_associativereorderingthreshold()) {
    options.set_associative_reordering_threshold(1.0);
  }
  if (compiler_options.setmore_associativereorderingthreshold()) {
    options.set_associative_reordering_threshold(3.0);
  }
  if (compiler_options.settrue_raisesliceandreducethroughdot()) {
    options.set_raise_slice_and_reduce_through_dot(true);
  }
  if (compiler_options.setless_raisesliceandreducethroughdotthreshold()) {
    options.set_raise_slice_and_reduce_through_dot_threshold(1.0);
  }
  if (compiler_options.setmore_raisesliceandreducethroughdotthreshold()) {
    options.set_raise_slice_and_reduce_through_dot_threshold(3.0);
  }
  if (compiler_options.settrue_useconvertconstantfolding()) {
    options.set_use_convert_constant_folding(true);
  }
  // if (compiler_options.setfalse_enableoptimizedsidinnderid()) {
  //   options.set_enable_optimize_ds_id_innderid(false);
  // }
  // if (compiler_options.setfalse_enableoptimizedsreshape()) {
  //   options.set_enable_optimize_ds_reshape(false);
  // }
  // if (compiler_options.setfalse_enableoptimizedstranspose()) {
  //   options.set_enable_optimize_ds_transpose(false);
  // }
  // if (compiler_options.setfalse_enableoptimizepowexp()) {
  //   options.set_enable_optimize_pow_exp(false);
  // }
  // if (compiler_options.setfalse_enableoptimizeselectpredtruefalse()) {
  //   options.set_enable_optimize_select_pred_true_false(false);
  // }
  // End of simplifier options
...
mlir::OwningOpRef<mlir::ModuleOp> HloImporter::parseXlaModuleFromString(
