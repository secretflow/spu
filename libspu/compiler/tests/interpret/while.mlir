...
func.func @while() {
  ...
  //   i += 1;
  // }
  %init_i = arith.constant dense<0> : tensor<i64>
  %init_sum = arith.constant dense<0> : tensor<i64>
  %one = arith.constant dense<1> : tensor<i64>
  %ten = arith.constant dense<10> : tensor<i64>
  ...
  %expected = arith.constant dense<10> : tensor<i64>
