...
TEST(UtilsTest, associative_scan) {
...

  {
    ...
    Value b = associative_scan(hal::_add, &ctx, a);
...

  {
    ...
    Value a = test::makeValue(&ctx, x, VIS_SECRET);
    Value b = associative_scan(hal::_mul, &ctx, a);
    auto ret =
        dump_public_as<int32_t>(&ctx, hal::reveal(&ctx, b), ctx.getFxpBits());
...

  {
    ...
    Value a = test::makeValue(&ctx, x, VIS_SECRET);
    Value b = associative_scan(hal::_and, &ctx, a);
...

  {
    ...
    Value a = test::makeValue(&ctx, x, VIS_SECRET);
    Value b = associative_scan(hal::_or, &ctx, a);
    auto ret = dump_public_as<bool>(&ctx, hal::reveal(&ctx, b));
