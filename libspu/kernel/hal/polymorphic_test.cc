...
TEST_P(LogisticTest, Logistic) {
  ...
  // public logistic
  {
    ...
    auto y = dump_public_as<float>(&ctx, c, ctx.getFxpBits());
  ...
  // secret logistic
  {
    ...
    auto y = dump_public_as<float>(&ctx, reveal(&ctx, c), ctx.getFxpBits());
    // low precision
    EXPECT_TRUE(xt::allclose(1.0 / (1.0 + xt::exp(-x)), y, 0.1, 0.5))
        << 1.0 / (1.0 + xt::exp(-x)) << std::endl
        << y;
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(SIGMOID_MM1, SIGMOID_SEG3, SIGMOID_REAL),
