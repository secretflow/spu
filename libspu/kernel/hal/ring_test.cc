...
TEST(RingTest, _bit_parity) {
  ...
  for (auto [init, exp] : para) {
    auto input = _make_p(&ctx, init, {3}, FM64).setDtype(DT_U64);
...
TEST(RingTest, _cast_ring) {
  auto ctx = test::makeSPUContext();
  std::vector<std::tuple<uint64_t, FieldType, uint64_t>> para = {
      {0x0505, FM32, 0x0505},
      {0x0505, FM64, 0x0505},
      {0x05050505, FM128, 0x05050505},
  };
  for (auto [init, ftype, exp] : para) {
    auto input = _make_p(&ctx, init, {3}, FM64);

    auto output = _cast_ring(&ctx, input, ftype);

    output.setDtype(DT_U64);
    auto p_ret = hal::dump_public_as<uint64_t>(&ctx, output);
    xt::xarray<uint64_t> expected{exp, exp, exp};
    EXPECT_EQ(p_ret, expected);
  }
}
