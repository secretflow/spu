...
Value _rand_p(SPUContext* ctx, const Shape& shape, FieldType ftype) {
  SPU_TRACE_HAL_DISP(ctx, shape);
  auto rnd = mpc::rand_p(ctx, shape, ftype);
  return rnd;
}

Value _rand_s(SPUContext* ctx, const Shape& shape, FieldType ftype) {
  SPU_TRACE_HAL_DISP(ctx, shape);
  auto rnd = mpc::rand_s(ctx, shape, ftype);
  return rnd;
}
...
Value _cast_ring_p(SPUContext* ctx, const Value& in, FieldType ftype) {
  SPU_TRACE_HAL_DISP(ctx, in, ftype);
  auto ret = mpc::cast_ring_p(ctx, in, ftype);
  return ret;
}

Value _cast_ring_s(SPUContext* ctx, const Value& in, FieldType ftype) {
  SPU_TRACE_HAL_DISP(ctx, in, ftype);
  auto ret = mpc::cast_ring_s(ctx, in, ftype);
  return ret;
}
