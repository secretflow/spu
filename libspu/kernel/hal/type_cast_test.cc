...
TEST(TypeCastTest, fxp2int) {
...

  {
    ...
    auto y = dump_public_as<int>(&ctx, _s2p(&ctx, c).setDtype(DT_I64));
    EXPECT_TRUE(xt::allclose(expected, y, 0.1, 0.5)) << x << std::endl
                                                     << expected << std::endl
                                                     << y;
  }
...
TEST(TypeCastTest, int2int) {
  const xt::xarray<int32_t> x = test::xt_random<int32_t>({5, 6});
  SPUContext ctx = test::makeSPUContext();

  {
    Value a = constant(&ctx, x, DT_I32);
    Value c = dtype_cast(&ctx, a, DT_I64);
    EXPECT_EQ(c.dtype(), DT_I64);

    auto y = dump_public_as<int64_t>(&ctx, c);
    EXPECT_TRUE(xt::allclose(x, y, 0.1, 0.5)) << x << std::endl << y;
  }

  {
    ...
    auto y = dump_public_as<int64_t>(&ctx, _s2p(&ctx, c).setDtype(DT_I64));
    EXPECT_TRUE(xt::allclose(x, y, 0.1, 0.5)) << x << std::endl << y;
  }
}

TEST(TypeCastTest, fxp2fxp) {
  const xt::xarray<float> x = {{0.0, 1.0, 5.0, 10.0, 100.0, 1000.0},
                               {0.1, 0.5, 0.7, 10.1, 10.5, 10.7},
                               {-0.0, -1.0, -5.0, -10.0, -100.0, -1000.0},
                               {-0.1, -0.5, -0.7, -10.1, -10.5, -10.7}};
  ...
  {
    Value a = constant(&ctx, x, DT_F32);
    Value c = dtype_cast(&ctx, a, DT_F64);
    EXPECT_EQ(c.dtype(), DT_F64);

    auto y = dump_public_as<double>(&ctx, c);
    EXPECT_TRUE(xt::allclose(x, y, 0.1, 0.5)) << x << std::endl << y;
  }

  {
    ...
    auto y = dump_public_as<double>(&ctx, _s2p(&ctx, c).setDtype(DT_F64));
    EXPECT_TRUE(xt::allclose(x, y, 0.1, 0.5)) << x << std::endl << y;
  }
}
