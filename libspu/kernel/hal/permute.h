...
using SimpleCompFn = std::function<spu::MemRef(
    SPUContext *, const spu::MemRef &, const spu::MemRef &)>;

using CompFn = std::function<spu::MemRef(absl::Span<const spu::MemRef>)>;

using Permute1dFn =
    std::function<std::vector<spu::MemRef>(absl::Span<const spu::MemRef>)>;
...
std::vector<spu::MemRef> sort1d(SPUContext *ctx,
                                absl::Span<spu::MemRef const> inputs,
                                const CompFn &cmp,
...
std::vector<spu::MemRef> simple_sort1d(SPUContext *ctx,
                                       absl::Span<spu::MemRef const> inputs,
...
// 1-d array
std::vector<spu::MemRef> permute(SPUContext *ctx,
...
//  -config: topk config
std::vector<MemRef> topk_1d(SPUContext *ctx, const spu::MemRef &input,
