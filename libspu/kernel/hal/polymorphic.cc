...
Value mixed_mul(SPUContext* ctx, const Value& x, const Value& y) {
  ...
  if (x.isInt()) {
    auto new_dtype = y.dtype();

    // cast int to fxp field
    auto f_field = getFieldTypeFromDataType(new_dtype);
    auto x_c = _cast_ring(ctx, x, f_field);

    return _mul(ctx, x_c, y).setDtype(new_dtype);
  } else {
    auto new_dtype = x.dtype();

    // cast int to fxp field
    auto f_field = getFieldTypeFromDataType(new_dtype);
    auto y_c = _cast_ring(ctx, y, f_field);

    return _mul(ctx, x, y_c).setDtype(new_dtype);
  }
}
...
Value mixed_mmul(SPUContext* ctx, const Value& x, const Value& y) {
  ...
  if (x.isInt()) {
    ...
    return _mmul(ctx, x_c, y).setDtype(new_dtype);
  } else {
    auto new_dtype = x.dtype();
    ...
    return _mmul(ctx, x, y_c).setDtype(new_dtype);
  }
}
...
Value logical_not(SPUContext* ctx, const Value& in) {
  ...
  auto _k1 = _constant(ctx, 1, in.shape(), in.dtype());
...
Value sign(SPUContext* ctx, const Value& x) {
  ...
  return dtype_cast(ctx, _sign(ctx, x), DT_I8);
}
