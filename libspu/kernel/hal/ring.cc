...
#define IMPL_UNARY_OP(Name, FnP, FnS)                       \
  Value Name(SPUContext* ctx, const Value& in) {            \
    ...
    if (in.isPublic()) {                                    \
      return FnP(ctx, in).setDtype(in.dtype());             \
    } else if (in.isSecret()) {                             \
      return FnS(ctx, in).setDtype(in.dtype());             \
    } else {                                                \
...
#define IMPL_SHIFT_OP(Name, FnP, FnS)                         \
  Value Name(SPUContext* ctx, const Value& in, size_t bits) { \
    ...
    if (in.isPublic()) {                                      \
      return FnP(ctx, in, bits).setDtype(in.dtype());         \
    } else if (in.isSecret()) {                               \
      return FnS(ctx, in, bits).setDtype(in.dtype());         \
    } else {                                                  \
      SPU_THROW("unsupport unary op={} for {}", #Name, in);   \
    }                                                         \
  }
...
#define IMPL_COMMUTATIVE_BINARY_OP(Name, FnPP, FnSP, FnSS)        \
  Value Name(SPUContext* ctx, const Value& x, const Value& y) {   \
    ...
    auto ret_dtype = std::max(x.dtype(), y.dtype());              \
    if (x.isPublic() && y.isPublic()) {                           \
      return FnPP(ctx, x, y).setDtype(ret_dtype);                 \
    } else if (x.isSecret() && y.isPublic()) {                    \
      return FnSP(ctx, x, y).setDtype(ret_dtype);                 \
    } else if (x.isPublic() && y.isSecret()) {                    \
      /* commutative, swap args */                                \
      return FnSP(ctx, y, x).setDtype(ret_dtype);                 \
    } else if (x.isSecret() && y.isSecret()) {                    \
      return FnSS(ctx, y, x).setDtype(ret_dtype);                 \
    } else {                                                      \
...
Value _msb(SPUContext* ctx, const Value& in) {
  ...
  auto to_dtype = getUIntegerTypeFromFieldType(from_field);
  if (in.isPublic()) {
    return _msb_p(ctx, in).setDtype(to_dtype);
  } else if (in.isSecret()) {
    return _msb_s(ctx, in).setDtype(to_dtype);
  } else {
    SPU_THROW("unsupport unary op={} for {}", "msb", in);
  }
}
...
[[maybe_unused]] Value _eqz(SPUContext* ctx, const Value& x) {
  ...
  auto _k1 = _constant(ctx, 1U, x.shape(), x.dtype());
...
static Value _mmul_impl(SPUContext* ctx, const Value& x, const Value& y) {
  auto ret_dtype = std::max(x.dtype(), y.dtype());
  if (x.isPublic() && y.isPublic()) {
    return _mmul_pp(ctx, x, y).setDtype(ret_dtype);
  } else if (x.isSecret() && y.isPublic()) {
    return _mmul_sp(ctx, x, y).setDtype(ret_dtype);
  } else if (x.isPublic() && y.isSecret()) {
    return transpose(ctx, _mmul_sp(ctx, transpose(ctx, y), transpose(ctx, x))
                              .setDtype(ret_dtype));
  } else if (x.isSecret() && y.isSecret()) {
    return _mmul_ss(ctx, x, y).setDtype(ret_dtype);
  } else {
...
static std::optional<Value> _equal_impl(SPUContext* ctx, const Value& x,
  ...
  auto in_field = getFieldTypeFromDataType(x.dtype());
  auto out_dtype = getUIntegerTypeFromFieldType(in_field);

  std::optional<Value> ret;
  if (x.isPublic() && y.isPublic()) {
    ret = _equal_pp(ctx, x, y);
  } else if (x.isSecret() && y.isPublic()) {
    ret = _equal_sp(ctx, x, y);
  } else if (x.isPublic() && y.isSecret()) { /* commutative, swap args */
    ret = _equal_sp(ctx, y, x);
  } else if (x.isSecret() && y.isSecret()) {
    ret = _equal_ss(ctx, y, x);
  }

  if (ret) {
    ret->setDtype(out_dtype);
  }
...
Value _equal(SPUContext* ctx, const Value& x, const Value& y) {
  ...
  auto l1 = _less(ctx, x, y);
  auto l2 = _less(ctx, y, x);
  const auto _k1 = _constant(ctx, 1, x.shape(), l1.dtype());

  return _and(ctx, _xor(ctx, l1, _k1), _xor(ctx, l2, _k1));
}
...
Value _negate(SPUContext* ctx, const Value& x) {
  ...
  // negate(x) = not(x) + 1
  return _add(ctx, _not(ctx, x), _constant(ctx, 1, x.shape(), x.dtype()));
}
...
Value _sign(SPUContext* ctx, const Value& x) {
  ...
  //      = +1 ,if x >= 0
  //      = -1 ,if x < 0
  const auto one = _constant(ctx, 1, x.shape(), x.dtype());
  const auto two = _constant(ctx, 2, x.shape(), x.dtype());
...
Value _bitrev(SPUContext* ctx, const Value& x, size_t start, size_t end) {
  ...
  if (x.isPublic()) {
    return _bitrev_p(ctx, x, start, end).setDtype(x.dtype());
  } else if (x.isSecret()) {
    return _bitrev_s(ctx, x, start, end).setDtype(x.dtype());
  }
...
Value _mux(SPUContext* ctx, const Value& pred, const Value& a, const Value& b) {
  ...
  // b + pred*(a-b)
  auto p_pred = _cast_ring(ctx, pred, getFieldTypeFromDataType(a.dtype()));
  return _add(ctx, b, _mul(ctx, p_pred, _sub(ctx, a, b)));
}
...
Value _constant(SPUContext* ctx, uint128_t init, const Shape& shape,
                DataType dtype) {
  auto field = getFieldTypeFromDataType(dtype);
  return _make_p(ctx, init, shape, field).setDtype(dtype);
}
...
Value _bit_parity(SPUContext* ctx, const Value& x, size_t bits) {
  ...
  ret = _and(ctx, ret, _constant(ctx, 1, x.shape(), x.dtype()));
  return ret;
}
...
Value _prefix_or(SPUContext* ctx, const Value& x) {
  ...
  auto field = getFieldTypeFromDataType(x.dtype());
...
Value _bitdeintl(SPUContext* ctx, const Value& in) {
  ...
  //      0000000011111111
  Value out = in;
  auto field = getFieldTypeFromDataType(in.dtype());
  const size_t k = SizeOf(field) * 8;
  for (int64_t idx = 0; idx + 1 < Log2Ceil(k); idx++) {
    auto keep =
        _constant(ctx, detail::kBitIntlKeepMasks[idx], in.shape(), in.dtype());
    auto move =
        _constant(ctx, detail::kBitIntlSwapMasks[idx], in.shape(), in.dtype());
    int64_t shift = 1 << idx;
...
Value _prefer_a(SPUContext* ctx, const Value& x) {
  if (x.storage_type().isa<BShare>()) {
    ...
    // B2A
    return _add(ctx, x, _constant(ctx, 0, x.shape(), x.dtype()))
        .setDtype(x.dtype());
  }
...
Value _prefer_b(SPUContext* ctx, const Value& x) {
  if (x.storage_type().isa<AShare>()) {
    const auto k0 = _constant(ctx, 0U, x.shape(), x.dtype());
...
// Assumption
//   ring_bits \in {32, 64, 128}
// This is out of the consideration that data types like int32/int64 are
// natively supported in C++
Value _cast_ring(SPUContext* ctx, const Value& x, FieldType ftype) {
  SPU_TRACE_HAL_LEAF(ctx, x, ftype);

  auto x_field = getFieldTypeFromDataType(x.dtype());
  if (x_field == ftype) {
    return x;
  }
  ...
  if (x.isPublic()) {
    // static_cast
    return _cast_ring_p(ctx, x, ftype);
  } else if (x.isSecret()) {
    // Dispatch to prot wrapper
    return _cast_ring_s(ctx, x, ftype);
  } else {
    SPU_THROW("cast_down input error x={}", x);
  }
}
