...
Value polynomial(SPUContext* ctx, const Value& x,
  ...
  auto fbits = ctx->getFxpBits(getFieldTypeFromDataType(x.dtype()));
  ...
  return _trunc(ctx, res, fbits).setDtype(x.dtype());
}
...
Value div_goldschmidt(SPUContext* ctx, const Value& a, const Value& b) {
  ...
  auto b_abs = _mux(ctx, is_negative, _negate(ctx, b), b);

  auto b_msb = detail::highestOneBit(ctx, b_abs);

  // factor = 2^{2f-m} = 2^{f-m} * 2^f, the fixed point repr of 2^{f-m}
  const size_t num_fxp_bits =
      ctx->getFxpBits(getFieldTypeFromDataType(b.dtype()));
  auto factor = _bitrev(ctx, b_msb, 0, 2 * num_fxp_bits);
  ...
  const auto k2 = _constant(ctx, 2, c.shape(), c.dtype());
  const auto k2_9142 = constant(ctx, 2.9142F, b.dtype(), c.shape());
  auto w = f_sub(ctx, k2_9142, _mul(ctx, k2, c));
  ...
  return _mux(ctx, is_negative, _negate(ctx, r), r);
}
...
Value reciprocal_goldschmidt_positive(SPUContext* ctx, const Value& b_abs) {
  ...
  // factor = 2^{2f-m} = 2^{f-m} * 2^f, the fixed point repr of 2^{f-m}
  const size_t num_fxp_bits =
      ctx->getFxpBits(getFieldTypeFromDataType(b_abs.dtype()));
  auto factor = _bitrev(ctx, b_msb, 0, 2 * num_fxp_bits);
  detail::hintNumberOfBits(factor, 2 * num_fxp_bits);
  ...
  auto w = f_mul(ctx, f_sub(ctx, k2_9142, _mul(ctx, k2, c)), factor);
...
Value f_mul(SPUContext* ctx, const Value& x, const Value& y, SignType sign) {
  ...
  auto fxp_bits = ctx->getFxpBits(getFieldTypeFromDataType(x.dtype()));

  return _trunc(ctx, _mul(ctx, x, y), fxp_bits, sign).setDtype(x.dtype());
}
...
Value f_mmul(SPUContext* ctx, const Value& x, const Value& y) {
  ...
  return _trunc(ctx, _mmul(ctx, x, y), fxp_bits).setDtype(x.dtype());
}
...
Value f_conv2d(SPUContext* ctx, const Value& x, const Value& y,
  ...
  return _trunc(ctx, _conv2d(ctx, x, y, window_strides), fxp_bits)
      .setDtype(x.dtype());
}
...
Value f_tensordot(SPUContext* ctx, const Value& x, const Value& y,
  ...
  return _trunc(ctx, _tensordot(ctx, x, y, ix, iy), fxp_bits)
      .setDtype(x.dtype());
}
...
Value f_equal(SPUContext* ctx, const Value& x, const Value& y) {
  ...
  return dtype_cast(ctx, _equal(ctx, x, y), DT_I1);
}
...
Value f_less(SPUContext* ctx, const Value& x, const Value& y) {
  ...
  return dtype_cast(ctx, _less(ctx, x, y), DT_I1);
}
...
Value f_square(SPUContext* ctx, const Value& x) {
  ...
  SPU_ENFORCE(x.isFxp());
  return _trunc(ctx, _mul(ctx, x, x),
                ctx->getFxpBits(getFieldTypeFromDataType(x.dtype())),
                SignType::Positive)
      .setDtype(x.dtype());
}
...
Value f_floor(SPUContext* ctx, const Value& x) {
  ...
  const size_t fbits = ctx->getFxpBits(getFieldTypeFromDataType(x.dtype()));
