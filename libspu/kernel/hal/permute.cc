...
inline int64_t _get_owner(const MemRef &x) {
  return x.eltype().as<Private>()->owner();
}

inline bool _has_same_owner(const MemRef &x, const MemRef &y) {
  return _get_owner(x) == _get_owner(y);
}

void _hint_nbits(const MemRef &a, size_t nbits) {
...
MemRef _2s(SPUContext *ctx, const MemRef &x) {
  if (x.isPublic()) {
    return _p2s(ctx, x);
  } else if (x.isPrivate()) {
    return _v2s(ctx, x);
...
MemRef _permute_1d(SPUContext *, const MemRef &x, const Index &indices) {
  SPU_ENFORCE(x.shape().size() == 1);
  return MemRef(x.linear_gather(indices));
}

MemRef _prefix_sum(SPUContext *ctx, const MemRef &x) {
...
void _cmp_swap(SPUContext *ctx, const CompFn &comparator_body,
               absl::Span<MemRef> values_to_sort, const Index &lhs_indices,
               const Index &rhs_indices) {
  size_t num_operands = values_to_sort.size();

  std::vector<MemRef> values;
  values.reserve(2 * num_operands);
  for (size_t i = 0; i < num_operands; ++i) {
    values.emplace_back(values_to_sort[i].linear_gather(lhs_indices));
    values.emplace_back(values_to_sort[i].linear_gather(rhs_indices));
  }

  MemRef predicate = comparator_body(values);
  ...
  for (size_t i = 0; i < num_operands; ++i) {
    ...
    auto greater = _mux(ctx, predicate, fst, sec);
    auto less = _add(ctx, _add(ctx, fst, sec), _negate(ctx, greater));

    values_to_sort[i].linear_scatter(greater, lhs_indices);
    values_to_sort[i].linear_scatter(less, rhs_indices);
  }
}
...
std::vector<MemRef> odd_even_merge_sort(SPUContext *ctx,
                                        const CompFn &comparator_body,
                                        absl::Span<MemRef const> inputs) {
  // make a copy for inplace sort
  std::vector<MemRef> ret;
  for (auto const &input : inputs) {
    MemRef casted;
    if (!input.isSecret()) {
      // we can not linear_scatter a secret value to a public operand
      casted = _2s(ctx, input.clone());
    } else {
      casted = input.clone();
    }
...
void Swap(absl::Span<MemRef> arr, const Index &lhs_indices,
          const Index &rhs_indices) {
  ...
  for (size_t i = 0; i < num_operands; ++i) {
    auto lhs_arr = arr[i].linear_gather(lhs_indices);
    auto rhs_arr = arr[i].linear_gather(rhs_indices);

    arr[i].linear_scatter(lhs_arr, rhs_indices);
    arr[i].linear_scatter(rhs_arr, lhs_indices);
  }
}

void CompSwapSingle(SPUContext *ctx, const CompFn &comparator_body,
                    absl::Span<MemRef> arr, int64_t lo, int64_t hi,
  ...
  auto _predicate = dump_public_as<bool>(ctx, hal::reveal(ctx, predicate))[0];
...
void TwoWayPartition(SPUContext *ctx, const CompFn &comparator_body,
  ...
  values.push_back(broadcast_to(ctx, slice_scalar_at(ctx, arr[0], {lo}),
                                {right - left + 1}));
  values.push_back(slice(ctx, arr[0], {left}, {right - left + 1}));
  if (config.confusion) {
    values.push_back(broadcast_to(ctx, slice_scalar_at(ctx, arr[1], {lo}),
                                  {right - left + 1}));
    values.push_back(slice(ctx, arr[1], {left}, {right - left + 1}));
  }
...
std::vector<MemRef> QuickSelectTopk(SPUContext *ctx,
                                    const CompFn &comparator_body,
                                    absl::Span<MemRef> input,
                                    const TopKConfig &config) {
...
std::vector<MemRef> PrepareInput(SPUContext *ctx, const MemRef &input,
                                 const TopKConfig &config) {
  std::vector<MemRef> inp;

  // shuffle with random permutation to break link of values
  auto rand_perm = _rand_perm_s(ctx, input.shape());
  inp.push_back(_perm_ss(ctx, input, rand_perm));
  ...
  if (config.confusion) {
    inp.push_back(_rand_s(ctx, input.eltype().semantic_type(), input.shape()));
  }

  if (!config.value_only) {
    // shuffle index with the same permutation as values
    inp.push_back(_perm_ss(
        ctx, _p2s(ctx, hal::_iota(ctx, PT_I32, input.numel())), rand_perm));
  }
...
std::pair<std::vector<MemRef>, MemRef> _opt_apply_inv_perm_ss(
    SPUContext *ctx, absl::Span<MemRef const> x, const MemRef &perm,
    const MemRef &random_perm) {
  // 1. <SP> = secure shuffle <perm>
  auto sp = hal::_perm_ss(ctx, perm, random_perm);

  // 2. <SX> = secure shuffle <x>
  std::vector<MemRef> sx;
...
//      res = [1, 0]
MemRef _gen_inv_perm_by_bv(SPUContext *ctx, const MemRef &x, const MemRef &y) {
  SPU_ENFORCE(x.shape() == y.shape(), "x and y should has the same shape");
  SPU_ENFORCE(x.shape().ndim() == 1, "x and y should be 1-d");

  const auto k1 = _constant(ctx, 1U, x.eltype().semantic_type(), x.shape());
  ...
  auto fs1 = slice(ctx, fs, {0, numel}, {1, numel}, {});
  auto fs2 = slice(ctx, fs, {0, 2 * numel}, {1, numel}, {});
  auto fs3 = slice(ctx, fs, {0, 3 * numel}, {1, numel}, {});
...
//   8) get res by sub r by one
//      res = [3, 0, 4, 1, 2]
MemRef _gen_inv_perm_by_bv(SPUContext *ctx, const MemRef &x) {
...
MemRef _opt_apply_perm_ss(SPUContext *ctx, const MemRef &perm,
                          const MemRef &shuffled_perm,
                          const MemRef &random_perm) {
...
std::vector<MemRef> _bit_decompose(SPUContext *ctx, const MemRef &x,
                                   int64_t valid_bits) {
  auto x_bshare = _prefer_b(ctx, x);
  size_t nbits = valid_bits != -1
                     ? static_cast<size_t>(valid_bits)
                     : x_bshare.eltype().as<BaseRingType>()->valid_bits();
  _hint_nbits(x_bshare, nbits);
  if (ctx->hasKernel("b2a_disassemble")) {
    auto ret =
        dynDispatch<std::vector<MemRef>>(ctx, "b2a_disassemble", x_bshare);
    return ret;
  }

  const auto k1 = _constant(ctx, 1U, x.eltype().semantic_type(), x.shape());
  std::vector<MemRef> rets_b;
  rets_b.reserve(nbits);

  for (size_t bit = 0; bit < nbits; ++bit) {
    auto x_bshare_shift = _rshift(ctx, x_bshare, {static_cast<int64_t>(bit)});
    rets_b.push_back(_and(ctx, x_bshare_shift, k1));
  }

  std::vector<MemRef> rets_a;
  vmap(rets_b.begin(), rets_b.end(), std::back_inserter(rets_a),
       [&](const MemRef &x) { return _prefer_a(ctx, x); });
  return rets_a;
}

// Generate vector of bit decomposition of sorting keys
std::vector<MemRef> _gen_bv_vector(SPUContext *ctx,
                                   absl::Span<MemRef const> keys,
  ...
  for (size_t i = keys.size(); i > 0; --i) {
    ...
    for (size_t j = 0; j < t.size() - 1; j++) {
      ...
      if (direction == SortDirection::Descending) {
        const auto k1 =
            _constant(ctx, 1U, t[j].eltype().semantic_type(), keys[0].shape());
    ...
    if (direction == SortDirection::Descending) {
      ...
      const auto k1 = _constant(ctx, 1U, t.back().eltype().semantic_type(),
                                keys[0].shape());
...
MemRef _gen_inv_perm_s(SPUContext *ctx, absl::Span<MemRef const> keys,
                       SortDirection direction, int64_t valid_bits) {
  // 1. generate bit decomposition vector of keys
  std::vector<MemRef> bv = _gen_bv_vector(ctx, keys, direction, valid_bits);
  SPU_ENFORCE_GT(bv.size(), 0U);

  // 2. generate natural permutation for initialization
  auto init_perm = _iota(ctx, PT_I32, keys[0].numel());
  ...
  for (; bv_idx < bv_size - 1; bv_idx += 2) {
    ...
    auto [shuffled_bv, shuffled_perm] = _opt_apply_inv_perm_ss(
        ctx, std::vector<MemRef>{bv[bv_idx], bv[bv_idx + 1]}, shared_perm,
        random_perm);
  ...
  if (bv_idx == bv_size - 1) {
    ...
    auto random_perm = hal::_rand_perm_s(ctx, keys[0].shape());
    auto [shuffled_bv, shuffled_perm] = _opt_apply_inv_perm_ss(
        ctx, std::vector<MemRef>{bv[bv_idx]}, shared_perm, random_perm);
...
MemRef _gen_inv_perm_s(SPUContext *ctx, const MemRef &key, bool is_ascending,
                       int64_t valid_bits) {
  std::vector<MemRef> keys{key};
  auto direction =
...
// permutation <perm>
std::vector<MemRef> _apply_inv_perm_ss(SPUContext *ctx,
                                       absl::Span<MemRef const> x,
                                       const MemRef &perm) {
  // 1. <SP> = secure shuffle <perm>
...
MemRef _apply_inv_perm_ss(SPUContext *ctx, const MemRef &x,
                          const MemRef &perm) {
  std::vector<MemRef> inputs{x};
...
MemRef _apply_perm_ss(SPUContext *ctx, const MemRef &x, const MemRef &perm) {
  // 1. <SP> = secure shuffle <perm>
...
MemRef _gen_inv_perm(SPUContext *ctx, const MemRef &in, bool is_ascending,
...
MemRef _apply_inv_perm_sv(SPUContext *ctx, const MemRef &in,
                          const MemRef &perm) {
...
#define MAP_APPLY_PERM_OP(NAME)                                                \
  MemRef _apply##NAME(SPUContext *ctx, const MemRef &in, const MemRef &perm) { \
...
#define MAP_PERM_OP(NAME)                                              \
  MemRef NAME(SPUContext *ctx, const MemRef &in, const MemRef &perm) { \
...
// ret[perm[i]] = i
MemRef _inverse(SPUContext *ctx, const MemRef &perm) {
  auto iota_perm =
      _iota(ctx, PT_I32, perm.numel(), ctx->config().fxp_fraction_bits());
  return _apply_inv_perm(ctx, iota_perm, perm);
}

MemRef _apply_perm_sv(SPUContext *ctx, const MemRef &in, const MemRef &perm) {
  if (ctx->hasKernel("inv_perm_av")) {
...
MemRef _compose_perm(SPUContext *ctx, const MemRef &x, const MemRef &y) {
  return _apply_perm(ctx, y, x);
}

MemRef _merge_keys(SPUContext *ctx, absl::Span<MemRef const> inputs,
                   bool is_ascending) {
  if (inputs[0].isPublic()) {
    SPU_ENFORCE(std::all_of(inputs.begin(), inputs.end(),
                            [](const MemRef &v) { return v.isPublic(); }),
...
MemRef _merge_pub_pri_keys(SPUContext *ctx, absl::Span<MemRef const> keys,
                           bool is_ascending) {
  SPU_ENFORCE(std::all_of(keys.begin(), keys.end(),
                          [](const MemRef &v) { return !v.isSecret(); }),
...
// radix sort.
MemRef gen_inv_perm(SPUContext *ctx, absl::Span<MemRef const> inputs,
...
std::vector<MemRef> apply_inv_perm(SPUContext *ctx,
                                   absl::Span<MemRef const> inputs,
                                   const MemRef &perm) {
  if (perm.isSecret()) {
    std::vector<MemRef> inputs_s;
    for (const auto &input : inputs) {
      inputs_s.emplace_back(_2s(ctx, input));
    }
    ...
    if (ctx->hasKernel("inv_perm_av")) {
      std::vector<MemRef> ret;
      for (const auto &input : inputs) {
        ret.emplace_back(_apply_inv_perm(ctx, input, perm));
      }
      return ret;
    } else {
      std::vector<MemRef> inputs_s;
...
std::vector<MemRef> radix_sort(SPUContext *ctx, absl::Span<MemRef const> inputs,
                               SortDirection direction, int64_t num_keys,
                               int64_t valid_bits) {
...
std::vector<MemRef> sort1d(SPUContext *ctx, absl::Span<MemRef const> inputs,
  ...
  if (comparator_ret_vis == VIS_PUBLIC) {
    ...
    auto comparator = [&cmp, &inputs, &ctx](int64_t a, int64_t b) {
      ...
      for (const auto &input : inputs) {
        auto lhs = hal::slice(ctx, input, {a}, {1});
        auto rhs = hal::slice(ctx, input, {b}, {1});
        lhs = hal::reshape(ctx, lhs, {});
        rhs = hal::reshape(ctx, rhs, {});
        values.emplace_back(lhs);
        values.emplace_back(rhs);
      }
      MemRef cmp_ret = cmp(values);
      return dump_public_as<bool>(ctx, cmp_ret)[0];
    };
...
std::vector<MemRef> simple_sort1d(SPUContext *ctx,
                                  absl::Span<MemRef const> inputs,
                                  SortDirection direction, int64_t num_keys,
                                  int64_t valid_bits) {
  // sanity check.
  ...
  if (!fallback) {
    ...
    auto scalar_cmp = [direction](spu::SPUContext *ctx, const MemRef &lhs,
                                  const MemRef &rhs) {
      if (direction == SortDirection::Ascending) {
        return hal::_less(ctx, lhs, rhs);
      }
      return hal::_less(ctx, rhs, lhs);
    };

    hal::CompFn comp_fn = [ctx, num_keys, &scalar_cmp](
                              absl::Span<const MemRef> values) -> MemRef {
      MemRef pre_equal =
          broadcast_to(ctx, _encode_int(ctx, true, SE_1), values[0].shape());
      MemRef result = scalar_cmp(ctx, values[0], values[1]);
      ...
      for (int64_t idx = 2; idx < num_keys * 2; idx += 2) {
        pre_equal = hal::_and(
            ctx, pre_equal, hal::_equal(ctx, values[idx - 2], values[idx - 1]));
        auto current = scalar_cmp(ctx, values[idx], values[idx + 1]);
        current = hal::_and(ctx, pre_equal, current);
        result = hal::_or(ctx, result, current);
      }
      return result;
    };
...
std::vector<MemRef> permute(SPUContext *ctx, absl::Span<const MemRef> inputs,
                            int64_t permute_dim,
  ...
  if (W == 0) {
    return std::vector<MemRef>(inputs.begin(), inputs.end());
  }
  ...
  // result is (M,shape)
  std::vector<MemRef> results(M);
  for (int64_t mi = 0; mi < M; mi++) {
    std::vector<MemRef> output2d;
    for (int64_t ni = 0; ni < N; ni++) {
...
std::vector<MemRef> topk_1d(SPUContext *ctx, const MemRef &input,
  ...
  if (input.isPublic()) {
    ...
    auto comparator = [&scalar_cmp, &input, &ctx](int64_t a, int64_t b) {
      ...
      MemRef cmp_ret = scalar_cmp(ctx, lhs_value, rhs_value);
      return dump_public_as<bool>(ctx, cmp_ret)[0];
    };
    ...
    ret.emplace_back(internal::_permute_1d(ctx, input, topk_indices));
    if (!config.value_only) {
      ret.emplace_back(_encode_int(ctx, topk_indices, SE_I64));
    }
  ...
  if (ctx->hasKernel("rand_perm_m") && ctx->hasKernel("perm_am")) {
    ...
    hal::CompFn comp_fn =
        [ctx, &scalar_cmp](absl::Span<const MemRef> values) -> MemRef {
      ...
      auto eq = hal::_equal(ctx, values[0], values[1]);

      // comparision of random value
      auto result = scalar_cmp(ctx, values[2], values[3]);
      result = hal::_and(ctx, eq, result);
      result = hal::_or(ctx, cmp, result);
      return result;
    };
    ...
    inp.push_back(input);
    if (!config.value_only) {
      inp.push_back(_p2s(ctx, hal::_iota(ctx, PT_I32, input.numel())));
    }
