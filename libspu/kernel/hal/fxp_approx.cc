...
Value log2_pade_normalized(SPUContext* ctx, const Value& x) {
  ...
  auto nbits = ctx->getFxpBits(getFieldTypeFromDataType(x.dtype()));
  p2524 = _add(ctx, p2524, _mul(ctx, x2, p2));
  p2524 = _add(ctx, p2524, _mul(ctx, x3, p3));
  p2524 = _add(ctx, _trunc(ctx, p2524, nbits), p0).setDtype(x.dtype());

  auto q2524 = _mul(ctx, x, q1);
  q2524 = _add(ctx, q2524, _mul(ctx, x2, q2));
  q2524 = _add(ctx, q2524, _mul(ctx, x3, q3));
  q2524 = _add(ctx, _trunc(ctx, q2524, nbits), q0).setDtype(x.dtype());
...
Value log2_pade(SPUContext* ctx, const Value& x) {
  ...
  const size_t num_fxp_bits = ctx->getFxpBits(field);
  ...
  return _add(ctx, log2_pade_normalized(ctx, norm),
              _lshift(ctx,
                      _sub(ctx, k,
                           _constant(ctx, num_fxp_bits, x.shape(), x.dtype())),
                      num_fxp_bits))
      .setDtype(x.dtype());
}
...
Value exp2_pade_normalized(SPUContext* ctx, const Value& x) {
  ...
  return _add(ctx, _trunc(ctx, res, nbits), p0).setDtype(x.dtype());
}
...
Value exp2_pade(SPUContext* ctx, const Value& x) {
  auto field = getFieldTypeFromDataType(x.dtype());
  const size_t fbits = ctx->getFxpBits(field);
  const auto k1 = _constant(ctx, 1U, x.shape(), x.dtype());
  ...
  for (size_t idx = 0; idx < int_bits; idx++) {
    ...
    ret = _mul(ctx, ret,
               _add(ctx, _mul(ctx, a, _constant(ctx, K, x.shape(), x.dtype())),
                    _sub(ctx, k1, a)))
              .setDtype(ret.dtype());
  }
...
Value compute_chebyshev_polynomials(SPUContext* ctx, const Value& x,
  ...
  auto i_type = getIntegerTypeFromFieldType(x_field);

  // y = 4*x^2 - 2
  auto four = dtype_cast(ctx, constant(ctx, 4, DT_I32, x.shape()), i_type);
...
static Value rsqrt_init_guess(SPUContext* ctx, const Value& x, const Value& z) {
  ...
  auto field = getFieldTypeFromDataType(x.dtype());
  const size_t f = ctx->getFxpBits(field);

  // let u in [0.25, 0.5)
  auto z_rev = _bitrev(ctx, z, 0, 2 * f);
  detail::hintNumberOfBits(z_rev, 2 * f);

  auto u = _trunc(ctx, _mul(ctx, x, z_rev), f).setDtype(x.dtype());
...
static Value rsqrt_comp(SPUContext* ctx, const Value& x, const Value& z) {
  ...
  const size_t f = ctx->getFxpBits(field);
  auto mask_type = getUIntegerTypeFromFieldType(field);
  ...
  Value b;
  {
    ...
    auto lo_mask = _constant(ctx, (static_cast<uint128_t>(1) << (k / 2)) - 1,
                             x.shape(), mask_type);
  ...
  Value c1;
  if (f % 2 == 1) {
    c0 = _constant(ctx, 1 << ((f + 3) / 2), x.shape(), mask_type);  // f+e even
    c1 = _constant(ctx, (1 << (f / 2 + 1)) * std::sqrt(2), x.shape(),
                   mask_type);  // f+e odd
  } else {
    c0 = _constant(ctx, (1 << (f / 2)) * std::sqrt(2), x.shape(),
                   mask_type);                                // f+e even
    c1 = _constant(ctx, 1 << (f / 2), x.shape(), mask_type);  // f+e odd
  }
...
Value f_rsqrt(SPUContext* ctx, const Value& x) {
  ...
  if (ctx->config().experimental_enable_intra_op_par()) {
    ...
    return _trunc(ctx, _mul(ctx, r.get(), comp), nbits).setDtype(x.dtype());
  } else {
    auto r = rsqrt_init_guess(ctx, x, z);
    auto comp = rsqrt_comp(ctx, x, z);
    return _trunc(ctx, _mul(ctx, r, comp), nbits).setDtype(x.dtype());
  }
}
