...
class BenchConfig {
 public:
  ...
  inline static std::vector<int64_t> bench_field_range = {64, 128};
};
...
void MPCBenchMark(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    const auto field = static_cast<size_t>(state.range(0));
    RuntimeConfig conf;
    conf.mutable_protocol()->set_field(field);
 ...
 public:
  OpData(SPUContext* obj, benchmark::State& st) : obj_(obj), state(st) {
    ...
    for (auto& b1 : b1s) {
      ...
      b1 = lshift_b(obj_, b1,
                    {static_cast<int64_t>(
                        SizeOf(static_cast<size_t>(state.range(0))) * 8 - 1)});
      b1 = rshift_b(obj_, b1,
                    {static_cast<int64_t>(
                        SizeOf(static_cast<size_t>(state.range(0))) * 8 - 1)});
    }
...
MPC_BENCH_DEFINE(BenchNegateS, OpData1S, negate_s, ss[0])
MPC_BENCH_DEFINE(BenchNegateP, OpData1P, negate_p, ps[0])
...
MPC_BENCH_DEFINE(BenchNegateA, OpData1A, negate_a, as[0])
...
MPC_BENCH_DEFINE(BenchBitRevB, OpData1B, bitrev_b, bs[0], 0,
                 SizeOf(static_cast<size_t>(state.range(0))))
