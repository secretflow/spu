...
class OpData {
 ...
 public:
  OpData(SPUContext* obj, benchmark::State& st) : obj_(obj), state(st) {
    ...
    for (auto& p : ps) {
      p = rand_p(obj_, Shape{state.range(1)}, field);
    }
    for (auto& s : ss) {
      s = p2s(obj_, rand_p(obj, Shape{state.range(1)}, field));
    }
    for (auto& a : as) {
      a = p2a(obj_, rand_p(obj_, Shape{state.range(1)}, field));
    }
    for (auto& b : bs) {
      b = p2b(obj_, rand_p(obj_, Shape{state.range(1)}, field));
    }
    auto matrix_size = Shape{state.range(1), state.range(1)}.numel();
    for (auto& mp : mps) {
      mp = rand_p(obj_, Shape{matrix_size}, field);
    }
    for (auto& ms : mss) {
      ms = p2s(obj_, rand_p(obj_, Shape{matrix_size}, field));
    }
    for (auto& ma : mas) {
      ma = p2a(obj_, rand_p(obj_, Shape{matrix_size}, field));
    }
    for (auto& mb : mbs) {
      mb = p2b(obj_, rand_p(obj_, Shape{matrix_size}, field));
    }
    for (auto& b1 : b1s) {
      b1 = p2b(obj_, rand_p(obj_, Shape{state.range(1)}, field));
      b1 = lshift_b(obj_, b1,
