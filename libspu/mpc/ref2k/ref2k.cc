...
class Ref2kMulSS : public BinaryKernel {
 public:
  ...
  NdArrayRef proc(KernelEvalContext* ctx, const NdArrayRef& lhs,
                  const NdArrayRef& rhs) const override {
    SPU_TRACE_MPC_LEAF(ctx, lhs, rhs);
    SPU_ENFORCE(lhs.eltype() == rhs.eltype(), "lhs = {}, rhs = {}", lhs, rhs);
...
class Ref2kCastRing : public CastRingKernel {
 public:
  static constexpr char kBindName[] = "cast_ring_s";
  ...
  NdArrayRef proc(KernelEvalContext* ctx, const NdArrayRef& in,
    ...
    SPU_TRACE_MPC_LEAF(ctx, in);
    return cast_ring(in, makeType<Ref2kSecrTy>(ftype));
  }
};
...
void regRef2kProtocol(SPUContext* ctx,
  ...
  ctx->prot()->regKernel<Ref2kCastRing>();
}
...
Type Ref2kIo::getShareType(Visibility vis, FieldType ftype,
                           int owner_rank) const {
  if (vis == VIS_PUBLIC) {
    return makeType<Pub2kTy>(ftype);
  } else if (vis == VIS_SECRET) {
    return makeType<Ref2kSecrTy>(ftype);
  }
