...
#include "libspu/mpc/alkaid_boolean_boolean/protocol.h"
...
int main()
{
    ...
    utils::simulate(3,                                                    
        ...
        { 
            ...
            auto x_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(x_s));
            auto y_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(y_s));
            ...
            // and test
            {
              ...
              auto zours_ass = alkaid_boolean::RssAnd2NoComm(&kectx, MyUnwrapValue(x_s), MyUnwrapValue(y_s));
              auto zours_rss = alkaid_boolean::ResharingAss2Rss(&kectx, zours_ass);
              auto zours_p = b2p(sctx.get(), MyWrapValue(zours_rss));
              
              auto xy_rss = alkaid_boolean::MssAnd2NoComm(&kectx, x_mss, y_mss);
            ...
            // A2B test.
            {
              ...
              auto x_b_ours = alkaid_boolean::A2BMultiFanIn(&kectx, MyUnwrapValue(x_a));
            ...
            // 64-fan-in
            {
              ...
              auto input10_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input10));
              auto input11_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input11));
              auto input12_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input12));
              auto input13_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input13));
              auto input30_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input30));
              auto input31_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input31));
              auto input32_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input32));
              auto input33_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input33));
              auto input50_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input50));
              auto input51_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input51));
              auto input52_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input52));
              auto input53_mss = alkaid_boolean::ResharingRss2Mss(&kectx, MyUnwrapValue(input53));

              start = std::chrono::high_resolution_clock::now();
              comm = GetComm.comm;
              latency = GetComm.latency;
              auto result1_ours = alkaid_boolean::MssAnd4NoComm(&kectx, input10_mss, input11_mss, input12_mss, input13_mss);
              auto result1_mss = alkaid_boolean::ResharingAss2Mss(&kectx, result1_ours);
              auto result3_ours = alkaid_boolean::MssAnd4NoComm(&kectx, input30_mss, input31_mss, input32_mss, input33_mss);
              auto result3_mss = alkaid_boolean::ResharingAss2Mss(&kectx, result3_ours);
              auto result5_ours = alkaid_boolean::MssAnd4NoComm(&kectx, input50_mss, input51_mss, input52_mss, input53_mss);
              auto result5_mss = alkaid_boolean::ResharingAss2Mss(&kectx, result5_ours);
              ...
              if (lctx.get()->Rank() == 0) std::cout << "64-fan-in sent: " << GetComm.comm - comm << std::endl;
              if (lctx.get()->Rank() == 0) std::cout << "64-fan-in latency: " << GetComm.latency - latency << std::endl;
              // alkaid_boolean::MssAnd4NoComm(&kectx, x_mss, y_mss, a_mss, b_mss);
