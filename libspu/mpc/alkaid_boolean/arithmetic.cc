...
#include "libspu/mpc/alkaid_boolean/ot.h"
...
NdArrayRef MulAA::proc(KernelEvalContext* ctx, const NdArrayRef& lhs,
  ...
  return DISPATCH_ALL_FIELDS(field, "alkaid_boolean.mulAA", [&]() {
    using el_t = ring2k_t;
    using shr_t = std::array<el_t, 2>;
...
NdArrayRef MulA1B::proc(KernelEvalContext* ctx, const NdArrayRef& lhs,
  ...
  auto parallel_online =
    ...
    if (self_rank == (sender1 + 2) % 3) {
      ...
      comm->sendAsync((sender1 + 1) % 3, c1, "alkaid_boolean-MUL-R1C1");  // 1k
      r1 = {c1.reshape(data1[0].shape()), data1[0]};
    }
    if (self_rank == (sender2 + 2) % 3) {
      // 1 latency overlapping with "alkaid_boolean-MUL-R1C1"
      auto c1 = ot2.recv(ring_cast_boolean(b1));
      comm->sendAsync((sender2 + 1) % 3, c1, "alkaid_boolean-MUL-R2C1");  // 1k
    ...
    if (self_rank == (sender1 + 1) % 3) {
      // 1 latency
      r1.second = comm->recv((sender1 + 2) % 3, a1.eltype(), "alkaid_boolean-MUL-R1C1")
                      .reshape(r1.first.shape());
    }
    if (self_rank == (sender2 + 1) % 3) {
      // 1 latency overlapping with "alkaid_boolean-MUL-R1C1"
      r2.second = comm->recv((sender2 + 2) % 3, a1.eltype(), "alkaid_boolean-MUL-R2C1")
...
NdArrayRef TruncAPr::proc(KernelEvalContext* ctx, const NdArrayRef& in,
  ...
  DISPATCH_ALL_FIELDS(field, "alkaid.truncpr", [&]() {
    using el_t = ring2k_t;
    using shr_t = std::array<el_t, 2>;
