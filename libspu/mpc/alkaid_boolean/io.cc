...
Type AlkaidBooleanIo::getShareType(Visibility vis, int owner_rank) const {
  if (vis == VIS_PUBLIC) {
...
std::vector<NdArrayRef> AlkaidBooleanIo::toShares(const NdArrayRef& raw, Visibility vis,
...
size_t AlkaidBooleanIo::getBitSecretShareSize(size_t numel) const {
  const auto type = makeType<BShrTy>(PT_U8, 1);
  return numel * type.size();
}

std::vector<NdArrayRef> AlkaidBooleanIo::makeBitSecret(const PtBufferView& in) const {
...
NdArrayRef AlkaidBooleanIo::fromShares(const std::vector<NdArrayRef>& shares) const {
...
std::unique_ptr<AlkaidBooleanIo> makeAlkaidBooleanIo(FieldType field, size_t npc) {
  SPU_ENFORCE(npc == 3U, "alkaid_boolean is only for 3pc.");
  registerTypes();
  return std::make_unique<AlkaidBooleanIo>(field, npc);
}
