...
class IoInterface {
 public:
  ...
  virtual std::vector<MemRef> toShares(const MemRef& raw, Visibility vis,
                                       int owner_rank = -1) const = 0;

  virtual Type getShareType(Visibility vis, PtType type,
                            int owner_rank = -1) const = 0;
  ...
  virtual std::vector<MemRef> makeBitSecret(const PtBufferView& raw) const = 0;
  ...
  virtual MemRef fromShares(const std::vector<MemRef>& shares) const = 0;
};
...
class BaseIo : public IoInterface {
 ...
 public:
  explicit BaseIo(size_t field, size_t world_size)
      : field_(field), world_size_(world_size) {}

  std::vector<MemRef> makeBitSecret(const PtBufferView&) const override {
