...
Type Spdz2kIo::getShareType(Visibility vis, FieldType field,
                            int owner_rank) const {
  if (vis == VIS_PUBLIC) {
    return makeType<Pub2kTy>(field);
  } else if (vis == VIS_SECRET) {
    const auto runtime_field = Spdz2kState::getRuntimeField(field);
...
NdArrayRef Spdz2kIo::fromShares(const std::vector<NdArrayRef>& shares) const {
  ...
  if (eltype.isa<Public>()) {
    ...
    auto d_field = Spdz2kState::getDataField(field);

    if (eltype.isa<AShare>()) {
      ring_bitmask_(res, 0, SizeOf(d_field) * 8);
    } else {
    ...
    {
      NdArrayRef x(makeType<Pub2kTy>(d_field), res.shape());

      DISPATCH_ALL_FIELDS(field, "_", [&]() {
        NdArrayView<ring2k_t> _res(res);
        DISPATCH_ALL_FIELDS(d_field, "_", [&]() {
          NdArrayView<ring2k_t> _x(x);
...
std::unique_ptr<Spdz2kIo> makeSpdz2kIo(size_t npc) {
  registerTypes();
  return std::make_unique<Spdz2kIo>(npc);
}
