...
NdArrayRef RandA::proc(KernelEvalContext* ctx, const Shape& shape,
                       FieldType field) const {
  SPU_TRACE_MPC_LEAF(ctx, shape);

  field = Spdz2kState::getRuntimeField(field);
  auto* prg_state = ctx->getState<PrgState>();
  const auto* state = ctx->getState<Spdz2kState>()->getStateImpl(field);
  auto* beaver = state->beaver();
  const size_t k = state->k();
...
NdArrayRef P2A::proc(KernelEvalContext* ctx, const NdArrayRef& in) const {
  ...
  const auto field =
      Spdz2kState::getRuntimeField(in.eltype().as<RingTy>()->field());
  auto* comm = ctx->getState<Communicator>();

  const auto* state = ctx->getState<Spdz2kState>()->getStateImpl(field);
  const auto key = state->key();

  auto res = zero_a_impl(ctx, in.shape(), field);
...
NdArrayRef A2P::proc(KernelEvalContext* ctx, const NdArrayRef& in) const {
  ...
  const auto out_field = Spdz2kState::getDataField(field);
...
NdArrayRef A2V::proc(KernelEvalContext* ctx, const NdArrayRef& in,
  ...
  // generate mask
  auto zero = zero_a_impl(ctx, in.shape(), field);
...
NdArrayRef V2A::proc(KernelEvalContext* ctx, const NdArrayRef& in) const {
  ...
  const auto field = Spdz2kState::getRuntimeField(in_ty->field());
  auto* comm = ctx->getState<Communicator>();
  const auto* state = ctx->getState<Spdz2kState>()->getStateImpl(field);
  auto* beaver = state->beaver();
  const size_t k = state->k();
...
NdArrayRef MatMulAA::proc(KernelEvalContext* ctx, const NdArrayRef& lhs,
  ...
  auto* beaver = state->beaver();
  const auto key = state->key();
  const size_t k_bits = state->k();
  const size_t s_bits = state->s();
