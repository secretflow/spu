...
TEST_P(BooleanTest, NotB) {
  ...
  const FieldType field = std::get<3>(GetParam());
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("not_b"), "not_b", field,
                           kShape, npc, cost));
  });
}

TEST_P(BooleanTest, AddBB) {
  const auto factory = std::get<0>(GetParam());
  const RuntimeConfig& conf = std::get<1>(GetParam());
  const size_t npc = std::get<2>(GetParam());
  const FieldType field = std::get<3>(GetParam());
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    EXPECT_TRUE(verifyCost(obj->getKernel("add_bb"), "add_bb", field, kShape,
                           npc, cost));
  });
}

TEST_P(ArithmeticTest, AddBP) {
  const auto factory = std::get<0>(GetParam());
  const RuntimeConfig& conf = std::get<1>(GetParam());
  const size_t npc = std::get<2>(GetParam());
  const FieldType field = std::get<3>(GetParam());
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    /* THEN */
    EXPECT_VALUE_EQ(re, rp);
    EXPECT_TRUE(verifyCost(obj->getKernel("add_bp"), "add_bp", field, kShape,
                           npc, cost));
  });
}
...
TEST_P(ConversionTest, Bit2A) {
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    EXPECT_TRUE(
        verifyCost(obj->getKernel("bit2a"), "bit2a", field, kShape, npc, cost));
  });
}
...
TEST_P(ConversionTest, A2Bit) {
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    EXPECT_VALUE_EQ(p0, p1);
    EXPECT_TRUE(
        verifyCost(obj->getKernel("a2bit"), "a2bit", field, kShape, npc, cost));
  });
}

TEST_P(BooleanTest, BitLT) {
  const auto factory = std::get<0>(GetParam());
  const RuntimeConfig& conf = std::get<1>(GetParam());
  const size_t npc = std::get<2>(GetParam());
  const FieldType field = std::get<3>(GetParam());
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    /* THEN */
    EXPECT_TRUE(verifyCost(obj->getKernel("bitlt_bb"), "bitlt_bb", field,
                           kShape, npc, cost));
  });
}

TEST_P(BooleanTest, BitLE) {
  const auto factory = std::get<0>(GetParam());
  const RuntimeConfig& conf = std::get<1>(GetParam());
  const size_t npc = std::get<2>(GetParam());
  const FieldType field = std::get<3>(GetParam());
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    /* THEN */
    EXPECT_TRUE(verifyCost(obj->getKernel("bitle_bb"), "bitle_bb", field,
                           kShape, npc, cost));
  });
}
...
TEST_P(BooleanTest, BitIntl) {
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    EXPECT_TRUE(verifyCost(obj->getKernel("bitintl_b"), "bitintl_b", field,
                           kShape, npc, cost));
  });
}
