...
NdArrayRef P2B::proc(KernelEvalContext* ctx, const NdArrayRef& in) const {
  ...
  const auto out_field =
      Spdz2kState::getRuntimeField(in.eltype().as<Pub2kTy>()->field(), true);
  const auto* state = ctx->getState<Spdz2kState>()->getStateImpl(out_field);
