...
#define TEST_ARITHMETIC_BINARY_OP_AA(OP)                                       \
  TEST_P(ArithmeticTest, OP##AA) {                                             \
    ...
    const spu::FieldType field = std::get<3>(GetParam());                      \
                                                                               \
    utils::simulate(                                                           \
        npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {           \
          auto obj = factory(conf, lctx);                                      \
                                                                               \
          /* GIVEN */                                                          \
          auto p0 = rand_p(obj.get(), kShape, field);                          \
          auto p1 = rand_p(obj.get(), kShape, field);                          \
                                                                               \
          /* WHEN */                                                           \
...
TEST_P(ArithmeticTest, MulA1B) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    const size_t K = spu::SizeOf(field) * 8;
    ...
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("mul_a1b"), "mul_a1b", field,
                           kShape, npc, cost));
  });
}
...
TEST_P(ArithmeticTest, MatMulAP) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* GIVEN */
    auto p0 = rand_p(obj.get(), shape_A, field);
    auto p1 = rand_p(obj.get(), shape_B, field);
    auto a0 = p2a(obj.get(), p0);
    ...
    EXPECT_VALUE_EQ(r_aa, r_pp);
    ce::Params params = {
        {"K", SizeOf(field) * 8}, {"N", npc}, {"m", M}, {"n", N}, {"k", K}};
...
TEST_P(ArithmeticTest, NotA) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("not_a"), "not_a", field,
                           kShape, npc, cost));
  });
}
...
TEST_P(ArithmeticTest, TruncA) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    if (!kernel->hasMsbError()) {
      ...
      p0 = arshift_p(obj.get(), p0, SizeOf(field) * 8 - 10);
    }
    ...
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("trunc_a"), "trunc_a", field,
                           kShape, npc, cost));
  });
}
...
TEST_P(ArithmeticTest, P2A) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("p2a"), "p2a", field, kShape,
                           npc, cost));
  });
}
...
TEST_P(ArithmeticTest, A2P) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_VALUE_EQ(p0, p1);
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("a2p"), "a2p", field, kShape,
                           npc, cost));
  });
}
...
TEST_P(BooleanTest, P2B) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_VALUE_EQ(p0, p1);
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("p2b"), "p2b", field, kShape,
                           npc, cost));
  });
}
...
TEST_P(BooleanTest, B2P) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_VALUE_EQ(p0, p1);
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("b2p"), "b2p", field, kShape,
                           npc, cost));
  });
}
...
TEST_P(ConversionTest, A2B) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("a2b"), "a2b", field, kShape,
                           npc, cost));
...
TEST_P(ConversionTest, B2A) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("b2a"), "b2a", field, kShape,
                           npc, cost));
...
TEST_P(ConversionTest, MSB) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("msb_a2b"), "msb_a2b", field,
                           kShape, npc, cost));
    EXPECT_VALUE_EQ(rshift_p(obj.get(), p0, SizeOf(field) * 8 - 1),
                    b2p(obj.get(), b1));
  });
}
