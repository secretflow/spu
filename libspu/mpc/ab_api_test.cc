...
#define EXPECT_VALUE_ALMOST_EQ(X, Y, ERR)       \
  {                                             \
    EXPECT_EQ((X).shape(), (Y).shape());        \
    EXPECT_TRUE(ring_all_equal((X), (Y), ERR)); \
  }
...
bool verifyCost(Kernel* kernel, std::string_view name, size_t field,
                const Shape& shape, size_t npc,
                const Communicator::Stats& cost) {
  ce::Params params = {{"K", field}, {"N", npc}};
    ...
    utils::simulate(                                                           \
        npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {           \
          auto obj = factory(conf, lctx);                                      \
                                                                               \
          /* GIVEN */                                                          \
          auto p0 = rand_p(obj.get(),                                          \
                           GetSemanticType(conf.protocol().field()), kShape);  \
          auto p1 = rand_p(obj.get(),                                          \
                           GetSemanticType(conf.protocol().field()), kShape);  \
                                                                               \
          /* WHEN */                                                           \
          ...
          EXPECT_TRUE(verifyCost(obj->prot()->getKernel(#OP "_aa"), #OP "_aa", \
                                 conf.protocol().field(), kShape, npc, cost)); \
        });                                                                    \
  }
...
TEST_P(ArithmeticTest, SquareA) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* GIVEN */
    auto p0 =
        rand_p(obj.get(), GetSemanticType(conf.protocol().field()), kShape);
    auto a0 = p2a(obj.get(), p0);
...
TEST_P(ArithmeticTest, MulA1B) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    const int64_t K = spu::SizeOf(conf.protocol().field()) * 8;

    /* GIVEN */
    auto p0 = rand_p(obj.get(), GetSemanticType(conf.protocol().field()),
                     conf.protocol().kind() == ProtocolKind::CHEETAH
                         ? Shape({200, 26})
                         : kShape);
    auto p1 = rand_p(obj.get(), GetSemanticType(conf.protocol().field()),
                     conf.protocol().kind() == ProtocolKind::CHEETAH
                         ? Shape({200, 26})
                         : kShape);
...
TEST_P(ArithmeticTest, MatMulAP) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_VALUE_EQ(r_aa, r_pp);
    ce::Params params = {{"K", SizeOf(conf.protocol().field()) * 8},
                         {"N", npc},
                         {"m", M},
                         {"n", N},
                         {"k", K}};
...
TEST_P(ArithmeticTest, TruncA) {
  ...
  // ArrayRef p0_large =
  //     ring_rand_range(conf.protocol().field(), kShape, -(1 << 28), -(1 <<
  //     27));
  // ArrayRef p0_small = ring_rand_range(conf.protocol().field(), kShape, 1,
  // 10000);
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    if (!kernel->hasMsbError()) {
      ...
      p0 = arshift_p(
          obj.get(), p0,
          {static_cast<int64_t>(SizeOf(conf.protocol().field()) * 8 - 10)});
    }
...
TEST_P(ArithmeticTest, P2A) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("p2a"), "p2a",
                           conf.protocol().field(), kShape, npc, cost));
  });
}
...
TEST_P(ArithmeticTest, A2P) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_VALUE_EQ(p0, p1);
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("a2p"), "a2p",
                           conf.protocol().field(), kShape, npc, cost));
  });
}
    ...
    utils::simulate(                                                          \
        npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {          \
          ...
          for (auto bits : kShiftBits) {                                      \
            ...
            /* THEN */                                                        \
            EXPECT_VALUE_EQ(r_b, r_p);                                        \
            EXPECT_TRUE(verifyCost(obj->prot()->getKernel(#OP), #OP,          \
                                   conf.protocol().field(), kShape, npc,      \
                                   cost));                                    \
          }                                                                   \
        });                                                                   \
  }
...
TEST_P(BooleanTest, P2B) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_VALUE_EQ(p0, p1);
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("p2b"), "p2b",
                           conf.protocol().field(), kShape, npc, cost));
  });
}
...
TEST_P(BooleanTest, B2P) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_VALUE_EQ(p0, p1);
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("b2p"), "b2p",
                           conf.protocol().field(), kShape, npc, cost));
  });
}
...
TEST_P(BooleanTest, BitrevB) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    for (size_t i = 0; i < SizeOf(conf.protocol().field()); i++) {
      for (size_t j = i; j < SizeOf(conf.protocol().field()); j++) {
        auto prev = obj->prot()->getState<Communicator>()->getStats();
        ...
        EXPECT_TRUE(verifyCost(obj->prot()->getKernel("bitrev"), "bitrev",
                               conf.protocol().field(), kShape, npc, cost));
      }
    }
  });
...
TEST_P(ConversionTest, A2B) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("a2b"), "a2b",
                           conf.protocol().field(), kShape, npc, cost));
...
TEST_P(ConversionTest, B2A) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_TRUE(verifyCost(obj->prot()->getKernel("b2a"), "b2a",
                           conf.protocol().field(), kShape, npc, cost));
...
TEST_P(ConversionTest, MSB) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    if (conf.protocol().kind() == ProtocolKind::SECURENN) {
      p0 = arshift_p(obj.get(), p0, {1});
    }
    ...
    auto exp = msb_p(obj.get(), p0);
    EXPECT_VALUE_EQ(exp, b2p(obj.get(), b1));
  });
}
...
TEST_P(ConversionTest, EqualAA) {
  ...
  utils::simulate(npc, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    auto r0 = rand_p(obj.get(), GetSemanticType(conf.protocol().field()),
                     conf.protocol().kind() == ProtocolKind::CHEETAH
                         ? Shape({10, 20, 30})
                         : kShape);
    auto r1 = rand_p(obj.get(), GetSemanticType(conf.protocol().field()),
                     conf.protocol().kind() == ProtocolKind::CHEETAH
                         ? Shape({10, 20, 30})
                         : kShape);
    auto r2 = rand_p(obj.get(), GetSemanticType(conf.protocol().field()),
                     conf.protocol().kind() == ProtocolKind::CHEETAH
                         ? Shape({10, 20, 30})
                         : kShape);
    std::memcpy(r2.data(), r0.data(), 16);
    std::vector<MemRef> test_values = {r0, r1, r2};
