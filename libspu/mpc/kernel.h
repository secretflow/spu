...
class RandKernel : public Kernel {
 public:
  ...
  virtual MemRef proc(KernelEvalContext* ctx, SemanticType type,
...
class RandPermKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, const Shape& shape) const = 0;
};

class UnaryKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in) const = 0;
};

class RevealToKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in,
                      size_t rank) const = 0;
};
...
class TruncKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in,
                      size_t bit) const = 0;
...
class BinaryKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& lhs,
                      const MemRef& rhs) const = 0;
};

class MatmulKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& a,
                      const MemRef& b) const = 0;
};
...
class Conv2DKernel : public Kernel {
 public:
  ...
  // memref: NxHxWxC
  // filter: hxwxCxO
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& memref,
...
class BitrevKernel : public Kernel {
 public:
  ...
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in, size_t start,
                      size_t end) const = 0;
};
...
class TruncAKernel : public Kernel {
 public:
  ...
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in, size_t bits,
...
class OramReadKernel : public Kernel {
  ...
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& onehot,
                      const MemRef& db, int64_t offset) const = 0;
};
...
class MergeKeysKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, absl::Span<MemRef const> inputs,
                      bool is_ascending) const = 0;
};
...
class ExtractSliceKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in,
                      const Index& start, const Shape& sizes,
                      const Strides& strides) const = 0;
};

class InsertSliceKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;

  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in,
                      const MemRef& update, const Index& offsets,
                      const Strides& strides, bool prefer_in_place) const = 0;
};
...
class PadKernel : public Kernel {
 public:
  ...
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in,
                      const MemRef& padding_value,
                      const Sizes& edge_padding_low,
                      const Sizes& edge_padding_high) const = 0;
};
...
class ConcateKernel : public Kernel {
 public:
  ...
  virtual MemRef proc(KernelEvalContext* ctx, const std::vector<MemRef>& values,
                      int64_t axis) const = 0;
};

class RingCastKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;
  virtual MemRef proc(KernelEvalContext* ctx, const MemRef& in,
                      SemanticType to_type) const = 0;
};

class DecomposeKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;

  virtual std::vector<MemRef> proc(KernelEvalContext* ctx,
                                   const MemRef& in) const = 0;
};

class ComposeKernel : public Kernel {
 public:
  void evaluate(KernelEvalContext* ctx) const override;

  virtual MemRef proc(KernelEvalContext* ctx,
                      const std::vector<MemRef>& in) const = 0;
};
