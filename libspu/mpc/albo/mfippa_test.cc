...
#include "libspu/mpc/albo_boolean/protocol.h"
...
spu::RuntimeConfig makeConfig(spu::FieldType field) {
  ...
  conf.set_protocol(spu::ProtocolKind::ALBO);
...
int main()
{
    ...
    utils::simulate(3,                                                    
        ...
        { 
            ...
            auto sctx = makeAlkaidBooleanProtocol(config, lctx); 
            ...
            auto x_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(x_s));
            auto y_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(y_s));
            ...
            // and test
            {
              ...
              auto zours_ass = albo::RssAnd2NoComm(&kectx, MyUnwrapValue(x_s), MyUnwrapValue(y_s));
              auto zours_rss = albo::ResharingAss2Rss(&kectx, zours_ass);
              auto zours_p = b2p(sctx.get(), MyWrapValue(zours_rss));
              
              auto xy_rss = albo::MssAnd2NoComm(&kectx, x_mss, y_mss);
            ...
            //   auto a_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(a_s));
            //   auto b_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(b_s));
            //   auto res_ass = albo::MssAnd4NoComm(&kectx, x_mss, y_mss, a_mss, b_mss);
            //   auto res_rss = albo::ResharingAss2Rss(&kectx, res_ass);
            //   auto res_ours_p = b2p(sctx.get(), MyWrapValue(res_rss));
            //   // auto xy_rss = albo::MssAnd2NoComm(&kectx, x_mss, y_mss);
            //   // auto xy_rss_p = OpenRef(xy_rss);
            //   // auto a_rss = albo::ResharingMss2Rss(&kectx, a_mss);
            //   // auto a_rss_p = OpenRef(a_rss);
            //   // auto xya_ass = albo::RssAnd2NoComm(&kectx, xy_rss, a_rss);
            //   // auto xya_rss = albo::ResharingAss2Rss(&kectx, xya_ass);
            ...
            //   auto z_p = b2p(sctx.get(), z_s);
            //   auto zours_ass = albo::RssAnd2NoComm(&kectx, MyUnwrapValue(x_s), MyUnwrapValue(y_s));
            //   auto zours_rss_2 = albo::ResharingAss2Rss(&kectx, zours_ass);
            //   auto zours_mss_2 = albo::ResharingRss2Mss(&kectx, zours_rss_2);
            //   auto zours_rss_3 = albo::ResharingMss2Rss(&kectx, zours_mss_2);
            //   auto zours_ass_3 = albo::ResharingRss2Ass(&kectx, zours_rss_3);
            //   auto zours_mss_3 = albo::ResharingAss2Mss(&kectx, zours_ass_3);
            //   auto zours_rss_4 = albo::ResharingMss2Rss(&kectx, zours_mss_3);
            ...
            // msb test
            {
              ...
              auto msbx_ours_s = albo::MsbA2BMultiFanIn(&kectx, MyUnwrapValue(x_as));
            ...
            // A2B test.
            {
              ...
              auto x_b_ours = albo::A2BMultiFanIn(&kectx, MyUnwrapValue(x_a));
            ...
            // 64-fan-in
            {
              ...
              auto input10_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input10));
              auto input11_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input11));
              auto input12_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input12));
              auto input13_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input13));
              auto input30_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input30));
              auto input31_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input31));
              auto input32_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input32));
              auto input33_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input33));
              auto input50_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input50));
              auto input51_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input51));
              auto input52_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input52));
              auto input53_mss = albo::ResharingRss2Mss(&kectx, MyUnwrapValue(input53));

              start = std::chrono::high_resolution_clock::now();
              comm = GetComm.comm;
              latency = GetComm.latency;
              auto result1_ours = albo::MssAnd4NoComm(&kectx, input10_mss, input11_mss, input12_mss, input13_mss);
              auto result1_mss = albo::ResharingAss2Mss(&kectx, result1_ours);
              auto result3_ours = albo::MssAnd4NoComm(&kectx, input30_mss, input31_mss, input32_mss, input33_mss);
              auto result3_mss = albo::ResharingAss2Mss(&kectx, result3_ours);
              auto result5_ours = albo::MssAnd4NoComm(&kectx, input50_mss, input51_mss, input52_mss, input53_mss);
              auto result5_mss = albo::ResharingAss2Mss(&kectx, result5_ours);
              ...
              if (lctx.get()->Rank() == 0) std::cout << "64-fan-in sent: " << GetComm.comm - comm << std::endl;
              if (lctx.get()->Rank() == 0) std::cout << "64-fan-in latency: " << GetComm.latency - latency << std::endl;
              // albo::MssAnd4NoComm(&kectx, x_mss, y_mss, a_mss, b_mss);
