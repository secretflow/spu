...
#include "libspu/mpc/albo/arithmetic_gpu_ext.h"
#endif
...
NdArrayRef RandA::proc(KernelEvalContext* ctx, const Shape& shape) const {
  ...
  if (ctx->getState<Communicator>()->getRank() == 0) std::cout << "RandA::proc" << std::endl;
...
NdArrayRef A2P::proc(KernelEvalContext* ctx, const NdArrayRef& in) const {
  ...
  if (comm->getRank() == 0) std::cout << "A2P::proc" << std::endl;
  ...
  return DISPATCH_ALL_FIELDS(field, "_", [&]() {
    ...
    pforeach(0, numel, [&](int64_t idx) { x2[idx] = _in[idx][2]; });
...
NdArrayRef P2A::proc(KernelEvalContext* ctx, const NdArrayRef& in) const {
  ...
  if (ctx->getState<Communicator>()->getRank() == 0) std::cout << "P2A::proc" << std::endl;
...
#ifdef ENABLE_MASK_DURING_ALBO_P2A
...
NdArrayRef V2A::proc(KernelEvalContext* ctx, const NdArrayRef& in) const {
  ...
  if (comm->getRank() == 0) std::cout << "V2A::proc" << std::endl;
...
NdArrayRef MulAA::proc(KernelEvalContext* ctx, const NdArrayRef& lhs,
  ...
  SPU_ENFORCE(lhs.eltype() == makeType<AShrTy>(field));

  return DISPATCH_ALL_FIELDS(field, "albo.mulAA", [&]() {
    using el_t = ring2k_t;
...
NdArrayRef MulA1B::proc(KernelEvalContext* ctx, const NdArrayRef& lhs,
  ...
  auto parallel_online =
    ...
    if (self_rank == (sender1 + 2) % 3) {
      ...
      comm->sendAsync((sender1 + 1) % 3, c1, "albo-MUL-R1C1");  // 1k
      r1 = {c1.reshape(data1[0].shape()), data1[0]};
    }
    if (self_rank == (sender2 + 2) % 3) {
      // 1 latency overlapping with "albo-MUL-R1C1"
      auto c1 = ot2.recv(ring_cast_boolean(b1));
      comm->sendAsync((sender2 + 1) % 3, c1, "albo-MUL-R2C1");  // 1k
    ...
    if (self_rank == (sender1 + 1) % 3) {
      // 1 latency
      r1.second = comm->recv((sender1 + 2) % 3, a1.eltype(), "albo-MUL-R1C1")
                      .reshape(r1.first.shape());
    }
    if (self_rank == (sender2 + 1) % 3) {
      // 1 latency overlapping with "albo-MUL-R1C1"
      r2.second = comm->recv((sender2 + 2) % 3, a1.eltype(), "albo-MUL-R2C1")
  ...
  // TODO: just to pass compile. Fix it.
  // return makeAShare(ret.first, ret.second, field);
  return makeAShare(ret.first, ret.first, ret.second, field);
}
...
NdArrayRef TruncA::proc(KernelEvalContext* ctx, const NdArrayRef& in,
  ...
  switch (comm->getRank()) {
    ...
    case 2: {
      ...
      return makeAShare(r_future.get().first, r_future.get().first, z2, field);
    }
...
NdArrayRef TruncAPr::proc(KernelEvalContext* ctx, const NdArrayRef& in,
  ...
  DISPATCH_ALL_FIELDS(field, "albo.truncpr", [&]() {
    using el_t = ring2k_t;
    using shr_t = std::array<el_t, 2>;
