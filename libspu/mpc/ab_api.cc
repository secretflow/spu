...
static inline FieldType maxBitsToField(const Value& in) {
  auto mb = in.storage_type().as<BShare>()->max_bits();
  switch (mb) {
    case 32:
      return FM32;
    case 64:
  ...
  SPU_THROW("Should not reach, max_bits = {}", mb);
  return FT_INVALID;
}
...
static Value hack_make_p(SPUContext* ctx, uint128_t init, const Shape& shape,
                         FieldType ftype) {
  return dynDispatch(ctx, "make_p", init, shape, ftype);
}
...
Value ppa_sklansky(SPUContext* ctx, Value const& lhs, Value const& rhs,
  ...
  for (int idx = 0; idx < Log2Ceil(nbits); ++idx) {
    ...
    const auto s_mask =
        hack_make_p(ctx, kSelMask[idx], lhs.shape(), maxBitsToField(lhs));
