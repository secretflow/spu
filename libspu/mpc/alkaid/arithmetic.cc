...
#include "libspu/mpc/alkaid/ot.h"
...
#include "libspu/mpc/alkaid/arithmetic_gpu_ext.h"
#endif
...
NdArrayRef MulAA::proc(KernelEvalContext* ctx, const NdArrayRef& lhs,
  ...
  return DISPATCH_ALL_FIELDS(field, "alkaid.mulAA", [&]() {
    using el_t = ring2k_t;
    using shr_t = std::array<el_t, 2>;
...
NdArrayRef MulA1B::proc(KernelEvalContext* ctx, const NdArrayRef& lhs,
  ...
  auto parallel_online =
    ...
    if (self_rank == (sender1 + 2) % 3) {
      ...
      comm->sendAsync((sender1 + 1) % 3, c1, "ALKAID-MUL-R1C1");  // 1k
      r1 = {c1.reshape(data1[0].shape()), data1[0]};
    }
    if (self_rank == (sender2 + 2) % 3) {
      // 1 latency overlapping with "ALKAID-MUL-R1C1"
      auto c1 = ot2.recv(ring_cast_boolean(b1));
      comm->sendAsync((sender2 + 1) % 3, c1, "ALKAID-MUL-R2C1");  // 1k
    ...
    if (self_rank == (sender1 + 1) % 3) {
      // 1 latency
      r1.second = comm->recv((sender1 + 2) % 3, a1.eltype(), "ALKAID-MUL-R1C1")
                      .reshape(r1.first.shape());
    }
    if (self_rank == (sender2 + 1) % 3) {
      // 1 latency overlapping with "ALKAID-MUL-R1C1"
      r2.second = comm->recv((sender2 + 2) % 3, a1.eltype(), "ALKAID-MUL-R2C1")
