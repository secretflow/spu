...
void RandKernel::evaluate(KernelEvalContext* ctx) const {
  auto res = proc(ctx, ctx->getParam<Shape>(0), ctx->getParam<FieldType>(1));
...
void CastRingKernel::evaluate(KernelEvalContext* ctx) const {
  const auto& val = ctx->getParam<Value>(0);
  const auto& ftype = ctx->getParam<FieldType>(1);

  auto res = proc(ctx, UnwrapValue(val), ftype);
