...
void Factory::RegisterProtocol(
  ...
  switch (ctx->config().protocol().kind()) {
    ...
    default: {
      SPU_THROW("Invalid protocol kind {}", ctx->config().protocol().kind());
    }
  }
}
...
std::unique_ptr<IoInterface> Factory::CreateIO(const RuntimeConfig& conf,
                                               size_t npc) {
  switch (conf.protocol().kind()) {
    case ProtocolKind::REF2K: {
      return makeRef2kIo(conf.protocol().field(), npc);
    }
    case ProtocolKind::SEMI2K: {
      return semi2k::makeSemi2kIo(conf.protocol().field(), npc);
    }
    case ProtocolKind::ABY3: {
      return aby3::makeAby3Io(conf.protocol().field(), npc);
    }
    case ProtocolKind::CHEETAH: {
      return cheetah::makeCheetahIo(conf.protocol().field(), npc);
    }
    case ProtocolKind::SECURENN: {
      return securenn::makeSecurennIo(conf.protocol().field(), npc);
    }
    default: {
      SPU_THROW("Invalid protocol kind {}", conf.protocol().kind());
    }
  }
  return nullptr;
}
