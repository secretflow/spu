...
Type Semi2kIo::getShareType(Visibility vis, PtType type, int owner_rank) const {
  if (vis == VIS_PUBLIC) {
    ...
    if (owner_rank >= 0 && owner_rank < static_cast<int>(world_size_)) {
      ...
      return makeType<semi2k::ArithShareTy>(GetEncodedType(type, field_),
                                            field_);
    }
  }
...
std::vector<MemRef> Semi2kIo::toShares(const MemRef& raw, Visibility vis,
  ...
  if (vis == VIS_PUBLIC) {
    ...
    if (owner_rank >= 0 && owner_rank < static_cast<int>(world_size_)) {
      ...
      const auto ty =
          makeType<semi2k::ArithShareTy>(raw.eltype().semantic_type(), field_);
...
MemRef Semi2kIo::fromShares(const std::vector<MemRef>& shares) const {
...
std::unique_ptr<Semi2kIo> makeSemi2kIo(size_t field, size_t npc) {
  registerTypes();
