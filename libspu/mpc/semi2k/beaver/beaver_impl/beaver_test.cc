    ...
    testing::Combine(
        testing::Values(std::make_pair(
                        ...
                        std::make_pair(
                            ...
                            "BeaverTtp")),
        testing::Values(4, 3, 2), testing::Values(32, 64, 128),
...
namespace {
template <class T>
std::vector<MemRef> open_buffer(std::vector<T>& in_buffers, size_t k_field,
                                const std::vector<Shape>& shapes,
                                size_t k_world_size, bool add_open) {
  std::vector<MemRef> ret;

  auto reduce = [&](MemRef& r, yacl::Buffer& b) {
    if (b.size() == 0) {
      return;
    }
    EXPECT_EQ(b.size(), r.shape().numel() * SizeOf(k_field));
    MemRef a(std::make_shared<yacl::Buffer>(std::move(b)), ret[0].eltype(),
             r.shape());
    if (add_open) {
      ring_add_(r, a);
    } else {
  ...
  if constexpr (std::is_same_v<T, Beaver::Triple>) {
    ...
    for (size_t i = 0; i < shapes.size(); i++) {
      MemRef zeros(makeType<RingTy>(SE_INVALID, k_field), shapes[i]);
      ring_zeros(zeros);
      ret[i] = std::move(zeros);
    }
...
TEST_P(BeaverTest, Mul_large) {
  ...
  const size_t kField = std::get<2>(GetParam());
  ...
  MemRef y_cache;
  {
    ...
    DISPATCH_ALL_STORAGE_TYPES(open[0].eltype().storage_type(), [&]() {
      using ring2k_t = ScalarT;
      MemRefView<ring2k_t> _a(open[0]);
      MemRefView<ring2k_t> _b(open[1]);
      MemRefView<ring2k_t> _c(open[2]);
  ...
  }
  {
    ...
    DISPATCH_ALL_STORAGE_TYPES(open[0].eltype().storage_type(), [&]() {
      using ring2k_t = ScalarT;
      MemRefView<ring2k_t> _a(open[0]);
      MemRefView<ring2k_t> _cache_a(x_cache);
      MemRefView<ring2k_t> _b(open[1]);
      MemRefView<ring2k_t> _c(open[2]);
      for (auto idx = 0; idx < _a.numel(); idx++) {
  ...
  }
  {
    ...
    DISPATCH_ALL_STORAGE_TYPES(open[0].eltype().storage_type(), [&]() {
      ...
      MemRefView<ring2k_t> _a(open[0]);
      MemRefView<ring2k_t> _b(open[1]);
      MemRefView<ring2k_t> _cache_b(y_cache);
      MemRefView<ring2k_t> _c(open[2]);
      for (auto idx = 0; idx < _a.numel(); idx++) {
...
TEST_P(BeaverTest, Dot) {
...

  {
    ...
    DISPATCH_ALL_STORAGE_TYPES(res.eltype().storage_type(), [&]() {
      using ring2k_t = ScalarT;
      MemRefView<ring2k_t> _r(res);
      MemRefView<ring2k_t> _c(open[2]);
  ...
  }
  {
    ...
    DISPATCH_ALL_STORAGE_TYPES(open[1].eltype().storage_type(), [&]() {
      using ring2k_t = ScalarT;
      MemRefView<ring2k_t> _b(open[1]);
      MemRefView<ring2k_t> _cache_b(y_cache);
      MemRefView<ring2k_t> _r(res);
  ...
  }
  {
    ...
    DISPATCH_ALL_STORAGE_TYPES(open[0].eltype().storage_type(), [&]() {
      ...
      MemRefView<ring2k_t> _a(transpose_a);
      MemRefView<ring2k_t> _cache_a(y_cache);
      auto transpose_b = open[1].transpose();
      MemRefView<ring2k_t> _b(transpose_b);
      MemRefView<ring2k_t> _cache_b(x_cache);
      auto transpose_r = res.transpose();
      MemRefView<ring2k_t> _r(transpose_r);
      MemRefView<ring2k_t> _c(open[2]);
      for (auto idx = 0; idx < res.numel(); idx++) {
        EXPECT_EQ(_cache_a[idx], _a[idx]);
        EXPECT_EQ(_cache_b[idx], _b[idx]);
...
TEST_P(BeaverTest, TruncPr) {
  ...
  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(kField), [&]() {
    using T = ScalarT;
...
TEST_P(BeaverTest, Randbit) {
  ...
  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(kField), [&]() {
    auto x = xt_adapt<ScalarT>(open[0]);
...
TEST_P(BeaverTest, Eqz) {
  ...
  EXPECT_EQ(pairs.size(), kWorldSize);
  MemRef sum_a(makeType<RingTy>(SE_INVALID, kField), {kNumel});
  MemRef sum_b(makeType<RingTy>(SE_INVALID, kField), {kNumel});
  ring_zeros(sum_a);
  ring_zeros(sum_b);

  for (Rank r = 0; r < kWorldSize; r++) {
    auto [a_buf, b_buf] = pairs[r];
    MemRef a(std::make_shared<yacl::Buffer>(std::move(a_buf)), sum_a.eltype(),
             sum_a.shape());
    MemRef b(std::make_shared<yacl::Buffer>(std::move(b_buf)), sum_b.eltype(),
             sum_b.shape());
