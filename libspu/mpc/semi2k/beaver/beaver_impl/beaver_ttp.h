...
class BeaverTtp final : public Beaver {
 ...
 public:
  ...
  Triple Mul(size_t field, int64_t size, ReplayDesc* x_desc = nullptr,
             ReplayDesc* y_desc = nullptr) override;

  Triple And(int64_t size) override;

  Triple Dot(size_t field, int64_t m, int64_t n, int64_t k,
             ReplayDesc* x_desc = nullptr,
             ReplayDesc* y_desc = nullptr) override;

  Pair Trunc(size_t field, int64_t size, size_t bits) override;

  Triple TruncPr(size_t field, int64_t size, size_t bits) override;

  Array RandBit(size_t field, int64_t size) override;

  Pair PermPair(size_t field, int64_t size, size_t perm_rank,
                absl::Span<const int64_t> perm_vec) override;

  std::unique_ptr<Beaver> Spawn() override;

  Pair Eqz(size_t field, int64_t size) override;
};
