...
void FillReplayDesc(Beaver::ReplayDesc* desc, size_t field, int64_t size,
...
template <class AdjustRequest>
std::vector<MemRef> RpcCall(
    brpc::Channel& channel, AdjustRequest req, size_t ret_field,
  ...
  for (const auto& buf : reader.GetBufVecRef()) {
    ...
    MemRef array(makeType<RingTy>(SE_INVALID, ret_field), {size});
...
BeaverTtp::Triple BeaverTtp::Mul(size_t field, int64_t size, ReplayDesc* x_desc,
                                 ReplayDesc* y_desc) {
  ...
  auto if_replay = [&](const ReplayDesc* replay_desc, size_t idx) {
    if (replay_desc == nullptr || replay_desc->status != Beaver::Replay) {
      MemRef ret(makeType<RingTy>(SE_INVALID, field), shape);
      prgCreateArray(ret, seed_, &counter_, &descs[idx]);
      return ret;
    } else {
      ...
      MemRef ret(makeType<RingTy>(SE_INVALID, field), shape);
      prgCreateArray(ret, replay_desc->seed, &tmp_counter, &descs[idx]);
      return ret;
    }
  };
  ...
  MemRef c(makeType<RingTy>(SE_INVALID, field), shape);
  prgCreateArray(c, seed_, &counter_, &descs[2]);
...
BeaverTtp::Triple BeaverTtp::Dot(size_t field, int64_t m, int64_t n, int64_t k,
  ...
  auto if_replay = [&](const ReplayDesc* replay_desc, size_t idx) {
    if (replay_desc == nullptr) {
      MemRef ret(makeType<RingTy>(SE_INVALID, field), shapes[idx]);
      prgCreateArray(ret, seed_, &counter_, &descs[idx]);
      return ret;
    } else {
      SPU_ENFORCE(replay_desc->field == field);
      ...
      PrgCounter tmp_counter = replay_desc->prg_counter;
      MemRef ret(makeType<RingTy>(SE_INVALID, field), shapes[idx]);
      prgCreateArray(ret, replay_desc->seed, &tmp_counter, nullptr);
      if (replay_desc->status == Beaver::TransposeReplay) {
  ...
  auto b = if_replay(y_desc, 1);
  MemRef c(makeType<RingTy>(SE_INVALID, field), {m, n});
  prgCreateArray(c, seed_, &counter_, &descs[2]);
...
BeaverTtp::Triple BeaverTtp::And(int64_t size) {
  ...
  MemRef a(makeType<RingTy>(SE_INVALID, field), shape);
  MemRef b(makeType<RingTy>(SE_INVALID, field), shape);
  MemRef c(makeType<RingTy>(SE_INVALID, field), shape);

  prgCreateArray(a, seed_, &counter_, descs.data());
  prgCreateArray(b, seed_, &counter_, &descs[1]);
  prgCreateArray(c, seed_, &counter_, &descs[2]);
  ...
  if (lctx_->Rank() == options_.adjust_rank) {
    ...
    auto adjusts = RpcCall(channel_, req, field);
    SPU_ENFORCE_EQ(adjusts.size(), 1U);
    ring_xor_(c, adjusts[0].reshape(shape));
  }
...
BeaverTtp::Pair BeaverTtp::Trunc(size_t field, int64_t size, size_t bits) {
...
BeaverTtp::Triple BeaverTtp::TruncPr(size_t field, int64_t size, size_t bits) {
  std::vector<PrgArrayDesc> descs(3);
  std::vector<absl::Span<const PrgSeedBuff>> descs_seed(1, encrypted_seeds_);
  Shape shape({size, 1});

  MemRef r(makeType<RingTy>(SE_INVALID, field), shape);
  MemRef rc(makeType<RingTy>(SE_INVALID, field), shape);
  MemRef rb(makeType<RingTy>(SE_INVALID, field), shape);

  prgCreateArray(r, seed_, &counter_, descs.data());
  prgCreateArray(rc, seed_, &counter_, &descs[1]);
  prgCreateArray(rb, seed_, &counter_, &descs[2]);
  ...
  if (lctx_->Rank() == options_.adjust_rank) {
    ...
    ring_add_(rc, adjusts[0].reshape(shape));
    ring_add_(rb, adjusts[1].reshape(shape));
  }
...
BeaverTtp::Array BeaverTtp::RandBit(size_t field, int64_t size) {
...
BeaverTtp::Pair BeaverTtp::PermPair(size_t field, int64_t size,
                                    size_t perm_rank,
...
BeaverTtp::Pair BeaverTtp::Eqz(size_t field, int64_t size) {
  std::vector<PrgArrayDesc> descs(2);
  std::vector<absl::Span<const PrgSeedBuff>> descs_seed(1, encrypted_seeds_);
  Shape shape({size, 1});
