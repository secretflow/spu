...
BeaverTfpUnsafe::Triple BeaverTfpUnsafe::Mul(size_t field, int64_t size,
                                             ReplayDesc* x_desc,
                                             ReplayDesc* y_desc) {
  ...
  auto if_replay = [&](const ReplayDesc* replay_desc, size_t idx) {
    if (replay_desc == nullptr || replay_desc->status != Beaver::Replay) {
      ...
      prgCreateArray(ret, seed_, &counter_, &ops[idx].desc);
      return ret;
    } else {
  ...
  prgCreateArray(c, seed_, &counter_, &ops[2].desc);
...
BeaverTfpUnsafe::Triple BeaverTfpUnsafe::Dot(size_t field, int64_t m, int64_t n,
...
BeaverTfpUnsafe::Triple BeaverTfpUnsafe::And(int64_t size) {
  ...
  prgCreateArray(a, seed_, &counter_, &ops[0].desc);
  prgCreateArray(b, seed_, &counter_, &ops[1].desc);
  prgCreateArray(c, seed_, &counter_, &ops[2].desc);
...
BeaverTfpUnsafe::Pair BeaverTfpUnsafe::Trunc(size_t field, int64_t size,
                                             size_t bits) {
...
BeaverTfpUnsafe::Triple BeaverTfpUnsafe::TruncPr(size_t field, int64_t size,
                                                 size_t bits) {
  std::vector<TrustedParty::Operand> ops(3);
  Shape shape({size, 1});
  ...
  prgCreateArray(r, seed_, &counter_, &ops[0].desc);
  prgCreateArray(rc, seed_, &counter_, &ops[1].desc);
  prgCreateArray(rb, seed_, &counter_, &ops[2].desc);
...
BeaverTfpUnsafe::Array BeaverTfpUnsafe::RandBit(size_t field, int64_t size) {
...
BeaverTfpUnsafe::Pair BeaverTfpUnsafe::PermPair(
    size_t field, int64_t size, size_t perm_rank,
...
BeaverTfpUnsafe::Pair BeaverTfpUnsafe::Eqz(size_t field, int64_t size) {
  std::vector<TrustedParty::Operand> ops(2);
  Shape shape({size, 1});
