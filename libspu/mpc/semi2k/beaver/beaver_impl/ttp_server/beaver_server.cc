...
BuildOperand(const AdjustRequest& req, uint32_t field_size,
  ...
  for (int64_t i = 0; i < req.prg_inputs().size(); i++) {
    ...
    ops.push_back(
        TrustedParty::Operand{{shape, field, prg_count}, seeds.back()});
  }
...
std::vector<yacl::Buffer> StripNdArray(std::vector<MemRef>& nds,
                                       size_t pad_length) {
  std::vector<yacl::Buffer> ret;
  ret.reserve(nds.size());

  auto if_pad = [&](MemRef& nd) {
...
template <class AdjustRequest>
std::vector<MemRef> AdjustImpl(const AdjustRequest& req,
