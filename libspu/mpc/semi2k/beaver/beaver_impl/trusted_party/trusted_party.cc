...
std::vector<MemRef> reconstruct(RecOp op,
                                absl::Span<TrustedParty::Operand> ops) {
  std::vector<MemRef> rs(ops.size());
...
MemRef TrustedParty::adjustMul(absl::Span<Operand> ops) {
...
MemRef TrustedParty::adjustSquare(absl::Span<Operand> ops) {
...
MemRef TrustedParty::adjustDot(absl::Span<Operand> ops) {
  SPU_ENFORCE_EQ(ops.size(), 3U);
...
MemRef TrustedParty::adjustAnd(absl::Span<Operand> ops) {
  SPU_ENFORCE_EQ(ops.size(), 3U);
  checkOperands(ops);
...
MemRef TrustedParty::adjustTrunc(absl::Span<Operand> ops, size_t bits) {
  SPU_ENFORCE_EQ(ops.size(), 2U);
  checkOperands(ops);
...
std::pair<MemRef, MemRef> TrustedParty::adjustTruncPr(absl::Span<Operand> ops,
                                                      size_t bits) {
...
MemRef TrustedParty::adjustRandBit(absl::Span<Operand> ops) {
  SPU_ENFORCE_EQ(ops.size(), 1U);
  auto rs = reconstruct(RecOp::ADD, ops);

  // adjust = bitrev - rs[0];
  const auto field = ops[0].desc.field;
  MemRef rbit(makeType<RingTy>(SE_INVALID, field), ops[0].desc.shape);
  ring_randbit(rbit);
  return ring_sub(rbit, rs[0]);
}

MemRef TrustedParty::adjustEqz(absl::Span<Operand> ops) {
  SPU_ENFORCE_EQ(ops.size(), 2U);
  checkOperands(ops);
...
MemRef TrustedParty::adjustPerm(absl::Span<Operand> ops,
