...
class BeaverCache {
 public:
  ...
  Cache GetCache(const MemRef&, bool allow_transpose = true) const;

  void SetCache(const MemRef&, const Beaver::ReplayDesc&, const MemRef&);

  void DisableCache(const MemRef&);
 ...
 private:
  ...
  MemRef ReadCache(const CacheMeta&, const MemRef&) const;
  void WriteCache(const MemRef&, const MemRef&, CacheMeta&);
  ...
  // for all MemRef share same buffer (cache enabled MemRef and slice of
  // this MemRef) use MemRef(data ptr with offset + shape + strides) to
  // track the specific cache.
  using BufferCacheMeta = std::unordered_map<MemRef, CacheMeta>;
  // use MemRef's under-layer buffer data ptr as first map's key to mark if
  // we need keep cache for a MemRef.
