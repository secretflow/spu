...
namespace {
void Check(const MemRef& x, const Beaver::ReplayDesc& replay) {
  SPU_ENFORCE(x.eltype().as<BaseRingType>()->valid_bits() ==
              static_cast<int64_t>(replay.field));
  SPU_ENFORCE(x.shape().numel() == replay.size);
}

std::string KeyPrefix(const MemRef& x) {
  // data_shape_strides
...
void BeaverCache::EnableCache(const MemRef& x) {
...
BeaverCache::Cache BeaverCache::GetCache(const MemRef& x,
...
void BeaverCache::SetCache(const MemRef& x, const Beaver::ReplayDesc& replay,
                           const MemRef& open_cache) {
  LazyInitCacheDB();
  std::unique_lock lock(mutex_);
...
void BeaverCache::DisableCache(const MemRef& x) {
  std::unique_lock lock(mutex_);
...
MemRef BeaverCache::ReadCache(const CacheMeta& meta, const MemRef& x) const {
  ...
  return MemRef(std::make_shared<yacl::Buffer>(std::move(ret_buf)), x.eltype(),
                x.shape());
}

void BeaverCache::WriteCache(const MemRef& x, const MemRef& open_cache,
                             CacheMeta& meta) {
