...
class CommTest : public ::testing::TestWithParam<std::tuple<size_t, size_t>> {};

TEST_P(CommTest, AllReduce) {
  const Rank kWorldSize = std::get<0>(GetParam());
  const size_t kField = std::get<1>(GetParam());
  const int64_t kNumel = 1000;
  Type eltype = makeType<RingTy>(SE_I64, kField);

  std::vector<MemRef> xs(kWorldSize);
  MemRef sum_x(eltype, {kNumel});
  ring_zeros(sum_x);
  MemRef xor_x(eltype, {kNumel});
  ring_zeros(xor_x);
  for (size_t idx = 0; idx < kWorldSize; idx++) {
    xs[idx] = MemRef(eltype, {kNumel});
    ring_rand(xs[idx]);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Combine(testing::Values(4, 3, 2), testing::Values(32, 64, 128)),
