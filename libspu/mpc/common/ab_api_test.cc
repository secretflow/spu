...
TEST_P(ArithmeticTest, MatMulAP) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    ce::Params params = {{"K", SizeOf(conf.field()) * 8},
                         ...
                         {"k", K}};
    EXPECT_TRUE(
        verifyCost(obj->getKernel("mmul_ap"), "mmul_ap", params, cost, 1));
  });
}
...
TEST_P(ArithmeticTest, MatMulAA) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    ce::Params params = {{"K", SizeOf(conf.field()) * 8},
                         ...
                         {"k", K}};
    EXPECT_TRUE(
        verifyCost(obj->getKernel("mmul_aa"), "mmul_aa", params, cost, 1));
  });
}
...
TEST_P(ConversionTest, MSB) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* THEN */
    EXPECT_TRUE(verifyCost(obj->getKernel("msb_a2b"), "msb_a2b", conf.field(),
                           kNumel, npc, cost));
