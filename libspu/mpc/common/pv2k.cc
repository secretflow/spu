...
class P2V : public RevealToKernel {
 public:
  ...
  NdArrayRef proc(KernelEvalContext* ctx, const NdArrayRef& in,
    ...
    auto* comm = ctx->getState<Communicator>();
    auto field = in.eltype().as<Pub2kTy>()->field();
...
class V2P : public UnaryKernel {
 public:
  ...
  NdArrayRef proc(KernelEvalContext* ctx, const NdArrayRef& in) const override {
    ...
    const auto field = in_type->field();
...
class MakeP : public Kernel {
 public:
  ...
  static Value proc(KernelEvalContext* ctx, uint128_t init, const Shape& shape,
                    FieldType ftype) {
    const auto eltype = makeType<Pub2kTy>(ftype);
    ...
    DISPATCH_ALL_FIELDS(ftype, "pub2k.make_p", [&]() {
...
class CastRingP : public CastRingKernel {
 public:
  static constexpr char kBindName[] = "cast_ring_p";

  ce::CExpr latency() const override { return ce::Const(0); }

  ce::CExpr comm() const override { return ce::Const(0); }

  NdArrayRef proc(KernelEvalContext* ctx, const NdArrayRef& val,
                  const FieldType& ftype) const override {
    auto new_type = makeType<Pub2kTy>(ftype);
    return cast_ring(val, new_type);
  }
};

class CastRingV : public CastRingKernel {
 public:
  static constexpr char kBindName[] = "cast_ring_v";
  ...
  NdArrayRef proc(KernelEvalContext* ctx, const NdArrayRef& val,
                  const FieldType& ftype) const override {
    if (isOwner(ctx, val.eltype())) {
      auto new_type =
          makeType<Priv2kTy>(ftype, val.eltype().as<Priv2kTy>()->owner());
      return cast_ring(val, new_type);
    } else {
      return val;
    }
...
void regPV2kKernels(Object* obj) {
  ...
  obj->regKernel<CastRingV>();
  obj->regKernel<CastRingP>();
}
