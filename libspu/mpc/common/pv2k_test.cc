...
TEST(Pub2kTest, TypeWorks) {
  ...
  {
    Type ty = makeType<Pub2kTy>(SE_I32);
    EXPECT_EQ(ty.size(), 4);
    ...
    EXPECT_EQ(ty.toString(), "Pub2k<SE_I32>");

    EXPECT_EQ(Type::fromString(ty.toString()), ty);
  }

  {
    Type ty = makeType<Pub2kTy>(SE_I128);
    ...
    EXPECT_EQ(ty.toString(), "Pub2k<SE_I128>");
...
TEST(Priv2kTest, TypeWorks) {
  ...
  {
    Type ty = makeType<Priv2kTy>(SE_I32, -1);
    EXPECT_EQ(ty.size(), 4);
    ...
    EXPECT_EQ(ty.toString(), "Priv2k<SE_I32,-1>");
  ...
  {
    Type ty = makeType<Priv2kTy>(SE_I128, 1);
    EXPECT_EQ(ty.size(), 16);
    ...
    EXPECT_EQ(ty.toString(), "Priv2k<SE_I128,1>");
