...
class Communicator : public State {
 ...
 public:
  ...
  MemRef allReduce(ReduceOp op, const MemRef& in, std::string_view tag);

  MemRef reduce(ReduceOp op, const MemRef& in, size_t root,
                std::string_view tag);

  MemRef rotate(const MemRef& in, std::string_view tag);

  void sendAsync(size_t dst_rank, const MemRef& in, std::string_view tag);

  MemRef recv(size_t src_rank, const Type& eltype, std::string_view tag);
