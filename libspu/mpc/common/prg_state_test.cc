...
TEST(PrgStateTest, Fork) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    dynamic_cast<PrgState*>(sub1.get())
        ->fillPrssPair(data0[rank].data(), &data0[rank][1], sizeof(int));
    dynamic_cast<PrgState*>(sub2.get())
        ->fillPrssPair(data1[rank].data(), &data1[rank][1], sizeof(int));
    dynamic_cast<PrgState*>(sub1_sub1.get())
        ->fillPrssPair(data2[rank].data(), &data2[rank][1], sizeof(int));
