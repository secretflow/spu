...
#define TEST_BINARY_OP_SS(OP)                                               \
  TEST_P(ApiTest, OP##_ss) {                                                \
    ...
    auto field = GetSemanticType(conf.protocol().field());                  \
                                                                            \
    utils::simulate(npc,                                                    \
                    [&](const std::shared_ptr<yacl::link::Context>& lctx) { \
                      auto sctx = factory(conf, lctx);                      \
                                                                            \
                      /* GIVEN */                                           \
                      auto x_p = rand_p(sctx.get(), field, kShape);         \
                      auto y_p = rand_p(sctx.get(), field, kShape);         \
    ...
    utils::simulate(npc,                                                    \
                    [&](const std::shared_ptr<yacl::link::Context>& lctx) { \
                      auto sctx = factory(conf, lctx);                      \
                                                                            \
                      /* GIVEN */                                           \
                      auto p0 = rand_p(sctx.get(), field, kShape);          \
                                                                            \
                      /* FIXME: verify self?*/                              \
    ...
    utils::simulate(                                                          \
        npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {          \
          ...
          for (auto bits : kShiftBits) {                                      \
            if (bits >= SizeOf(conf.protocol().field()) * 8) {                \
              continue;                                                       \
            }                                                                 \
            /* WHEN */                                                        \
            auto r_s = s2p(sctx.get(), OP##_s(sctx.get(), x_s,                \
...
TEST_P(ApiTest, TruncS) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    p0 = arshift_p(
        sctx.get(), p0,
        {static_cast<int64_t>(
            SizeOf(GetSemanticType(conf.protocol().field())) * 8 - 10)});
...
TEST_P(ApiTest, MatMulSS) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* GIVEN */
    auto p0 = rand_p(sctx.get(), field, shape_A);
    auto p1 = rand_p(sctx.get(), field, shape_B);
    auto p2 = rand_p(sctx.get(), field, shape_B2);
