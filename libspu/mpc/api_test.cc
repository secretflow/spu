    ...
    utils::simulate(npc,                                                    \
                    [&](const std::shared_ptr<yacl::link::Context>& lctx) { \
                      auto sctx = factory(conf, lctx);                      \
                                                                            \
                      /* GIVEN */                                           \
                      auto x_p = rand_p(sctx.get(), kShape, field);         \
                      auto y_p = rand_p(sctx.get(), kShape, field);         \
    ...
    utils::simulate(npc,                                                    \
                    [&](const std::shared_ptr<yacl::link::Context>& lctx) { \
                      auto sctx = factory(conf, lctx);                      \
                                                                            \
                      /* GIVEN */                                           \
                      auto p0 = rand_p(sctx.get(), kShape, field);          \
                                                                            \
                      /* FIXME: verify self?*/                              \
...
TEST_P(ApiTest, TruncS) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    auto field = p0.data().eltype().as<RingTy>()->field();
    p0 = arshift_p(sctx.get(), p0, SizeOf(field) * 8 - 10);
...
TEST_P(ApiTest, MatMulSS) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    /* GIVEN */
    auto p0 = rand_p(sctx.get(), shape_A, field);
    auto p1 = rand_p(sctx.get(), shape_B, field);
    auto p2 = rand_p(sctx.get(), shape_B2, field);
...
TEST_P(ApiTest, CastRingS) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    if (!sctx->hasKernel("cast_ring_s")) {
      return;  // If protcol has no cast_type_s...give up
    }

    std::vector<FieldType> para = {FM64};

    for (auto ftype : para) {
      // auto p0 = rand_p(sctx.get(), kShape);
      auto p0 = make_p(sctx.get(), 100000, kShape, field);

      // TODO: here we assume has msb error, only use lowest 10 bits.
      p0 = arshift_p(sctx.get(), p0, SizeOf(field) * 8 - 10);

      auto r_s =
          s2p(sctx.get(), cast_ring_s(sctx.get(), p2s(sctx.get(), p0), ftype));
      auto r_p = cast_ring_p(sctx.get(), p0, ftype);
