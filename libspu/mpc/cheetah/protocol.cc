...
void regCheetahProtocol(SPUContext* ctx,
  ...
  ctx->prot()->addState<cheetah::CheetahMulState>(
      lctx, ctx->config().protocol().cheetah_config().enable_mul_lsb_error());
  ctx->prot()->addState<cheetah::CheetahDotState>(
      lctx, ctx->config().protocol().cheetah_config().disable_matmul_pack());
  ctx->prot()->addState<cheetah::CheetahOTState>(
      ctx->getClusterLevelMaxConcurrency(),
      ctx->config().protocol().cheetah_config().ot_kind());
  ...
  ctx->prot()
      ->regKernel<cheetah::P2A, cheetah::A2P, cheetah::V2A,
                  ...
                  cheetah::BitCompose, cheetah::BitDecompose,               //
                  cheetah::RandA, cheetah::RingCastS>();
}
