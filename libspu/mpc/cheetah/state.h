...
using OTUnaryFunc = std::function<MemRef(
    const MemRef& sub, const std::shared_ptr<BasicOTProtocols>& ot)>;

using OTBinaryFunc =
    std::function<MemRef(const MemRef& op0, const MemRef& op1,
                         const std::shared_ptr<BasicOTProtocols>& ot)>;

using OTUnaryFuncWithU8 = std::function<MemRef(
    absl::Span<const uint8_t> op, const std::shared_ptr<BasicOTProtocols>& ot)>;

using OTBinaryFuncWithU8 =
    std::function<MemRef(const MemRef& op0, absl::Span<const uint8_t> op1,
                         const std::shared_ptr<BasicOTProtocols>& ot)>;
...
class CheetahMulState : public State {
 ...
 public:
  ...
  std::array<MemRef, 3> TakeCachedBeaver(size_t field, int64_t num);
};
