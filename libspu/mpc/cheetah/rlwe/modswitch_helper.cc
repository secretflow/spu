...
struct ModulusSwitchHelper::Impl {
 public:
  ...
  void ModulusUpAt(MemRefView<const Scalar> src, size_t mod_idx,
  ...
  // \in [2^64, 2^128).
  void ModulusUpAt(MemRefView<const uint128_t> src, size_t mod_idx,
                   absl::Span<uint64_t> out) const {
    using namespace seal::util;
  ...
  template <typename Scalar>
  void CenteralizeAt(MemRefView<const Scalar> src, size_t mod_idx,
                     absl::Span<uint64_t> out) const {
    using namespace seal::util;
    SPU_ENFORCE_EQ(sizeof(Scalar) * 8, absl::bit_ceil(base_mod_bitlen_),
                   fmt::format("expect base_mod_bitlen={} but got {}",
...
void ModulusSwitchHelper::ModulusUpAt(const MemRef &src, size_t mod_idx,
                                      absl::Span<uint64_t> out) const {
  yacl::CheckNotNull(impl_.get());
  ...
  SPU_ENFORCE(eltype.isa<BaseRingType>(), "source must be ring_type, got={}",
              eltype);

  DISPATCH_ALL_STORAGE_TYPES(eltype.storage_type(), [&]() {
    using ring2u = std::make_unsigned<ScalarT>::type;
    impl_->ModulusUpAt(MemRefView<const ring2u>(src), mod_idx, out);
  });
}

void ModulusSwitchHelper::CenteralizeAt(const MemRef &src, size_t mod_idx,
                                        absl::Span<uint64_t> out) const {
  yacl::CheckNotNull(impl_.get());
  const Type &eltype = src.eltype();
  const size_t numel = src.numel();
  ...
  DISPATCH_ALL_STORAGE_TYPES(eltype.storage_type(), [&]() {
    using ring2u = std::make_unsigned<ScalarT>::type;
    impl_->CenteralizeAt(MemRefView<const ring2u>(src), mod_idx, out);
  });
}

MemRef ModulusSwitchHelper::ModulusDownRNS(
    size_t field, const Shape &shape, absl::Span<const uint64_t> src) const {
  yacl::CheckNotNull(impl_.get());
  ...
  MemRef out(makeType<RingTy>(SE_INVALID, field), shape);
  ring_zeros(out);
...
void ModulusSwitchHelper::ModulusDownRNS(absl::Span<const uint64_t> src,
  ...
  return DISPATCH_ALL_STORAGE_TYPES(eltype.storage_type(), [&]() {
    using ring2u = std::make_unsigned<ScalarT>::type;
