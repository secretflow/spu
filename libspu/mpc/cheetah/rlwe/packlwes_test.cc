...
class VectorEncoder {
 public:
  ...
  void Forward(const MemRef &vec, RLWEPt *out, bool scale_delta = true) const;

  void Backward(const MemRef &vec, RLWEPt *out, bool scale_delta = false) const;
...
class PackLWEsTest : public testing::TestWithParam<std::tuple<size_t, size_t>> {
 protected:
...
TEST_P(PackLWEsTest, PackRLWEs) {
  ...
  std::vector<MemRef> arrays(num_rlwes);
  std::vector<RLWECt> rlwes(num_rlwes);
  for (size_t i = 0; i < num_rlwes; ++i) {
    MemRef array(makeType<RingTy>(SE_INVALID, field), {poly_N});
    ring_rand(array);
  ...
  for (size_t i = 0; i < poly_N; i += num_rlwes) {
    ...
    for (size_t j = 0; j < num_rlwes; ++j) {
      MemRefView<scalar_t> expected(arrays[j]);
...
TEST_P(PackLWEsTest, Basic) {
  ...
  MemRef array(makeType<RingTy>(SE_INVALID, field), {poly_N});
  ring_rand(array);
  MemRefView<scalar_t> _array(array);
...
void VectorEncoder::Forward(const MemRef &vec, RLWEPt *out,
...
void VectorEncoder::Backward(const MemRef &vec, RLWEPt *out,
                             bool scale_delta) const {
  ...
  const auto field = eltype.as<BaseRingType>()->valid_bits();

  DISPATCH_ALL_STORAGE_TYPES(eltype.storage_type(), [&]() {
    using ring2k_t = ScalarT;
    MemRef tmp_buff(makeType<RingTy>(SE_INVALID, field),
                    {static_cast<int64_t>(poly_deg_)});
    ring_zeros(tmp_buff);
    auto xvec = MemRefView<const ring2k_t>(vec);
    auto xtmp = MemRefView<ring2k_t>(tmp_buff);
