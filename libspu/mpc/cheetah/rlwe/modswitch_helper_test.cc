...
class RLWE2LWETest : public testing::TestWithParam<size_t> {
 protected:
  ...
  inline uint32_t FieldBitLen(size_t f) const { return 8 * SizeOf(f); }

  MemRef CPRNG(size_t field, size_t size) {
    MemRef ret(makeType<RingTy>(SE_INVALID, field),
...
INSTANTIATE_TEST_SUITE_P(
    Cheetah, RLWE2LWETest, testing::Values(32, 64, 128),
...
TEST_P(RLWE2LWETest, ModulusSwitch_UpDown) {
  ...
  for (int64_t stride : {1, 2, 3}) {
    MemRef vec(makeType<RingTy>(SE_INVALID, field_), {poly_deg});
    ring_rand(vec);

    auto _vec = vec.slice({3}, {(vec.numel() - 3 - 1) / stride + 1}, {stride});
    ...
    // check r =? e
    DISPATCH_ALL_STORAGE_TYPES(_vec.eltype().storage_type(), [&]() {
      using ring2k_t = ScalarT;
      auto expected = MemRefView<ring2k_t>(_vec);
      auto computed = MemRefView<ring2k_t>(cmp);
...
TEST_P(RLWE2LWETest, ModulusSwitch_DownUp) {
  // a <- R_t
  MemRef vec_a(makeType<RingTy>(SE_INVALID, field_), {poly_deg});
  ring_rand(vec_a);
  ...
  MemRef shr0(makeType<RingTy>(SE_INVALID, field_),
              {static_cast<int64_t>(poly_deg)});
  MemRef shr1(makeType<RingTy>(SE_INVALID, field_),
              {static_cast<int64_t>(poly_deg)});
  ring_zeros(shr0);
  ring_zeros(shr1);
  {
  ...
  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field_), [&]() {
    using ring2k_t = ScalarT;
    auto expected = MemRefView<ring2k_t>(vec_a);
    auto computed0 = MemRefView<ring2k_t>(shr0);
    auto computed1 = MemRefView<ring2k_t>(shr1);
