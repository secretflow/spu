...
class ModulusSwitchHelper {
 public:
  ...
  // Need 1D array
  void ModulusUpAt(const MemRef& src, size_t mod_idx,
                   absl::Span<uint64_t> out) const;

  // Cast [-2^{k-1}, 2^{k-1}) to [0, qj)
  // Need 1D array
  void CenteralizeAt(const MemRef& src, size_t mod_idx,
                     absl::Span<uint64_t> out) const;
  // Given x' in \ [0, Q), compute round(x'*2^k/Q) \in [0, 2^k)
  MemRef ModulusDownRNS(size_t field, const Shape& shape,
                        absl::Span<const uint64_t> src) const;

  void ModulusDownRNS(absl::Span<const uint64_t> src, MemRef out) const;
