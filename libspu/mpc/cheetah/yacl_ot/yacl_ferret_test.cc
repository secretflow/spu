...
TEST_P(FerretCOTTest, ChosenMsgChosenChoice) {
  ...
  DISPATCH_ALL_FIELDS(field, "", [&]() {
    ...
    for (size_t bw : {2UL, 4UL, sizeof(scalar_t) * 8}) {
      ...
      // @wenfan
      // N = 4,5,6,7,8 would raise errors as following:
      // 1. (N=8) --> corrupted double-linked list
      // 2. (otherwise) --> free(): invalid next size (fast)
      for (int64_t N : {2, 3, 4, 5, 6, 7, 8}) {
        SPDLOG_INFO(fmt::format("bw is {}, N is {}", bw, N));
        ...
        utils::simulate(
            kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
              ...
              SPDLOG_INFO(fmt::format("Rank {} End of Simulation", rank));
            });
        SPDLOG_INFO(fmt::format("Test Checking"));
        ...
        SPDLOG_INFO(fmt::format("End of Test Case"));
      }
    }
  });
}
