...
#include "libspu/mpc/cheetah/beaver/modswitch_helper.h"
...
struct Beaver::MulImpl : public EnablePRNG {
 public:
...
void Beaver::MulImpl::LazyInitModSwitchHelper(uint32_t field_bitlen) {
...
void Beaver::MulImpl::LazyExpandSEALContexts(uint32_t field_bitlen,
  ...
  SPDLOG_INFO("Beaver::Mul uses {} modulus ({} bit each) for {} bit ring",
...
ArrayRef Beaver::MulImpl::MulOLE(const ArrayRef &shr, yacl::link::Context *conn,
                                 bool evaluator) {
...
Beaver::Triple Beaver::MulImpl::Mul(FieldType field, size_t size) {
...
size_t Beaver::MulImpl::EncryptArrayThenSend(const ArrayRef &array,
                                             std::vector<RLWEPt> *out,
                                             yacl::link::Context *conn) {
  size_t num_elts = array.numel();
  auto eltype = array.eltype();
...
ArrayRef Beaver::MulImpl::_PrepareRandomMask(
...
void Beaver::MulImpl::EncodeArray(const ArrayRef &array, const Options &options,
                                  absl::Span<RLWEPt> out) {
  size_t num_elts = array.numel();
  auto eltype = array.eltype();
  YACL_ENFORCE(num_elts > 0, "empty array");
...
ArrayRef Beaver::MulImpl::ElementMulThenResponse(
    FieldType field, size_t num_elts, absl::Span<const yacl::Buffer> ciphers,
    absl::Span<const RLWEPt> plains, yacl::link::Context *conn) {
  YACL_ENFORCE(ciphers.size() > 0, "Beaver: empty cipher");
  YACL_ENFORCE(plains.size() == ciphers.size(), "Beaver: ct/pt size mismatch");
  ...
  YACL_ENFORCE(ecd_random.size() == num_ciphers,
               "Beaver: encoded poly size mismatch");
...
ArrayRef Beaver::MulImpl::DecryptArray(
  ...
  const size_t num_seal_ctx = WorkingContextSize(field_bitlen);
  const size_t num_ciphers = num_seal_ctx * num_splits;
  YACL_ENFORCE(ct_array.size() == num_ciphers, "Beaver: cipher size mismatch");
  YACL_ENFORCE(ms_helpers_.count(field_bitlen) > 0);
...
void Beaver::MulImpl::_NoiseFloodCiphertext(RLWECt &ct,
...
void Beaver::MulImpl::RandomizeCipherForDecryption(RLWECt &ct, size_t cidx) {
...
struct Beaver::DotImpl : public EnablePRNG {
 public:
...
void Beaver::DotImpl::LazyInit(size_t field_bitlen) {
  {
    std::shared_lock<std::shared_mutex> guard(context_lock_);
  ...
  SPDLOG_INFO("Beaver::Dot uses {} modulus {} degree for {} bit ring",
              modulus.size(), parms.poly_modulus_degree(), field_bitlen);
}

ArrayRef Beaver::DotImpl::DotOLE(const ArrayRef &prv, yacl::link::Context *conn,
...
// Compute C = A*B where |A|=M*K, |B|=K*N
Beaver::Triple Beaver::DotImpl::Dot(FieldType field, size_t M, size_t N,
                                    size_t K) {
...
Beaver::Beaver(std::shared_ptr<yacl::link::Context> lctx)
...
Beaver::Triple Beaver::Mul(FieldType field, size_t size) {
  yacl::CheckNotNull(mul_impl_.get());
  return mul_impl_->Mul(field, size);
}

ArrayRef Beaver::MulOLE(const ArrayRef &shr, yacl::link::Context *conn,
                        bool evaluator) {
  yacl::CheckNotNull(mul_impl_.get());
  return mul_impl_->MulOLE(shr, conn, evaluator);
}

Beaver::Triple Beaver::Dot(FieldType field, size_t M, size_t N, size_t K) {
  yacl::CheckNotNull(dot_impl_.get());
  return dot_impl_->Dot(field, M, N, K);
}

ArrayRef Beaver::DotOLE(const ArrayRef &prv, yacl::link::Context *conn,
                        size_t M, size_t N, size_t K, bool is_rhs) {
  yacl::CheckNotNull(dot_impl_.get());
  return dot_impl_->DotOLE(prv, conn, M, N, K, is_rhs);
}

Beaver::Triple Beaver::And(FieldType field, size_t size) {
