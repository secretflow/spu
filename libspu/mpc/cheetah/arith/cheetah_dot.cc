...
struct CheetahDot::Impl : public EnableCPRNG {
 public:
  ...
  NdArrayRef DotOLE(const NdArrayRef &prv_mat, yacl::link::Context *conn,
                    const Shape3D &dim3, bool is_self_lhs, bool rand_mask,
                    bool profiling);
  ...
  void doDotOLEReceiverRecvStep(const NdArrayRef &prv_mat, const Shape3D &dim3,
                                ...
                                yacl::link::Context *conn, bool rand_mask,
                                bool profiling);
  ...
  void RShiftPacking(size_t gap, absl::Span<RLWECt> rlwes, RLWECt &packed,
                     const seal::SEALContext &context);
...
void CheetahDot::Impl::doDotOLEReceiverRecvStep(
    const NdArrayRef &prv_mat, const Shape3D &dim3, bool is_self_lhs,
    CipherPackingType cptype, absl::Span<RLWECt> result_cts,
    yacl::link::Context *conn, bool rand_mask, bool profiling) {
  ...
  if (!profiling) {
    // receive Enc(y_complete) for correctness, comment when profiling.
    ...
    io_task.get();  // TODO: this can be done asyncly, put it at a later place
  } else {
    // TODO: maybe not correct resize.
    for (size_t i = 0; i < enc_mat.size(); ++i) {
      if (enc_mat[i].size() == 0) {
        enc_mat[i].resize(this_context, this_context.first_parms_id(),
                          result_cts[0].size());
      }
    }
  }

  double pack_time_1 = 0.0;
  yacl::ElapsedTimer _timer_1;
  ...
  // yacl::parallel_for(0, plain_mat.size(),
  // CalculateWorkLoad(plain_mat.size()),
  //                    [&](size_t bgn, size_t end) {
  //                      for (size_t i = bgn; i < end; ++i) {
  //                        NttInplace(plain_mat[i], this_context);
  //                      }
  //                    });
  pack_time_1 = _timer_1.CountMs();
  if (conn->Rank() == 1) {
    SPDLOG_INFO("empty@{}x{}x{} => {}x{}x{} Plain Encode {} ms", meta.dims[0],
                meta.dims[1], meta.dims[2], subshape[0], subshape[1],
                subshape[2], std::roundf(pack_time_1 * 1000) / 1000.);
  }

  // 3. HE multiplications
  double pack_time_2 = 0.0;
  yacl::ElapsedTimer _timer_2;
  if (is_self_lhs) {
    std::cout << "This should not be called" << std::endl;
    matmat_prot.Compute(plain_mat, enc_mat, meta, result_cts);
  } else {
    // matmat_prot.Compute(enc_mat, plain_mat, meta, result_cts, rand_mask);
    matmat_prot.Compute(enc_mat, prv_mat, meta, result_cts);
  }
  pack_time_2 = _timer_2.CountMs();

  if (conn->Rank() == 1) {
    SPDLOG_INFO("empty@{}x{}x{} => {}x{}x{} Compute {} ms", meta.dims[0],
                meta.dims[1], meta.dims[2], subshape[0], subshape[1],
                subshape[2], std::roundf(pack_time_2 * 1000) / 1000.);
  }
}

void CheetahDot::Impl::RShiftPacking(size_t gap, absl::Span<RLWECt> rlwes,
                                     RLWECt &packed,
                                     const seal::SEALContext &context) {
  ...
  RLWECt tmp;
  for (size_t i = 1; i < rlwes.size(); ++i) {
    tmp = rlwes[i];
    NegacyclicRightShiftInplace(tmp, i * poly_degree / gap, context);
    CATCH_SEAL_ERROR(evaluator.add_inplace(rlwes[0], tmp));
  }
  packed = rlwes[0];
}
...
NdArrayRef CheetahDot::Impl::doDotOLEReceiverSendStep(
  ...
  if (cptype == CipherPackingType::none) {
    ...
    int64_t poly_degree =
        this_context.first_context_data()->parms().poly_modulus_degree();
    ;
    // TODO: support shape cannot be divided.
    SPU_ENFORCE_EQ(poly_degree % subshape[0], 0);
    const size_t gap = poly_degree / subshape[0];
    ...
    for (size_t i = 0; i < out_n; i += pack_stride) {
      ...
      RShiftPacking(gap, ct_array_to_pack.subspan(i, this_batch),
                    ct_array_to_pack[packed_idx], this_context);
    }
...
void CheetahDot::Impl::doDotOLESenderSendStep(const NdArrayRef &prv_mat,
  ...
  pack_time_1 = _timer_1.CountMs();
  if (conn->Rank() == 0) {
    SPDLOG_INFO("empty@{}x{}x{} => {}x{}x{} Act Sender Encode {} ms",
                meta.dims[0], meta.dims[1], meta.dims[2], subshape[0],
                subshape[1], subshape[2],
                std::roundf(pack_time_1 * 1000) / 1000.);
  } else if (conn->Rank() == 1) {
    SPDLOG_INFO("empty@{}x{}x{} => {}x{}x{} Act Receiver Encode {} ms",
                meta.dims[0], meta.dims[1], meta.dims[2], subshape[0],
                subshape[1], subshape[2],
                std::roundf(pack_time_1 * 1000) / 1000.);
  }
...
NdArrayRef CheetahDot::Impl::DotOLE(const NdArrayRef &prv_mat,
  ...
  return doDotOLE(prv_mat, conn, dim3, is_self_lhs, rand_mask, profiling);
}
...
NdArrayRef CheetahDot::Impl::BatchDotOLE(const NdArrayRef &prv_mat,
  ...
  if (eltype.template as<Ring2k>()->field() == FM32) {
    ...
    for (int64_t b = 0; b < dim4[0]; ++b) {
      ...
      auto one_out = DotOLE(one_mat, conn, dim3, is_self_lhs, false, false);
      auto out_slice =
...
NdArrayRef CheetahDot::Impl::doDotOLE(const NdArrayRef &prv_mat,
  ...
  if (!profiling) {
    ...
    bool to_encrypt_lhs = lhs_poly_n <= rhs_poly_n;
    act_as_encryptor = (is_self_lhs ^ to_encrypt_lhs) == 0;
  } else {
    // Force the lhs (holds complete weight) send, comment when profiling.
    act_as_encryptor = is_self_lhs;
  }

  if (act_as_encryptor) {
    if (!profiling) {
      // send Enc(y_complete) for correctness, comment when profiling.
    ...
    if (cptype == CipherPackingType::rlwes) {
      ...
      // (poly/subshape[0]) CTs are packed, so we need to divide
      // The subshape[0] should be a factor of poly_deg
      num_ct_to_recv = CeilDiv<size_t>(num_ct_to_recv, poly_deg / subshape[0]);
    } else {
...
NdArrayRef CheetahDot::DotOLE(const NdArrayRef &inp, yacl::link::Context *conn,
  ...
  return impl_->DotOLE(inp, conn, dim3, is_self_lhs, rand_mask, profiling);
}

NdArrayRef CheetahDot::DotOLE(const NdArrayRef &inp, const Shape3D &dim3,
                              bool is_self_lhs, bool rand_mask,
                              bool profiling) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->DotOLE(inp, nullptr, dim3, is_self_lhs, rand_mask, profiling);
}
