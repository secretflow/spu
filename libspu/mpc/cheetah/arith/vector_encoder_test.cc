...
class VectorEncoderTest : public ::testing::TestWithParam<size_t> {
 protected:
...
INSTANTIATE_TEST_SUITE_P(
    Cheetah, VectorEncoderTest, testing::Values(32, 64, 128),
...
TEST_P(VectorEncoderTest, ForwardBackward) {
  ...
  for (bool scale_forward : {true, false}) {
    for (int64_t n : {100L, (long)poly_deg}) {
      MemRef vec0(makeType<RingTy>(SE_INVALID, field_), {n});
      MemRef vec1(makeType<RingTy>(SE_INVALID, field_), {n});
      ring_rand(vec0);
      ring_rand(vec1);
      ...
      DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field_), [&]() {
        MemRefView<ScalarT> got(computed);
        MemRefView<ScalarT> v0(vec0);
        MemRefView<ScalarT> v1(vec1);
        ScalarT expected = 0;
