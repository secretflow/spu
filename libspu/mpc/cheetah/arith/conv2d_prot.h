...
class Conv2DProtocol {
 public:
  ...
  size_t GetKernelSize(const Meta& meta) const {
    return GetKernelSize(meta, GetSubMemRefShape(meta));
  }

  size_t GetInputSize(const Meta& meta) const {
    return GetInputSize(meta, GetSubMemRefShape(meta));
  }

  size_t GetOutSize(const Meta& meta) const {
    return GetOutSize(meta, GetSubMemRefShape(meta));
  }
  ...
  Shape3D GetSubMemRefShape(const Conv2DProtocol::Meta& meta) const;
  ...
  void Compute(absl::Span<const RLWEPt> memref, absl::Span<const RLWEPt> kernel,
               const Meta& meta, absl::Span<RLWEPt> out) const;

  void Compute(absl::Span<const RLWECt> memref, absl::Span<const RLWEPt> kernel,
               const Meta& meta, absl::Span<RLWECt> out) const;

  void Compute(absl::Span<const RLWEPt> memref, absl::Span<const RLWECt> kernel,
               const Meta& meta, absl::Span<RLWECt> out) const;
 ...
 private:
  ...
  std::shared_ptr<MemRefEncoder> tencoder_{nullptr};
};
