...
class MatMatProtocol {
 ...
 private:
  mutable std::atomic_int64_t skiped_ = 0;
  mutable std::atomic_int64_t total_ = 0;
 ...
 public:
  ...
  void Compute(absl::Span<const RLWECt> lhs_mat, const NdArrayRef& rhs_mat,
               const Meta& meta, absl::Span<RLWECt> out_mat) const;

 private:
  // work horse
  template <typename LHS, typename RHS, typename O>
  void DoCompute(absl::Span<const LHS> lhs, absl::Span<const RHS> rhs,
                 const Meta& meta, absl::Span<O> out, bool rand_mask) const;
  void DoCompute(absl::Span<const RLWECt> lhs, const NdArrayRef& rhs,
                 const Meta& meta, absl::Span<RLWECt> out,
                 bool rand_mask) const;

  // accum += x * y
  template <class T0, class T1, class T2>
  void FusedMulAddInplace(T0& accum, const T1& x, const T2& y) const;

  void FusedMulAddInplace_scalar(RLWECt& accum, const RLWECt& x,
                                 absl::Span<const uint64_t> y) const;
