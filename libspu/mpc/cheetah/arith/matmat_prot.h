...
class MatMatProtocol {
 public:
  ...
  void EncodeLHS(const MemRef& lhs_mat, const Meta& meta, bool need_encrypt,
                 absl::Span<RLWEPt> out) const;

  void EncodeRHS(const MemRef& rhs_mat, const Meta& meta, bool need_encrypt,
                 absl::Span<RLWEPt> out) const;

  bool IsValidMeta(const Meta& meta) const;

  // Parse the polynomails into matmul result without any packing
  MemRef ParseResult(size_t field, const Meta& meta,
  ...
  MemRef ParsePackedResult(size_t field, const Meta& meta,
                           absl::Span<const RLWEPt> ans_poly,
                           const ModulusSwitchHelper& msh) const;
  ...
  MemRef ParseBatchPackedResult(size_t field, size_t batch_size,
                                const Meta& meta,
  ...
  // PackLWEs packing.
  MemRef ParsePackLWEsResult(size_t field, const Meta& meta,
                             absl::Span<const RLWEPt> ans_poly,
                             const ModulusSwitchHelper& msh) const;
 ...
 private:
  ...
  void EncodeMatrix(const MemRef& mat, const Meta& meta, int pivot,
