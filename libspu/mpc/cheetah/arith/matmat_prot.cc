...
MemRef ConcatSubMatrix(const MemRef& mat, const Shape2D& mat_shape,
  ...
  const auto field = SizeOf(eltype.storage_type()) * 8;
  // NOTE: zero padding via initialization
  MemRef flatten(makeType<RingTy>(SE_INVALID, field), {num_coeff});
  ring_zeros(flatten);

  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
    using uT = std::make_unsigned<ScalarT>::type;
...
void MatMatProtocol::EncodeLHS(const MemRef& mat, const Meta& meta,
                               bool need_encrypt,
...
void MatMatProtocol::EncodeRHS(const MemRef& mat, const Meta& meta,
                               bool need_encrypt,
                               absl::Span<RLWEPt> out) const {
  int pivot = 1;
...
template <typename Indexer>
void MatMatProtocol::EncodeMatrix(const MemRef& mat, const Meta& meta,
...
MemRef MatMatProtocol::ParseResult(size_t field, const Meta& meta,
  ...
  MemRef matmat(makeType<RingTy>(SE_INVALID, field),
    ...
    for (int64_t cb = 0; cb < out_blks[1]; ++cb) {
      ...
      for (int64_t r = 0; r < row_ext; ++r) {
        for (int64_t c = 0; c < col_ext; ++c) {
          ...
          DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
            matmat.at<ScalarT>(dst_idx) =
                result_poly.at<ScalarT>(r * subdims[2] + c);
          });
        }
      }
    }
...
MemRef MatMatProtocol::ParseBatchPackedResult(
    size_t field, size_t batch_size, const Meta& meta,
  ...
  std::vector<MemRef> decoded_vectors(polys.size());
  ...
  MemRef mat(makeType<RingTy>(SE_INVALID, field),
             {static_cast<int64_t>(batch_size * numel_per_dot)});
  ring_zeros(mat);
  ...
  for (int64_t idx = 0; idx < total_polys; idx += polys_per_dot) {
    ...
    auto out_slice =
        mat.slice({out_mat_idx * numel_per_dot}, {numel_per_dot}, {1});
    ...
    for (int64_t poly_idx = 0; poly_idx < polys_per_dot; ++poly_idx) {
      ...
      for (int64_t r = 0; r < row_ext; ++r) {
        for (int64_t c = 0; c < col_ext; ++c) {
          ...
          const MemRef& dcd_vec = decoded_vectors.at(o.first);
          std::memcpy(
...
MemRef MatMatProtocol::ParsePackLWEsResult(
    size_t field, const Meta& meta, absl::Span<const RLWEPt> ans_poly,
    const ModulusSwitchHelper& msh) const {
  ...
  std::vector<MemRef> decoded_vectors(ans_poly.size());
  ...
  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
    MemRefView<ScalarT> xmatmat(matmat);

    for (size_t i = 0; i < ans_poly.size(); ++i) {
      MemRefView<const ScalarT> decoded_vec(decoded_vectors[i]);
