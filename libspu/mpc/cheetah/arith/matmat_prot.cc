...
MatMatProtocol::~MatMatProtocol() {
  auto t = total_.load();
  auto s = skiped_.load();
  if (t != 0)
    SPDLOG_WARN("skiped {}, total {}, ratio {}", s, t,
                static_cast<float>(s) / t);
}
...
Shape3D MatMatProtocol::GetSubMatShape(const Meta& meta, int64_t poly_deg,
                                       bool disable_pack) {
  // force to perform the scalar-vector output.
  // vector from lhs, scalar from rhs.
  Shape3D subshape;
  ...
  if (dim3[0] >= poly_deg) {
    ...
    tmp |= tmp >> 32;
    tmp = (tmp + 1);
    // TODO: maybe using smaller closest is better? But this will
    // cause potential error. For example, 3395x1x417. Using 2048
    // makes computation block number different from packed number.
  }
  subshape = {tmp, 1, 1};
  return subshape;
  ...
  //                   num_automorph * cpu_price + num_ct_mul * cpu_price
  //                   / 10.0;
...
void MatMatProtocol::FusedMulAddInplace(RLWECt& acc, const RLWECt& lhs,
  ...
  size_t num_modulus = lhs.coeff_modulus_size();

  // // force to perform 1 * vector
  // std::vector<uint64_t> _scalar(num_modulus);
  // auto scalar_wrap = absl::MakeSpan(_scalar);
  // NdArrayRef scalar = ring_ones(FM64, {1});

  // DISPATCH_ALL_FIELDS(FM64, "_", [&]() {
  //   NdArrayView<ring2k_t> _ret(scalar);
  //   pforeach(0, 1, [&](int64_t idx) { _ret[idx] = _ret[idx] << 18; });
  // });

  // for (size_t midx = 0; midx < num_modulus; ++midx) {
  //   msh_.CenteralizeAt(scalar, midx, scalar_wrap.subspan(midx, 1));
  // }
  ...
  for (size_t k = 0; k < lhs.size(); ++k) {
    ...
    for (size_t mod_idx = 0; mod_idx < num_modulus; ++mod_idx) {
      for (size_t poly_idx = 0; poly_idx < coeff_count; ++poly_idx) {
        // Multiply batched scalar*vector is not supported.
        *dst = sut::multiply_add_uint_mod(*op0, *op1, *dst, modulus[mod_idx]);
        ++dst;
        ++op0;
        // ++op1;
      }
...
void MatMatProtocol::FusedMulAddInplace_scalar(
    RLWECt& acc, const RLWECt& lhs, absl::Span<const uint64_t> rhs) const {
  namespace sut = seal::util;
  // SPU_ENFORCE(lhs.parms_id() == rhs.parms_id());
  auto cntxt_data = context_.get_context_data(lhs.parms_id());
  SPU_ENFORCE(cntxt_data != nullptr);
  ...
  for (size_t k = 0; k < lhs.size(); ++k) {
    ...
    for (size_t mod_idx = 0; mod_idx < num_modulus; ++mod_idx) {
      for (size_t poly_idx = 0; poly_idx < coeff_count; ++poly_idx) {
        // Multiply batched scalar*vector is not supported.
        *dst = sut::multiply_add_uint_mod(*op0, rhs[mod_idx], *dst,
                                          modulus[mod_idx]);
        ++dst;
        ++op0;
      }
...
struct PackRLWEMappingHelper {
  ...
  std::pair<size_t, size_t> GetPackedIndex(size_t poly_idx,
    ...
    auto idx0 = poly_idx;  // poly_idx / gap_;
    auto idx1 =
        offset / gap_ +
        (offset % gap_) * group_size_;  // offset * gap_ + poly_idx % gap_;
    return {idx0, idx1};
  }
...
NdArrayRef MatMatProtocol::ParseBatchPackedResult(
  ...
  auto subshape = GetSubMatShape(meta);
  // forced subshape in "GetSubMatShape", force to recover after packing
  subshape[2] = poly_deg_ / subshape[0];
  const int64_t out_n = GetOutSize(meta, subshape);
  SPU_ENFORCE_EQ(polys.size(),
                 CeilDiv<size_t>(out_n * batch_size,
                                 subshape[1]));  // always subshape[1] = 1

  const int64_t gap = poly_deg_ / subshape[0];
  ...
  // std::cout << "total_polys: " << total_polys << std::endl;
  // std::cout << "polys_per_dot: " << polys_per_dot << std::endl;
  // std::cout << "numel_per_dot: " << numel_per_dot << std::endl;
  ...
  for (int64_t idx = 0; idx < total_polys; idx += polys_per_dot) {
    ...
    for (int64_t poly_idx = 0; poly_idx < polys_per_dot; ++poly_idx) {
      ...
      // std::cout << "row_ext: " << row_ext << std::endl;
      // std::cout << "col_ext: " << col_ext << std::endl;
      for (int64_t r = 0; r < row_ext; ++r) {
        for (int64_t c = 0; c < col_ext; ++c) {
...
bool Masked() { return dis(gen) > 10.2; }
...
void MatMatProtocol::DoCompute(absl::Span<const LHS> lhs,
                               ...
                               absl::Span<O> out, bool rand_mask) const {
  constexpr bool should_mask =
      std::is_same_v<LHS, RLWECt> && std::is_same_v<RHS, RLWEPt>;
  auto subshape = GetSubMatShape(meta);
  ...
  total_.fetch_add(dims[0] * dims[1] * dims[2]);
...
void MatMatProtocol::DoCompute(absl::Span<const RLWECt> lhs,
                               const NdArrayRef& rhs, const Meta& meta,
                               absl::Span<RLWECt> out, bool rand_mask) const {
  auto subshape = GetSubMatShape(meta);
  size_t lhs_n = GetLeftSize(meta, subshape);
  // size_t rhs_n = GetRightSize(meta, subshape);
  size_t out_n = GetOutSize(meta, subshape);
  SPU_ENFORCE_EQ(lhs.size(), lhs_n);
  // SPU_ENFORCE_EQ(rhs.numel(), rhs_n);  // rhs is a NdArrayRef by 1x1
  SPU_ENFORCE_EQ(out.size(), out_n);

  size_t num_modulus = lhs[0].coeff_modulus_size();
    ...
    yacl::parallel_for(0, dims[0], wload, [&](int64_t bgn, int64_t end) {
      ...
      for (int64_t i = bgn; i < end; ++i) {
        ...
        for (int64_t k = 0; k < dims[2]; ++k) {
          for (int64_t j = 0; j < dims[1]; ++j) {
            // rhs[j, k]
            auto scalar = rhs.slice({j, k}, {j + 1, k + 1}, {1, 1});
            for (size_t midx = 0; midx < num_modulus; ++midx) {
              msh_.CenteralizeAt(scalar, midx, scalar_wrap.subspan(midx, 1));
            }
            FusedMulAddInplace_scalar(out_row[k], lhs_row[j], scalar_wrap);
          }
        }
      }
    });
...
void MatMatProtocol::Compute(absl::Span<const RLWEPt> lhs_mat,
  ...
  DoCompute<RLWEPt, RLWEPt, RLWEPt>(lhs_mat, rhs_mat, meta, out_mat, false);
}

void MatMatProtocol::Compute(absl::Span<const RLWECt> lhs_mat,
                             absl::Span<const RLWEPt> rhs_mat, const Meta& meta,
                             absl::Span<RLWECt> out_mat, bool rand_mask) const {
  for (auto& ct : out_mat) {
    ct.release();
  }
  DoCompute<RLWECt, RLWEPt, RLWECt>(lhs_mat, rhs_mat, meta, out_mat, rand_mask);
}
...
void MatMatProtocol::Compute(absl::Span<const RLWEPt> lhs_mat,
  ...
  DoCompute<RLWEPt, RLWECt, RLWECt>(lhs_mat, rhs_mat, meta, out_mat, false);
}

void MatMatProtocol::Compute(absl::Span<const RLWECt> lhs_mat,
                             const NdArrayRef& rhs_mat, const Meta& meta,
                             absl::Span<RLWECt> out_mat) const {
  for (auto& ct : out_mat) {
    ct.release();
  }
  DoCompute(lhs_mat, rhs_mat, meta, out_mat, false);
}
}  // namespace spu::mpc::cheetah