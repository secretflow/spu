...
TEST_P(MatMatProtTest, Plain) {
  ...
  for (int64_t stride : {1, 2, 3}) {
    MemRef _lhs(makeType<RingTy>(SE_INVALID, field_),
                {meta.dims[0] * meta.dims[1] * stride});
    MemRef _rhs(makeType<RingTy>(SE_INVALID, field_),
                {meta.dims[1] * meta.dims[2] * stride + 1});
    ring_rand(_lhs);
    ring_rand(_rhs);

    auto lhs = _lhs.slice({0}, {meta.dims[0] * meta.dims[1]}, {stride});
    auto rhs = _rhs.slice({1}, {meta.dims[1] * meta.dims[2]}, {stride});
    ...
    DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field_), [&]() {
      auto xe = MemRefView<ScalarT>(expected);
      auto xc = MemRefView<ScalarT>(computed);
...
TEST_P(MatMatProtTest, EncLHS) {
  ...
  MemRef lhs(makeType<RingTy>(SE_INVALID, field_),
             {meta.dims[0], meta.dims[1]});
  ring_rand(lhs);
  MemRef rhs(makeType<RingTy>(SE_INVALID, field_),
