...
// A slice of a multi-dim memref with zero padding.
...
struct SlicedMemRef {
 public:
  ...
  SlicedMemRef(const ArrayRef &base, const Shape &base_shape,
               const Shape &offsets, const Shape &extents);

  static SlicedMemRef<Dim> Wrap(const ArrayRef &base, const Shape &base_shape,
                                const Shape &offsets, const Shape &extents);

  SlicedMemRef(const SlicedMemRef<Dim> &oth) = default;

  SlicedMemRef(SlicedMemRef<Dim> &&oth) noexcept = default;

  SlicedMemRef<Dim> &operator=(const SlicedMemRef<Dim> &oth) = delete;
...
// forward
using Sliced3DMemRef = SlicedMemRef<3>;
using Sliced4DMemRef = SlicedMemRef<4>;
...
class Conv2DHelper {
 public:
  ...
  Sliced3DMemRef Slice(const ArrayRef &base, const Shape3D &base_shape,
