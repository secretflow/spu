...
template <int Dim>
SlicedMemRef<Dim>::SlicedMemRef(const ArrayRef &base, const Shape &base_shape,
...
template <int Dim>
SlicedMemRef<Dim> SlicedMemRef<Dim>::Wrap(const ArrayRef &base,
                                          const Shape &shape,
                                          const Shape &offsets,
                                          const Shape &extents) {
  ...
  return SlicedMemRef<Dim>(base, shape, offsets, extents);
}
...
Sliced3DMemRef Conv2DHelper::Slice(
  ...
  return Sliced3DMemRef::Wrap(base, shape, offsets, extents);
}
...
void Conv2DHelper::GetResultCoefficients(std::array<int64_t, 3> indices,
  ...
  for (int d : {kH, kW}) {
    // handle submemref on the margins
