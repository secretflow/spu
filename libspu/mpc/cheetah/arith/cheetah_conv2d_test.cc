...
TEST_P(CheetahConv2dTest, Basic) {
  ...
  MemRef memref = ring_rand(field, {calcNumel(tshape) * N});
  MemRef kernel = ring_rand(field, {calcNumel(kshape) * O});
  ...
  utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    if (rank == 0) {
      result[rank] = dot->Conv2dOLE(flatten(memref), N, tshape, O, kshape,
                                    window_strides, true);
    } else {
  ...
  MemRef expected =
      ring_conv2d(memref, kernel, N, tshape, O, kshape, window_strides);
  ArrayRef computed =
  ...
  DISPATCH_ALL_FIELDS(field, [&]() {
    ...
    MemRefView<ring2k_t> exp(expected);
