...
class CheetahMul {
 public:
  ...
  void LazyInitKeys(size_t field, uint32_t msg_width_hint = 0);

  // x, y => [x*y] for two private inputs
  // NOTE: make sure to call InitKeys first
  MemRef MulOLE(const MemRef& inp, yacl::link::Context* conn, bool is_evaluator,
                uint32_t msg_width_hint = 0);

  // x, y => [x*y] for two private inputs
  // NOTE: make sure to call InitKeys first
  MemRef MulOLE(const MemRef& inp, bool is_evaluator,
                uint32_t msg_width_hint = 0);

  // [x], [y] => [x*y] for two shares
  // NOTE: make sure to call InitKeys first
  MemRef MulShare(const MemRef& x, const MemRef& y, yacl::link::Context* conn,
                  bool is_evaluator, uint32_t msg_width_hint = 0);

  // [x], [y] => [x*y] for two shares
  // NOTE: make sure to call InitKeys first
  MemRef MulShare(const MemRef& x, const MemRef& y, bool is_evaluator,
                  uint32_t msg_width_hint = 0);
