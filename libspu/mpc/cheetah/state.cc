...
void CheetahMulState::makeSureCacheSize(size_t field, int64_t numel) {
  ...
  MemRef rand(makeType<RingTy>(SE_INVALID, field), {num_ole * ole_sze});
  ring_rand(rand);
  ...
  if (rank == 0) {
    ...
    beaver[1] = rand.slice({num_beaver}, {num_beaver}, {1});
  } else {
    beaver[0] = rand.slice({num_beaver}, {num_beaver}, {1});
    beaver[1] = rand.slice({0}, {num_beaver}, {1});
  }
  a0b1 = cross.slice({0}, {num_beaver}, {1});
  a1b0 = cross.slice({num_beaver}, {num_beaver}, {1});

  beaver[2] = ring_add(ring_add(cross.slice({0}, {num_beaver}, {1}),
                                cross.slice({num_beaver}, {num_beaver}, {1})),
                       ring_mul(beaver[0], beaver[1]));

  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
    for (size_t i : {0, 1, 2}) {
      MemRef tmp(makeType<RingTy>(SE_INVALID, field),
                 {num_beaver + cached_sze_});
      ring_zeros(tmp);
      MemRefView<ScalarT> new_cache(tmp);

      // concate two array
      if (cached_sze_ > 0) {
        MemRefView<const ScalarT> old_cache(cached_beaver_[i]);
        pforeach(0, cached_sze_,
                 [&](int64_t j) { new_cache[j] = old_cache[j]; });
      }

      MemRefView<const ScalarT> _beaver(beaver[i]);
...
std::array<MemRef, 3> CheetahMulState::TakeCachedBeaver(size_t field,
                                                        int64_t numel) {
  SPU_ENFORCE(numel > 0);
  std::unique_lock guard(lock_);
  makeSureCacheSize(field, numel);

  std::array<MemRef, 3> ret;
  for (size_t i : {0, 1, 2}) {
    ...
    if (cached_sze_ == numel) {
      ...
      cached_beaver_[i] = MemRef(cached_beaver_[i].eltype(), {0});
    } else {
      cached_beaver_[i] =
          cached_beaver_[i].slice({numel}, {cached_sze_ - numel}, {1});
    }
  }
...
MemRef TiledDispatchOTFunc(KernelEvalContext* ctx, const MemRef& x,
                           OTUnaryFunc func) {
  ...
  std::vector<MemRef> outs(nworker);
  ...
  for (int64_t wi = 0; wi + 1 < nworker; ++wi) {
    ...
    auto slice_input = x.slice({slice_bgn}, {slice_end - slice_bgn}, {});
    futures.emplace_back(std::async(
        [&](int64_t idx, const MemRef& input) {
  ...
  auto slice_input = x.slice({slice_end}, {numel - slice_end}, {1});
  ...
  MemRef out(outs[0].eltype(), x.shape());
  int64_t offset = 0;
...
MemRef TiledDispatchOTFunc(KernelEvalContext* ctx, const MemRef& x,
                           const MemRef& y, OTBinaryFunc func) {
  const Shape& shape = x.shape();
  ...
  for (int64_t wi = 0; wi + 1 < nworker; ++wi) {
    int64_t slice_bgn = wi * workload;
    slice_end = std::min(numel, slice_bgn + workload);
    auto x_slice = x.slice({slice_bgn}, {slice_end - slice_bgn}, {1});
    auto y_slice = y.slice({slice_bgn}, {slice_end - slice_bgn}, {1});
    futures.emplace_back(std::async(
        [&](int64_t idx, const MemRef& inp0, const MemRef& inp1) {
          auto ot_instance = ctx->getState<CheetahOTState>()->get(idx);
  ...
  auto x_slice = x.slice({slice_end}, {numel - slice_end}, {});
  auto y_slice = y.slice({slice_end}, {numel - slice_end}, {});
  auto ot_instance = ctx->getState<CheetahOTState>()->get(nworker - 1);
...
MemRef TiledDispatchOTFunc(KernelEvalContext* ctx, absl::Span<const uint8_t> x,
  ...
  MemRef out(outs[0].eltype(), {numel});
  int64_t offset = 0;
...
MemRef TiledDispatchOTFunc(KernelEvalContext* ctx, const MemRef& x,
  ...
  for (int64_t wi = 0; wi + 1 < nworker; ++wi) {
    ...
    futures.emplace_back(std::async(
        [&](int64_t idx, const MemRef& inp0, absl::Span<const uint8_t> inp1) {
          auto ot_instance = ctx->getState<CheetahOTState>()->get(idx);
          outs[idx] = func(inp0, inp1, ot_instance);
        },
        wi, x_slice, y_slice));
