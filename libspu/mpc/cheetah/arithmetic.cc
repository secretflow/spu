...
// A is (M, K); B is (K, N)
// lhs is server (rank0), holding weight (x)
// rhs is client (rank1), holding activation (y)
NdArrayRef RandMaskMatMulAA::proc(KernelEvalContext* ctx, const NdArrayRef& x,
                                  const NdArrayRef& y) const {
  bool profiling = false;  // TODO: change here to run a simulated profiling.
  ...
  NdArrayRef x_complete;
  if (!profiling) {
    // reconstruct y_complete for correctness, comment when profiling.
    if (rank == 1) {
      comm->sendAsync(next_rank, x, "x_1");
    } else {
      x_complete = comm->recv(next_rank, x.eltype(), "x_1");
      x_complete = x_complete.reshape(x.shape());
      x_complete = ring_add(x, x_complete);
    }
  } else {
    x_complete = x;
  }
  ...
  NdArrayRef xy1;
  if (rank == 0) {
    xy1 = dot_prot->DotOLE(x_complete, conn, dim3, true, true, profiling);
  } else {
    xy1 = dot_prot->DotOLE(y, conn, dim3, false, true, profiling);
  }

  if (rank == 0) {
    // x*y0+(x*y1-r)
    auto ret = ring_mmul(x_complete, y);
    ring_add_(ret, xy1);
    return ret.as(x.eltype());
  } else {
    // r from x*y1
    return xy1.as(x.eltype());
  }
}
...
NdArrayRef MatMulAA::proc(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  std::future<NdArrayRef> task = std::async(std::launch::async, [&] {
    // Compute x0*y1
    if (rank == 0) {
      return dot_prot->DotOLE(x, dupx.get(), dim3, true, false, false);
    } else {
      return dot_prot->DotOLE(y, dupx.get(), dim3, false, false, false);
    }
  });

  NdArrayRef x1y0;
  if (rank == 0) {
    x1y0 = dot_prot->DotOLE(y, conn, dim3, false, false, false);
  } else {
    x1y0 = dot_prot->DotOLE(x, conn, dim3, true, false, false);
  }
