...
MemRef CompareProtocol::DoCompute(const MemRef& inp, bool greater_than,
                                  MemRef* keep_eq, int64_t bitwidth) {
  ...
  auto boolean_t = makeType<BoolShareTy>(SE_1, GetStorageType(field), 1);
  MemRef prev_cmp(boolean_t, {static_cast<int64_t>(num_digits * num_cmp)});
  ring_zeros(prev_cmp);
  MemRef prev_eq(boolean_t, {static_cast<int64_t>(num_digits * num_cmp)});
  ring_zeros(prev_eq);

  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
    MemRefView<ScalarT> xprev_cmp(prev_cmp);
    MemRefView<ScalarT> xprev_eq(prev_eq);
...
std::array<MemRef, 2> CompareProtocol::TraversalANDWithEqFullBinaryTree(
    MemRef cmp, MemRef eq, size_t num_input, size_t num_digits) {
  ...
  for (size_t i = 1; i <= num_digits; i += 1) {
    ...
    auto lhs_eq = eq.slice({0}, {(n - 1) / 2 + 1}, {2});
    auto rhs_eq = eq.slice({1}, {(n - 1) / 2 + 1}, {2});

    auto lhs_cmp = cmp.slice({0}, {(n - 1) / 2 + 1}, {2});
    auto rhs_cmp = cmp.slice({1}, {(n - 1) / 2 + 1}, {2});
...
std::array<MemRef, 2> CompareProtocol::TraversalANDWithEq(MemRef cmp, MemRef eq,
                                                          size_t num_input,
                                                          size_t num_digits) {
  ...
  MemRef current_cmp(cmp.eltype(), current_shape);
  MemRef current_eq(eq.eltype(), current_shape);
  ...
  MemRef subtree_cmp = _cmp;
  MemRef subtree_eq = _eq;
...
MemRef CompareProtocol::TraversalANDFullBinaryTree(MemRef cmp, MemRef eq,
                                                   size_t num_input,
                                                   size_t num_digits) {
  SPU_ENFORCE(num_digits > 0 && absl::has_single_bit(num_digits),
              "require num_digits be a 2-power");
  ...
  for (size_t i = 1; i <= num_digits; i += 1) {
    ...
    MemRef _lhs_eq(lhs_eq.eltype(), subshape);
    MemRef _rhs_eq(rhs_eq.eltype(), subshape);
    MemRef _lhs_cmp(lhs_cmp.eltype(), subshape);

    MemRef _lhs_cmp_col0(lhs_cmp.eltype(), {static_cast<int64_t>(nrow)});
    MemRef _rhs_eq_col0(rhs_eq.eltype(), _lhs_cmp_col0.shape());
    ...
    // Concat two ANDs
    eq = MemRef(eq.eltype(), {_next_cmp_col0.numel() + _next_cmp.numel()});
    cmp = MemRef(cmp.eltype(), eq.shape());
...
MemRef CompareProtocol::TraversalAND(MemRef cmp, MemRef eq, size_t num_input,
                                     size_t num_digits) {
  if (absl::has_single_bit(num_digits)) {
  ...
  MemRef subtree_cmp = TraversalANDFullBinaryTree(
      current_cmp, current_eq, num_input, current_num_digits);
...
MemRef CompareProtocol::Compute(const MemRef& inp, bool greater_than,
                                int64_t bitwidth) {
  int64_t bw = SizeOf(inp.eltype().storage_type()) * 8;
...
std::array<MemRef, 2> CompareProtocol::ComputeWithEq(const MemRef& inp,
                                                     bool greater_than,
                                                     int64_t bitwidth) {
  int64_t bw = SizeOf(inp.eltype().storage_type()) * 8;
  SPU_ENFORCE(bitwidth >= 0 && bitwidth <= bw, "bit_width={} out of bound",
