...
MemRef EqualProtocol::DoCompute(const MemRef& inp, size_t bit_width) {
  ...
  while (current_num_digits > 1) {
    ...
    auto rhs_eq = prev_eq.slice({lhs_eq.numel()}, {lhs_eq.numel()}, {1});
    ...
    if (remain_d > 0) {
      MemRef tmp(prev_eq.eltype(), {next_eq.numel() + remain_d * num_cmp});
...
MemRef EqualProtocol::Compute(const MemRef& inp, size_t bit_width) {
