...
MemRef TruncateProtocol::ComputeWrap(const MemRef& inp, const Meta& meta) {
    ...
    default: {
      ...
      if (rank == 0) {
        ...
        DISPATCH_ALL_STORAGE_TYPES(inp.eltype().storage_type(), [&]() {
          MemRefView<ScalarT> xadj(adjusted);
      ...
      return basic_ot_prot_->B2ASingleBitWithSize(
          wrap_bool.as(makeType<BoolShareTy>(SE_1, GetStorageType(field), 1)),
          meta.shift_bits);
      break;
    }
  }
...
MemRef TruncateProtocol::MSB1ToWrap(const MemRef& inp, size_t shift_bits) {
  const auto field = SizeOf(inp.eltype().storage_type()) * 8;
  const int64_t numel = inp.numel();
  const int rank = basic_ot_prot_->Rank();
  const size_t bw = SizeOf(field) * 8;

  MemRef cot_output(makeType<RingTy>(SE_INVALID, field), inp.shape());
  ring_zeros(cot_output);
  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
    using u2k = std::make_unsigned<ScalarT>::type;
    MemRefView<const u2k> xinp(inp);
  ...
  return cot_output.as(makeType<BoolShareTy>(SE_1, GetStorageType(field), 1));
}
...
MemRef TruncateProtocol::MSB0ToWrap(const MemRef& inp, size_t shift_bits) {
  const auto field = SizeOf(inp.eltype().storage_type()) * 8;
  const int64_t numel = inp.numel();
  const int rank = basic_ot_prot_->Rank();
  const size_t bw = SizeOf(field) * 8;
  ...
  MemRef outp(makeType<RingTy>(SE_INVALID, field), inp.shape());
  if (0 == rank) {
    ring_randbit(outp);
    ...
    DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
      using u2k = std::make_unsigned<ScalarT>::type;
      MemRefView<const u2k> xinp(inp);
      MemRefView<const u2k> xrnd(outp);
    ...
    ring_zeros(outp);
    DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
      MemRefView<ScalarT> xoup(outp);
      pforeach(0, numel,
               [&](int64_t i) { xoup[i] = static_cast<ScalarT>(recv[i] & 1); });
    });
  }

  return basic_ot_prot_->B2ASingleBitWithSize(
      outp.as(makeType<BoolShareTy>(SE_1, GetStorageType(field), 1)),
      static_cast<int>(shift_bits));
}

MemRef TruncateProtocol::Compute(const MemRef& inp, Meta meta) {
  ...
  if (meta.signed_arith && meta.sign == SignType::Unknown &&
    ...
    if (rank == 0) {
      MemRef tmp = inp.clone();
      DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&] {
        MemRefView<ScalarT> _inp(tmp);
        ScalarT big_value = static_cast<ScalarT>(1)
  ...
  MemRef wrap_ashr;
  MemRef out(makeType<RingTy>(SE_INVALID, field), inp.shape());
  ring_zeros(out);
  ...
  return DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
    ...
    MemRefView<const ring2k_t> xinp(inp);
    ...
    if (meta.signed_arith && rank == 0) {
      ...
      MemRef tmp(makeType<RingTy>(SE_INVALID, field), inp.shape());
      ring_zeros(tmp);
      MemRefView<ring2k_t> xtmp(tmp);
      pforeach(0, inp.numel(),
