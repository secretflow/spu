...
class EqualProtTest : public ::testing::TestWithParam<size_t> {
  void SetUp() override {}
};

INSTANTIATE_TEST_SUITE_P(
    Cheetah, EqualProtTest, testing::Values(32, 64, 128),
...
TEST_P(EqualProtTest, Basic) {
  ...
  DISPATCH_ALL_STORAGE_TYPES(GetStorageType(field), [&]() {
    auto xeq0 = MemRefView<ScalarT>(eq_oup[0]);
    auto xeq1 = MemRefView<ScalarT>(eq_oup[1]);
    auto xinp0 = MemRefView<ScalarT>(inp[0]);
    auto xinp1 = MemRefView<ScalarT>(inp[1]);
