...
MemRef OpenShare(const MemRef &shr, ReduceOp op, size_t nbits,
  ...
  SPU_ENFORCE(shr.eltype().isa<BaseRingType>());
  SPU_ENFORCE(op == ReduceOp::ADD or op == ReduceOp::XOR);

  auto field = SizeOf(shr.eltype().storage_type()) * 8;
  ...
  MemRef out(shr.eltype(), {static_cast<int64_t>(numel)});
  DISPATCH_ALL_STORAGE_TYPES(out.eltype().storage_type(), [&]() {
    auto inp = absl::MakeConstSpan(&shr.at<ScalarT>(0), numel);
    auto oup = absl::MakeSpan(&out.at<ScalarT>(0), compact_numel);

    size_t used = ZipArray(inp, space_bits, oup);
    SPU_ENFORCE_EQ(used, compact_numel);

    std::vector<ScalarT> opened;
    if (op == ReduceOp::XOR) {
      opened = conn->allReduce<ScalarT, std::bit_xor>(oup, "open");
    } else {
      opened = conn->allReduce<ScalarT, std::plus>(oup, "open");
    }

    oup = absl::MakeSpan(&out.at<ScalarT>(0), numel);
    UnzipArray(absl::MakeConstSpan(opened), space_bits, oup);

    if (space_bits > nbits and nbits < fwidth) {
      auto msk = (static_cast<ScalarT>(1) << nbits) - 1;
