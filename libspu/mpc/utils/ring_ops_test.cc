...
static MemRef makeRandomArray(size_t field, int64_t numel, int64_t stride) {
  const Type ty = makeType<RingTy>(SE_I64, field);
  ...
  return MemRef(buf, ty, {numel}, {stride}, 0);
}
...
TEST_P(RingArrayRefTest, Assign) {
  const size_t field = std::get<0>(GetParam());
...
TEST_P(RingArrayRefTest, RingBitMask) {
...

  {
    ...
    EXPECT_TRUE(ring_all_equal(z, 0));
  }
  {
    // GIVEN
    const auto x = makeRandomArray(field, numel, stride);
