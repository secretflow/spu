...
NdArrayRef cast_ring(const NdArrayRef& x, const Type& new_type) {
  // SPU_ENFORCE_RING(x);
  const auto field = x.eltype().as<Ring2k>()->field();
  const auto to_field = new_type.as<Ring2k>()->field();
  size_t nbits = SizeOf(field) * 8 - 1;
  size_t to_bits = SizeOf(to_field) * 8 - 1;

  auto numel = x.numel();

  return DISPATCH_ALL_FIELDS(field, kModule, [&]() {
    using from_ring2k_t = ring2k_t;
    return DISPATCH_ALL_FIELDS(to_field, kModule, [&]() {
      using to_ring2k_t = ring2k_t;
      NdArrayRef res(new_type, x.shape());

      if (nbits < to_bits) {
        pforeach(0, numel, [&](int64_t idx) {
          res.at<to_ring2k_t>(idx) =
              static_cast<to_ring2k_t>(x.at<from_ring2k_t>(idx) +
                                       (from_ring2k_t(1) << nbits)) -
              (to_ring2k_t(1) << nbits);
        });
      } else if (nbits == to_bits) {
        return x;
      } else {
        pforeach(0, numel, [&](int64_t idx) {
          res.at<to_ring2k_t>(idx) =
