...
Value make_p(SPUContext* ctx, uint128_t init, const Shape& shape,
             FieldType ftype) {
  FORCE_DISPATCH(ctx, init, shape, ftype);
}

Value rand_p(SPUContext* ctx, const Shape& shape, FieldType ftype) {
  FORCE_DISPATCH(ctx, shape, ftype);
}

Value rand_s(SPUContext* ctx, const Shape& shape, FieldType ftype) {
...
Value cast_ring_s(SPUContext* ctx, const Value& x, const FieldType& ftype) {
  FORCE_DISPATCH(ctx, x, ftype);
}

Value cast_ring_v(SPUContext* ctx, const Value& x, const FieldType& ftype) {
  FORCE_DISPATCH(ctx, x, ftype);
}

Value cast_ring_p(SPUContext* ctx, const Value& x, const FieldType& ftype) {
  FORCE_DISPATCH(ctx, x, ftype);
}
