...
MemRef OramOneHotAA::proc(KernelEvalContext *ctx, const MemRef &in,
                          int64_t s) const {
  auto *comm = ctx->getState<Communicator>();

  MemRef out(makeType<OramShareTy>(in.eltype().semantic_type(),
                                   in.eltype().storage_type()),
             {s});

  DISPATCH_ALL_STORAGE_TYPES(in.eltype().storage_type(), [&]() {
    using el_t = ScalarT;
    using shr_t = std::array<el_t, 2>;
    MemRefView<shr_t> out_(out);

    auto in_b = a2b(ctx->sctx(), in);
    MemRefView<shr_t> target_idxs_(in_b);
...
MemRef OramOneHotAP::proc(KernelEvalContext *ctx, const MemRef &in,
                          int64_t s) const {
  auto *comm = ctx->getState<Communicator>();

  auto semantic_type = in.eltype().semantic_type();
  auto storage_type = in.eltype().storage_type();

  const auto numel = in.numel();
  MemRef out(makeType<OramPubShareTy>(semantic_type, storage_type), {s});

  DISPATCH_ALL_STORAGE_TYPES(storage_type, [&]() {
    using el_t = ScalarT;
    using shr_t = std::array<el_t, 2>;
    MemRefView<el_t> out_(out);
    ...
    if (comm->getRank() == 2) {
      ...
      // 3->2
      MemRefView<shr_t> in_(in_b);
      MemRef target_point_2pc(
          makeType<RingTy>(semantic_type, SizeOf(storage_type) * 8),
          in.shape());
      MemRefView<el_t> target_point_2pc_(target_point_2pc);
      // reblind
      if (comm->getRank() == 0) {
        pforeach(0, numel,
...
MemRef OramReadOA::proc(KernelEvalContext *ctx, const MemRef &onehot,
                        const MemRef &db, int64_t offset) const {
  auto *comm = ctx->getState<Communicator>();
  auto *prg = ctx->getState<PrgState>();

  auto semantic_type = db.eltype().semantic_type();
  auto storage_type = db.eltype().storage_type();
  ...
  MemRef out(makeType<ArithShareTy>(
                 semantic_type, ctx->getState<Z2kState>()->getDefaultField()),
             {1, index_times});
  ...
  DISPATCH_ALL_STORAGE_TYPES(storage_type, [&]() {
    ...
    auto r = std::async([&] {
      Type type = makeType<RingTy>(semantic_type, field);
      Shape shape = {1, index_times};

      MemRef r0(type, shape);
      MemRef r1(type, shape);

      prg->fillPrssPair(r0.data(), r1.data(), shape.numel() * type.size());

      return ring_sub(r0, r1);
    });

    MemRefView<shr_t> onehot_(onehot);

    MemRef shifted_onehot(makeType<OramShareTy>(semantic_type, storage_type),
                          onehot.shape());
    MemRefView<shr_t> shifted_onehot_(shifted_onehot);
    if (offset != 0) {
...
MemRef OramReadOP::proc(KernelEvalContext *ctx, const MemRef &onehot,
                        const MemRef &db, int64_t offset) const {
  auto *comm = ctx->getState<Communicator>();
  auto *prg = ctx->getState<PrgState>();

  auto semantic_type = onehot.eltype().semantic_type();
  auto storage_type = onehot.eltype().storage_type();
  auto field = SizeOf(storage_type) * 8;
  ...
  DISPATCH_ALL_STORAGE_TYPES(storage_type, [&]() {
    ...
    MemRefView<shr_t> out_(out);
    MemRef out2pc(makeType<RingTy>(semantic_type, field), {1, index_times});
    MemRefView<el_t> out2pc_(out2pc);
    ...
    if (comm->getRank() == 2) {
      ...
      MemRefView<el_t> onehot_(onehot);
      MemRef shifted_onehot(
          makeType<OramPubShareTy>(semantic_type, storage_type),
          onehot.shape());
      MemRefView<el_t> shifted_onehot_(shifted_onehot);
      if (offset != 0) {
        pforeach(0, db_numel, [&](int64_t idx) {
          shifted_onehot_[idx] = onehot_[(idx - offset + db_numel) % db_numel];
        });
...
Triple<std::vector<T>> genOramBeaverPrim(KernelEvalContext *ctx, int64_t num,
  ...
  std::size_t beaver_triple_bytes = GetVectorNumBytes(beaver_triple);

  if (comm->getRank() == adjust_rank) {
    prg->fillPrssPair(nullptr, beaver_triple.data(), beaver_triple_bytes);
  } else {
    prg->fillPrssPair(beaver_triple.data(), nullptr, beaver_triple_bytes);
  }
...
void genOramBeaverHelper(KernelEvalContext *ctx, int64_t num, OpKind op) {
  ...
  prg->fillPrssPair(nullptr, beaver_triple0.data(),
                    GetVectorNumBytes(beaver_triple0));
  prg->fillPrssPair(beaver_triple1.data(), nullptr,
                    GetVectorNumBytes(beaver_triple1));
...
void OramContext<T>::onehotB2A(KernelEvalContext *ctx, DpfGenCtrl ctrl) {
  ...
  T F = 0;
  std::vector<T> r(1);
  prg->fillPriv(r.data(), GetVectorNumBytes(r));
  ...
  // open blinded_pm
  comm->sendAsync<T>(dst_rank,
                     absl::Span<T>(reinterpret_cast<T *>(&blinded_pm),
                                   sizeof(blinded_pm) / sizeof(T)),
                     "open(blinded_pm)");
  ...
  // open blinded_F
  comm->sendAsync<T>(dst_rank,
                     absl::Span<T>(reinterpret_cast<T *>(&blinded_F),
                                   sizeof(blinded_F) / sizeof(T)),
                     "open(blinded_F)");
