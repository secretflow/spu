...
MemRef A2B::proc(KernelEvalContext* ctx, const MemRef& in) const {
  ...
  const auto out_ty = makeType<BoolShareTy>(
      ring_type->semantic_type(), ring_type->storage_type(),
      SizeOf(ring_type->storage_type()) * 8);

  MemRef m(out_ty, in.shape());
  MemRef n(out_ty, in.shape());

  auto numel = in.numel();

  DISPATCH_ALL_STORAGE_TYPES(ring_type->storage_type(), [&]() {
    using ashr_t = std::array<ScalarT, 2>;
    ...
    DISPATCH_ALL_STORAGE_TYPES(out_ty.storage_type(), [&]() {
      ...
      MemRefView<bshr_t> _m(m);
      MemRefView<bshr_t> _n(n);
  ...
  return add_bb(ctx->sctx(), m, n);  // comm => log(k) + 1, 2k(logk) + k
}

MemRef B2ASelector::proc(KernelEvalContext* ctx, const MemRef& in) const {
  const auto* in_ty = in.eltype().as<BoolShareTy>();
  const size_t in_nbits = in_ty->valid_bits();
...
MemRef B2AByPPA::proc(KernelEvalContext* ctx, const MemRef& in) const {
  const auto field = ctx->getState<Z2kState>()->getDefaultField();
  const auto* in_ty = in.eltype().as<BoolShareTy>();
  const size_t in_nbits = in_ty->valid_bits();

  SPU_ENFORCE(in_nbits <= SizeOf(field) * 8, "invalid nbits={}", in_nbits);
  const auto out_ty = makeType<ArithShareTy>(in_ty->semantic_type(), field);
  ...
  DISPATCH_ALL_STORAGE_TYPES(in_ty->storage_type(), [&]() {
    ...
    DISPATCH_ALL_STORAGE_TYPES(out_ty.storage_type(), [&]() {
      ...
      const auto expanded_ty = makeType<BoolShareTy>(
          in_ty->semantic_type(), GetStorageType(field), SizeOf(field) * 8);
      MemRef x(expanded_ty, in.shape());
      MemRefView<ashr_t> _x(x);
      ...
      prg_state->fillPrssPair(ra0.data(), ra1.data(), GetVectorNumBytes(ra0));
      prg_state->fillPrssPair(rb0.data(), rb1.data(), GetVectorNumBytes(rb0));
      ...
      // compute [x+r]B
      MemRef r(expanded_ty, in.shape());
      MemRefView<ashr_t> _r(r);
      pforeach(0, numel, [&](int64_t idx) {
        _r[idx][0] = rb0[idx];
        _r[idx][1] = rb1[idx];
      });
      ...
      auto x_plus_r = add_bb(ctx->sctx(), x, r);
      MemRefView<ashr_t> _x_plus_r(x_plus_r);
...
template <typename T>
static std::vector<bool> bitDecompose(const MemRef& in, size_t nbits) {
  auto numel = in.numel();
...
MemRef B2AByOT::proc(KernelEvalContext* ctx, const MemRef& in) const {
  const auto field = ctx->getState<Z2kState>()->getDefaultField();
  const auto* in_ty = in.eltype().as<BoolShareTy>();
  const size_t in_nbits = in_ty->valid_bits();
  ...
  auto out_ty = makeType<ArithShareTy>(in_ty->semantic_type(), field);
  MemRef out(out_ty, in.shape());
  ...
  DISPATCH_ALL_STORAGE_TYPES(in_ty->storage_type(), [&]() {
    ...
    DISPATCH_ALL_STORAGE_TYPES(out_ty.storage_type(), [&]() {
      ...
      if (comm->getRank() == P0) {
        ...
        prg_state->fillPrssPair(m0.data(), nullptr, GetVectorNumBytes(m0));
        prg_state->fillPrssPair(m1.data(), nullptr, GetVectorNumBytes(m1));
        ...
        std::vector<ashr_el_t> m0(total_nbits);
        std::vector<ashr_el_t> m1(total_nbits);
        prg_state->fillPrssPair(nullptr, m0.data(), GetVectorNumBytes(m0));
        prg_state->fillPrssPair(nullptr, m1.data(), GetVectorNumBytes(m1));
        pforeach(0, total_nbits, [&](int64_t idx) {
          m0[idx] ^= r0[idx];
          m1[idx] ^= r1[idx];
        });
...
//   xAyBzCwD -> (xyzw, ABCD)
[[maybe_unused]] std::pair<MemRef, MemRef> bit_split(const MemRef& in) {
  ...
  const auto out_type = makeType<BoolShareTy>(
      in.eltype().semantic_type(), GetStorageType(out_nbits), out_nbits);

  MemRef lo(out_type, in.shape());
  MemRef hi(out_type, in.shape());
  ...
  DISPATCH_ALL_STORAGE_TYPES(in.eltype().storage_type(), [&]() {
    ...
    DISPATCH_ALL_STORAGE_TYPES(out_type.storage_type(), [&]() {
      ...
      MemRefView<out_shr_t> _lo(lo);
      MemRefView<out_shr_t> _hi(hi);
...
MemRef MsbA2B::proc(KernelEvalContext* ctx, const MemRef& in) const {
  ...
  auto storage_type = in.eltype().storage_type();
  auto filed = SizeOf(storage_type) * 8;

  auto bshr_ty =
      makeType<BoolShareTy>(in.eltype().semantic_type(), storage_type, filed);
  auto out_ty = makeType<BoolShareTy>(SE_1, ST_8, 1);

  MemRef m(bshr_ty, in.shape());
  MemRef n(bshr_ty, in.shape());

  DISPATCH_ALL_STORAGE_TYPES(storage_type, [&]() {
    using el_t = ScalarT;
    using shr_t = std::array<el_t, 2>;

    MemRefView<shr_t> _in(in);
    MemRefView<shr_t> _m(m);
    MemRefView<shr_t> _n(n);
  ...
  int64_t nbits = filed - 1;
  auto* sctx = ctx->sctx();

  {
    // 2. 2k + 16 * 2 bits
    auto carry = carry_a2b(sctx, m, n, nbits);

    // Compute the k'th bit.
    //   (m^n)[k] ^ carry
    auto msb = xor_bb(sctx, rshift_b(sctx, xor_bb(sctx, m, n), {nbits}), carry);

    // cast the semantic type to SE_1
    auto out = cast_type_b(sctx, msb, out_ty);
...
MemRef eqz(KernelEvalContext* ctx, const MemRef& in) {
  ...
  MemRef out(makeType<BoolShareTy>(SE_1, ST_8, 8), in.shape());
  ...
  DISPATCH_ALL_STORAGE_TYPES(storage_type, [&]() {
    ...
    DISPATCH_ALL_STORAGE_TYPES(storage_type, [&]() {
      ...
      if (comm->getRank() == P0) {
        ...
        prg_state->fillPriv(r.data(), GetVectorNumBytes(r));

        std::vector<ashr_el_t> r_arith_0(numel);
        prg_state->fillPrssPair(nullptr, r_arith_0.data(),
                                GetVectorNumBytes(r_arith_0));
        std::vector<bshr_el_t> r_bool_0(numel);
        prg_state->fillPrssPair(nullptr, r_bool_0.data(),
                                GetVectorNumBytes(r_bool_0));
        ...
        prg_state->fillPrssPair(nullptr, zero_flag_3pc_1.data(),
                                GetVectorNumBytes(zero_flag_3pc_1));
        ...
        if (comm->getRank() == P1) {
          ...
          prg_state->fillPrssPair(r_arith.data(), nullptr,
                                  GetVectorNumBytes(r_arith));
          prg_state->fillPrssPair(r_bool.data(), nullptr,
                                  GetVectorNumBytes(r_bool));
        } else {
        ...
        if (comm->getRank() == P1) {
          ...
          prg_state->fillPrssPair(r_z.data(), nullptr, GetVectorNumBytes(r_z));
          pforeach(0, numel, [&](int64_t idx) {
      ...
      auto cur_bytes = SizeOf(ashr_type->storage_type()) * numel;
      ...
      while (cur_bits != cur_numel) {
        ...
        prg_state->fillPrssPair(r0.data(), r1.data(), half_num_bytes);
      ...
      MemRefView<std::array<std::byte, 2>> _out(out);
...
MemRef EqualAP::proc(KernelEvalContext* ctx, const MemRef& lhs,
  ...
  auto rank = ctx->getState<Communicator>()->getRank();
...
void CommonTypeV::evaluate(KernelEvalContext* ctx) const {
  ...
  const auto* ref = lhs_v->size() > rhs_v->size() ? lhs_v : rhs_v;
  ctx->pushOutput(makeType<ArithShareTy>(ref->semantic_type(), valid_bits));
}
...
std::vector<MemRef> BitDecompose::proc(KernelEvalContext* ctx,
  ...
  DISPATCH_ALL_STORAGE_TYPES(ty->storage_type(), [&]() {
    using InElT = ScalarT;
    using InT = std::array<InElT, 2>;
    MemRefView<InT> _in(in);
    DISPATCH_ALL_STORAGE_TYPES(bty.storage_type(), [&]() {
      using OutElT = ScalarT;
      using OutT = std::array<OutElT, 2>;
      for (size_t i = 0; i < nbits; ++i) {
        MemRef bit(bty, in.shape());
        MemRefView<OutT> _bit(bit);
        pforeach(0, in.numel(), [&](int64_t idx) {
          _bit[idx][0] = static_cast<OutElT>((_in[idx][0] >> i) & 0x1);
          _bit[idx][1] = static_cast<OutElT>((_in[idx][1] >> i) & 0x1);
        });
    ...
    for (size_t i = 0; i < nbits; ++i) {
      DISPATCH_ALL_STORAGE_TYPES(in[i].eltype().storage_type(), [&]() {
        ...
        pforeach(0, in[0].numel(), [&](int64_t idx) {
          _out[idx][0] |= static_cast<OutElT>(_in[idx][0] & 0x1) << i;
          _out[idx][1] |= static_cast<OutElT>(_in[idx][1] & 0x1) << i;
        });
      });
    }
  });
