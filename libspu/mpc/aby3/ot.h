...
class Ot3 {
 ...
 protected:
  ...
  const size_t field_;  // each msg has SizeOf(field_) bits
  ...
  //
  bool reentrancy_;
  std::optional<std::pair<MemRef, MemRef>> masks_;
  std::pair<MemRef, MemRef> genMasks();
 ...
 public:
  ...
  explicit Ot3(size_t field, absl::Span<const int64_t> shape,
               const RoleRanks& roles, Communicator* comm, PrgState* prg_state,
               bool reentrancy = true);

  void send(const MemRef& m0, const MemRef& m1);

  MemRef recv(const std::vector<uint8_t>& choices, SemanticType semantic_type);
