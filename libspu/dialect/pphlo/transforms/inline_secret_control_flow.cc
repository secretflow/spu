...
class CaseConverter : public OpRewritePattern<CaseOp> {
 private:
  ...
  void inlineRegionIntoParent(CaseOp &op, PatternRewriter &rewriter) const {
    ...
    auto zero_const = rewriter.create<arith::ConstantOp>(
        op->getLoc(),
        rewriter.getZeroAttr(RankedTensorType::get({}, index_base)));
    auto num_cases_const = rewriter.create<arith::ConstantOp>(
        op->getLoc(),
    ...
    // build mask
    llvm::SmallVector<APInt> mask(num_cases);
    for (int64_t idx = 0; idx < num_cases; ++idx) {
      mask[idx] = APInt(index_base.getIntOrFloatBitWidth(), idx);
    }

    auto iota = rewriter.create<arith::ConstantOp>(
        op->getLoc(),
        DenseIntElementsAttr::get(
            RankedTensorType::get({num_cases}, index_base), mask));
