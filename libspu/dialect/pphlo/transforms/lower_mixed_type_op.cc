...
struct GenerateFpIntOp : public OpRewritePattern<OpT> {
 private:
 ...
 public:
  explicit GenerateFpIntOp(MLIRContext *context)
...
template <typename OpT>
struct GenerateFxpIntOp : public OpRewritePattern<OpT> {
 private:
  TypeTools typetools_;
  ...
  bool isLegitConvert(ConvertOp op) const {
    ...
    // Only int->fxp conversion is considered legit
    return typetools_.isFixedPointType(op.getType()) &&
           typetools_.isIntType(op.getOperand().getType());
  }

 public:
  explicit GenerateFxpIntOp(MLIRContext *context)
      : OpRewritePattern<OpT>(context), typetools_(context) {}
  ...
  LogicalResult matchAndRewrite(OpT op,
    ...
    if (!typetools_.isFixedPointType(op.getType())) {
      return failure();  // Must be an op result in fp type
    }
    ...
    if (((lhs_convert != nullptr && rhs_convert == nullptr) ||
      ...
      operands[0] = lhs_convert == nullptr ? lhs : lhs_convert.getOperand();
      operands[1] = rhs_convert == nullptr ? rhs : rhs_convert.getOperand();
      auto newop = rewriter.create<OpT>(op.getLoc(), operands, op->getAttrs());
      rewriter.replaceOpWithNewOp<ConvertOp>(op, op.getType(), newop);
      return success();
    }
    return failure();
  }
...
struct LowerMixedTypeOp : public LowerMixedTypeOpBase<LowerMixedTypeOp> {
 ...
 private:
  static void populateOwningPatterns(RewritePatternSet *patterns,
                                     MLIRContext *ctx) {
    // Float patterns
    patterns->insert<GenerateFpIntOp<MulOp>, GenerateFpIntOp<DotOp>,
                     GenerateFpIntOp<DotGeneralOp>>(ctx);

    // Fixpoint patterns
    patterns->insert<GenerateFxpIntOp<MulOp>, GenerateFxpIntOp<DotOp>>(ctx);
  }
};
}  // namespace
