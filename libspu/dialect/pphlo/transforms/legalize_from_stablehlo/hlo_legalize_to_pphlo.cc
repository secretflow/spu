...
#include "libspu/dialect/pphlo/transforms/legalize_from_stablehlo/map_stablehlo_to_pphlo_op.h"
#include "libspu/dialect/pphlo/transforms/legalize_from_stablehlo/value_visibility_map.h"
#include "libspu/dialect/pphlo/transforms/legalize_from_stablehlo/visibility_inference.h"
...
class HloToPPHloOpConverter<stablehlo::ConstantOp>
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
      ...
      ConversionPatternRewriter &rewriter) const override {
    rewriter.replaceOpWithNewOp<arith::ConstantOp>(hlo_op, hlo_op.getValue());
    return success();
  }
};
...
class HloToPPHloOpConverter<stablehlo::IotaOp>
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
      stablehlo::IotaOp op, stablehlo::IotaOpAdaptor /*adaptor*/,
      ConversionPatternRewriter &rewriter) const override {
    auto result_type = convertResultType(op.getResult());

    auto outputType = mlir::cast<ShapedType>(result_type);
    ...
    auto intOrFpType = outputType.getElementType();
    int bitwidth = intOrFpType.getIntOrFloatBitWidth();
    ...
    Value iotaConst = rewriter.create<arith::ConstantOp>(
        op.getLoc(), DenseIntElementsAttr::get(intShapeType, values));

    rewriter.replaceOpWithNewOp<pphlo::ConvertOp>(op, result_type, iotaConst);
...
class HloToPPHloOpConverter<stablehlo::CustomCallOp>
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    std::string callee = op.getCallTargetName().str();
    ...
    auto new_op = rewriter.replaceOpWithNewOp<pphlo::CustomCallOp>(
        op, result_types, materializeInputs(op, adaptor.getOperands()), callee,
        op.getHasSideEffect());
