...
constexpr bool is_memref_like_v = false;

// Detection idioms for size() and data().
template <typename T>
constexpr bool
    is_memref_like_v<T, std::void_t<decltype(std::declval<T>().data()),
...
struct PtBufferView {
  ...
  Shape const shape;             // Shape of the memref.
  ...
  // View a memref-like type (i.e. xt::xarray) as a buffer.
  template <typename T,
            std::enable_if_t<detail::is_memref_like_v<T>, bool> = true>
  ...
  yacl::Buffer copyBuffer() const {
    auto numel = shape.numel();
    if (isBitSet()) {
      SPU_ENFORCE(isCompact() && pt_type == PT_I1);
      yacl::Buffer out(shape.numel());
      auto* out_ptr = out.data<bool>();
      for (int64_t idx = 0; idx < numel; ++idx) {
        out_ptr[idx] = getBit(idx);
      }
      return out;
    }
    yacl::Buffer out(numel * SizeOf(pt_type));
    return DISPATCH_ALL_PT_TYPES(pt_type, [&]() {
      using T = ScalarT;
      if (numel > 0) {
        auto* out_ptr = out.data<T>();
        Index indices(shape.size(), 0);
        if (isCompact()) {
          std::memcpy(out_ptr, &get<T>(indices), sizeof(T) * numel);
        } else {
          do {
            *out_ptr = get<T>(indices);
            out_ptr += 1;
          } while (bumpIndices(shape, absl::MakeSpan(indices)));
        }
      }
      return out;
    });
