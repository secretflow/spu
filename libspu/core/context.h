...
class SPUContext final {
 ...
 public:
  ...
  size_t getFxpBits(FieldType ftype) const {
    auto iter = config_.fxp_fraction_bits().find(FieldType_Name(ftype));
    SPU_ENFORCE(iter != config_.fxp_fraction_bits().end(),
                "Invalid field type = {}", ftype);
    const auto fbits = iter->second;
    SPU_ENFORCE(fbits != 0);
    return fbits;
  }
