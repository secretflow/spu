...
void verify_array(const MemRef &in, const std::vector<int32_t> &expected) {
  auto numel = in.numel();
...
TEST(MemRefTest, Iterator) {
  MemRef a(std::make_shared<yacl::Buffer>(9 * sizeof(int32_t)),
           makeType<RingTy>(SE_I32, 32), {3, 3}, {3, 1}, 0);
...
TEST(MemRefTest, StridedIterator) {
  // Make 3x3 element, strides = 2x2 array
  MemRef a(std::make_shared<yacl::Buffer>(36 * sizeof(int32_t)),
           makeType<RingTy>(SE_I32, 32), {3, 3}, {2L * 6, 2}, 0);
...
TEST(MemRefTest, InsertSlice) {
  // Make 3x3 element, strides = 2x2 array
  MemRef a(std::make_shared<yacl::Buffer>(9 * sizeof(int32_t)),
           makeType<RingTy>(SE_I32, 32), {3, 3}, {3, 1}, 0);
  MemRef b(std::make_shared<yacl::Buffer>(4 * sizeof(int32_t)),
           makeType<RingTy>(SE_I32, 32), {2, 2}, {2, 1}, 0);
  // a : 0,1,2
  //     3,4,5
  //     6,7,8
  // b : 0,1
  ...
  EXPECT_THROW(a.insert_slice(b, {0, 2}, {1, 1}), ::yacl::EnforceNotMet);
}

TEST(MemRefTest, Indexing) {
  ...
  {
    // 3x2 compact
    MemRef arr(buf, makeType<RingTy>(SE_I32, 32), {3, 2}, {2, 1}, 0);
  ...
  }
  {
    ...
    // c x nc
    MemRef arr(buf, makeType<RingTy>(SE_I32, 32), {3, 2}, {4, 2}, 0);
    EXPECT_TRUE(arr.canUseFastIndexing());
...

  {
    ...
    MemRef arr(buf, makeType<RingTy>(SE_I32, 32), {3, 2}, {8, 2}, 0);
...
TEST(MemRefTest, EmptyIterator) {
  MemRef a(makeType<RingTy>(SE_I32, 32), {2, 0, 8});
  EXPECT_EQ(a.numel(), 0);
