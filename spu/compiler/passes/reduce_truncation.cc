...
private:
  ...
  bool isMulSP(MulOp op) const {
    auto lhs_vis = tools_.getTypeVisibility(op.lhs().getType());
    auto rhs_vis = tools_.getTypeVisibility(op.rhs().getType());
  ...
  getSecretAndPublicOperand(MulOp op) const {
    ...
    auto secret_operand =
        lhs_vis == Visibility::VIS_SECRET ? op.lhs() : op.rhs();
    auto public_operand =
        lhs_vis == Visibility::VIS_SECRET ? op.rhs() : op.lhs();
