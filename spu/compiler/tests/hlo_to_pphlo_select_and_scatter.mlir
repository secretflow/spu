// RUN: mlir-pphlo-opt --hlo-legalize-to-pphlo='io-visibility-json={"inputs":["VIS_SECRET"]}' --split-input-file %s | FileCheck %s

func @main(%arg0: tensor<128x5x5x32xf32>, %arg1: tensor<128x4x4x32xf32>, %arg2: tensor<f32>) -> tensor<128x5x5x32xf32>   {
  ...
  // CHECK:   }) {padding = dense<0> : tensor<4x2xi64>, window_dimensions = dense<[1, 2, 2, 1]> : tensor<4xi64>, window_strides = dense<[1, 2, 2, 1]> : tensor<4xi64>} : (tensor<128x5x5x32x!pphlo.sec<f32>>, tensor<128x4x4x32x!pphlo.pub<f32>>, tensor<!pphlo.sec<f32>>) -> tensor<128x5x5x32x!pphlo.sec<f32>>
