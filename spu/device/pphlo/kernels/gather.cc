...
IndexIterationSpace iterationSpaceForOutputBatchIndices(
    ...
    const spu::device::pphlo::kernel::GatherConfig &config) {
...
private:
  ...
  const spu::device::pphlo::kernel::GatherConfig &config_;
...
spu::hal::Value reshapedGatherIndices(spu::HalContext *ctx,
                                      int64_t index_vector_dim,
                                      const spu::hal::Value &start_indices) {
  ...
  return spu::hal::reshape(ctx, start_indices, new_shape);
}
...
hal::Value Gather(HalContext *ctx, const hal::Value &operand,
                  const hal::Value &start_indicies, const GatherConfig &config,
  ...
  hal::Value result(NdArrayRef(operand.data().eltype(), result_shape),
                    operand.dtype());
