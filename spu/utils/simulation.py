...
class Simulator(object):
    def __init__(self, wsize: int, rt_config: spu_pb2.RuntimeConfig):
        ...
        spu_fe.propogate_global_copts(rt_config)

    @classmethod
    def simple(cls, wsize: int, prot: spu_pb2.ProtocolKind, field: int):
        ...
        Returns:
            A SPU Simulator
        """
        proto_config = spu_pb2.ProtocolConfig(kind=prot, field=field)
        config = spu_pb2.RuntimeConfig(protocol=proto_config)

        spu_fe.propogate_global_copts(config)
...
def sim_jax(
    ...
    *,
    static_argnums=(),
    copts=spu_fe.getDefaultCompileOptions(),
    emit_string=False,
):
    """
    Decorates a jax numpy fn that simulated on SPU.

        >>> sim = Simulator.simple(3, spu_pb2.ProtocolKind.ABY3, 64)
    ...
    if emit_string:
        copts.disable_emit_bytecode = True

    spu_fe.setDefaultCompileOptions(copts)
    spu_fe.propogate_global_copts(sim.rt_config)
