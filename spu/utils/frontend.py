...
default_compile_options = spu_pb2.CompilerOptions()


def getDefaultCompileOptions():
    global default_compile_options
    return default_compile_options


def setDefaultCompileOptions(copts):
    global default_compile_options
    default_compile_options = copts


def propogate_global_copts(rt_config: spu_pb2.RuntimeConfig):
    copts = getDefaultCompileOptions()

    # FIXME: use runtime ring_fxp_bits and width
    if rt_config.protocol.field == 128:
        copts.ring_width = 128
        copts.ring_fxp_bits = 26
    if rt_config.protocol.field == 64:
        copts.ring_width = 64
        copts.ring_fxp_bits = 18

    if rt_config.fxp_fraction_bits != 0:
        copts.ring_fxp_bits = rt_config.fxp_fraction_bits
...
def compile(
    ...
    if kind == Kind.JAX:
        ...
        with _jax_lock:
            ...
            output_dtypes = [o.dtype.name for o in output_flat]
        ...
        output_dtypes = [o.dtype.name for o in cf.outputs]
    else:
...
def torch_compile(
    ...
    output_dtypes = [o.dtype for o in method.meta.output_signature]
