...
static spu::MemRef ValueFromPyBindShare(const PyBindShare& py_share) {
  ...
  return MemRef::fromProto(value);
}

static PyBindShare ValueToPyBindShare(const spu::MemRef& value, PtType pt_type,
  ...
  ret.pt_type = PtTypeToPyFormat(pt_type);
 ...
 public:
  ...
  void SetVar(const std::string& name, const PyBindShare& share) {
    env_.setVar(name, ValueFromPyBindShare(share),
                PyFormatToPtType(share.pt_type));
  }

  PyBindShare GetVar(const std::string& name) const {
    const auto& [v, pt_type] = env_.getVar(name);
    return ValueToPyBindShare(v, pt_type, max_chunk_size_);
  }

  size_t GetVarChunksCount(const std::string& name) {
    return env_.getVar(name).first.chunksCount(max_chunk_size_);
  }

  py::bytes GetVarMeta(const std::string& name) const {
    return env_.getVar(name).first.toMetaProto().SerializeAsString();
  }
 ...
 public:
  ...
  std::vector<PyBindShare> MakeShares(const py::array& arr, int visibility,
    ...
    for (const auto& share : shares) {
      serialized.emplace_back(
          ValueToPyBindShare(share, pt_type, max_chunk_size_));
    }
  ...
  py::array Reconstruct(const std::vector<PyBindShare>& vals) {
    ...
    for (size_t idx = 1; idx < shares.size(); ++idx) {
      ...
      SPU_ENFORCE(cur.eltype() == prev.eltype(), "storage type mismatch, {} {}",
                  cur.eltype(), prev.eltype());
    }

    const PtType pt_type = PyFormatToPtType(vals.front().pt_type);
    std::vector<size_t> shape = {shares.front().shape().begin(),
                                 shares.front().shape().end()};

    py::array ret(py::dtype(vals.front().pt_type), shape);
...
PYBIND11_MODULE(libspu, m) {
  ...
  py::class_<PyBindShare>(m, "Share", "Share in python runtime")
      ...
      .def_readwrite("dtype", &PyBindShare::pt_type, "share dtype")
      .def(py::pickle(
          [](const PyBindShare& s) {  // dump
            return py::make_tuple(s.meta, s.share_chunks, s.pt_type);
          },
