...
Value concatenate(HalContext* ctx, absl::Span<const Value> values,
  ...
  for (size_t idx = 0; idx < values.size(); ++idx) {
    yasl::parallel_for(
        0, values[idx].numel(), 2048, [&](int64_t begin, int64_t end) {
          std::vector<int64_t> from_indicies(result_shape.size(), 0);
          unflattenIndex(begin, values[idx].shape(), from_indicies);
          ...
          for (int64_t e_idx = begin; e_idx < end; ++e_idx) {
            ...
            result.copyElementFrom(values[idx], from_indicies, to_indicies,
                                   elsize);
