...
class LowerComplexPassTest(unittest.TestCase):
    ...
    def test_complex_abs(self):
        """Test complex absolute value operation is properly lowered."""
        def complex_abs_fn(x):
            return jnp.abs(x)

        x = np.array([3.0 + 4.0j, 5.0 + 12.0j], dtype=np.complex64)
        ...
        self.assertEqual(executable.name, "complex_abs_fn")
        
        ir_code = executable.code.decode()
        print("\n=== Complex Abs IR ===")
        print(ir_code)
        
        # abs of complex returns float
        self.assertEqual(output.shape, (2,))
