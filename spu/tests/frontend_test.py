...
class UnitTests(unittest.TestCase):
    def test_jax_compile_static_args(self):
        ...
        # self.assertTrue(
        #     "  func.func @main(%arg0: tensor<2xi32>, %arg1: tensor<2xi32>) -> tensor<2x!pphlo.fxp<64, 18>> {\n"
        #     "    %0 = pphlo.constant() {value = dense<3.000000e+00> : tensor<2xf32>} : () -> tensor<2x!pphlo.fxp<64, 18>>\n"
        #     "    %1 = pphlo.convert %arg0 : (tensor<2xi32>) -> tensor<2x!pphlo.fxp<64, 18>>\n"
        #     "    %2 = pphlo.add %1, %0 : tensor<2x!pphlo.fxp<64, 18>>\n"
        #     "    %3 = pphlo.convert %arg1 : (tensor<2xi32>) -> tensor<2x!pphlo.fxp<64, 18>>\n"
        #     "    %4 = pphlo.add %2, %3 : tensor<2x!pphlo.fxp<64, 18>>\n"
        #     "    return %4 : tensor<2x!pphlo.fxp<64, 18>>\n" in executable.code.decode()
        # )
