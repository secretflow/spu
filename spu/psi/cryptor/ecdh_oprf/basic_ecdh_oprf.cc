...
std::string BasicEcdhOprfServer::FullEvaluate(absl::string_view input) const {
  std::string point_bytes =
      ItemMul(absl::string_view((const char *)&private_key_[0], kEccKeySize),
              input, ec_group_nid_);

  return HashItem(input, point_bytes, GetCompareLength(), hash_type_);
}
...
std::string FourQBasicEcdhOprfServer::FullEvaluate(
  ...
  return HashItem(input, pt_mul_bytes, GetCompareLength(), hash_type_);
}
