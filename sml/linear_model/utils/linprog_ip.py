...
def _get_delta(
    ...
    # [4] Equation 8.29
    u, v = _sym_solve(
        Dinv, A, rhatd - (1 / x) * rhatxs, rhatp, M, lstsq, sym_pos, cholesky
    )
...
def perform_iteration(
    ...
    status = jnp.where(
        jnp.logical_or(inf1, inf2), jnp.where(b.transpose().dot(y) > tol, 2, 3), status
    )
...
# @jit
def _ip_hsd(A, b, c, c0, alpha0, beta, maxiter, tol, pc, ip, lstsq, sym_pos, cholesky):
...
def _linprog_ip(
    ...
    cholesky = jnp.logical_or(
        ...
        jnp.logical_and(
            cholesky is None,
            jnp.logical_and(
                sym_pos, jnp.logical_not(lstsq)
            ),  # 且 sym_pos 为 True, lstsq 为 False
        ),
    )
    # cholesky = cholesky or (cholesky is None and sym_pos and not lstsq)
    x, status, iteration = _ip_hsd(
        A, b, c, c0, alpha0, beta, maxiter, tol, pc, ip, lstsq, sym_pos, cholesky
    )
    print(x)
