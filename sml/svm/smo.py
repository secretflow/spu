...
class SMO():
    ...
    def working_set_select_i(self, alpha, y, neg_y_grad):
        ...
        alpha_lower_C, alpha_upper_0, y_lower_0, y_upper_0 = jnp.array(
            [self.Cs, alpha, self.zeros, y]) > jnp.array([alpha, self.zeros, y, self.zeros])
    ...
    def update(self, i, j, Q, y, alpha, neg_y_grad):
        ...
        delta = (- yi_mul_yj * neg_y_grad[i]
                 * yi + neg_y_grad[j] * yj) / quad_coef
        diff_sum = alpha_i + yi_mul_yj*alpha_j
        alpha_i = alpha_i + (-1*yi_mul_yj*delta)
        alpha_j = alpha_j + delta

        # first cal
        diff_sum_upper_0, diff_sum_upper_C, alpha_i_lower_0, alpha_j_lower_0, alpha_i_upper_C = jnp.array(
            [diff_sum, diff_sum, 0, 0, alpha_i]) > jnp.array([0, self.C, alpha_i, alpha_j, self.C])
        outer = jnp.array([yi_neq_yj, yi_neq_yj, 1-yi_neq_yj, 1-yi_neq_yj]) * jnp.array(
            [diff_sum_upper_0, 1-diff_sum_upper_0, diff_sum_upper_C, 1-diff_sum_upper_C])
        ...
        inner = (update_from + update_condition *
                 (update_to-update_from)).reshape(2, -1)
        alpha_i, alpha_j = jnp.dot(inner, outer.T)

        # second cal
        alpha_i_lower_0, alpha_i_upper_C, alpha_j_upper_C = jnp.array(
            [0, alpha_i, alpha_j]) > jnp.array([alpha_i, self.C, self.C])
        update_condition = jnp.array(
            [alpha_i_upper_C, alpha_j_upper_C, alpha_j_upper_C, alpha_i_lower_0]*2)
        update_from = jnp.array(
            [alpha_i, alpha_i, alpha_i, alpha_i, alpha_j, alpha_j, alpha_j, alpha_j])
        update_to = jnp.array([self.C, self.C+diff_sum, diff_sum -
                              self.C, 0, self.C-diff_sum, self.C, self.C, diff_sum])
        inner = (update_from + update_condition *
                 (update_to-update_from)).reshape(2, -1)
        alpha_i, alpha_j = jnp.dot(inner, outer.T)
