...
def _ovr_decision_function(predictions, confidences, n_classes):
    ...
    for i in range(n_classes):
        for j in range(i + 1, n_classes):
            sum_of_confidences = sum_of_confidences.at[i].set(sum_of_confidences[i] - confidences[k])
            sum_of_confidences = sum_of_confidences.at[j].set(sum_of_confidences[j] + confidences[k])
            votes = votes.at[i].set(jnp.where(predictions[k] == 0, votes[i]+1, votes[i]))
            votes = votes.at[j].set(jnp.where(predictions[k] == 1, votes[j]+1, votes[j]))
            k += 1
...
class OneVsOneClassifier():
    ...
    def fit(self, X0, y):
        X = jnp.array(X0)
        self.estimators_ = [_fit_ovo_binary(self.estimator, X, y, i, j) for i in range(len(self.classes_)) for j in range(i + 1, len(self.classes_))]
...
class OneVsRestClassifier():
    ...
    def fit(self, X, y):
        self.estimators_ = [_fit_ovr_binary(self.estimator, X, y, i) for i in range(len(self.classes_))]
