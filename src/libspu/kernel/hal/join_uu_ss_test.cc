...
TEST_P(JoinTest, Join_uu_ss_work) {
  ...
  mpc::utils::simulate(
      2, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
        ...
        auto ret = join_uu_ss(&sctx, table1_columns, table2_columns,
...
TEST_P(MultiKeyJoinTest, Join_uu_ss_work) {
  FieldType field = std::get<0>(GetParam());
  ProtocolKind prot = std::get<1>(GetParam());
...
TEST(BigDataJoinTest, Join_uu_ss_work) {
