...
TEST(OddEvenMergeTest, WithValidBitsLargeScale) {
  ...
  const int input1_size = 524288;  // 128K
  const int input2_size = 524288;  // 128K
      ...
      npc, [&](const std::shared_ptr<yacl::link::Context> &lctx) {
        ...
        if (lctx->Rank() == 0) {
          ...
          std::cout << "Communication: " << comm_bytes << " bytes ("
                    << (double)comm_bytes / 1024.0 / 1024.0 << " MB)" << std::endl;
          std::cout << "Actions: " << comm_actions << std::endl;
          std::cout << "Avg bytes/element: "
                    << (double)comm_bytes / (num_rows * total_size) << std::endl;
          std::cout << "========================================\n" << std::endl;
        }
        ...
        for (int row = 0; row < num_rows; ++row) {
          ...
          double relative_error = std::abs(actual_sum - expected_sum) / expected_sum;
          EXPECT_LT(relative_error, 0.001)  // 允许 0.1% 的相对误差
              << "Row " << row << " values sum mismatch! Expected: "
              << expected_sum << ", Got: " << actual_sum
