...
Type AlkaidIo::getShareType(Visibility vis, int owner_rank) const {
  if (vis == VIS_PUBLIC) {
...
std::vector<NdArrayRef> AlkaidIo::toShares(const NdArrayRef& raw, Visibility vis,
...
size_t AlkaidIo::getBitSecretShareSize(size_t numel) const {
  const auto type = makeType<BShrTy>(PT_U8, 1);
  return numel * type.size();
}

std::vector<NdArrayRef> AlkaidIo::makeBitSecret(const PtBufferView& in) const {
...
NdArrayRef AlkaidIo::fromShares(const std::vector<NdArrayRef>& shares) const {
...
std::unique_ptr<AlkaidIo> makeAlkaidIo(FieldType field, size_t npc) {
  SPU_ENFORCE(npc == 3U, "alkaid is only for 3pc.");
  registerTypes();
  return std::make_unique<AlkaidIo>(field, npc);
}
