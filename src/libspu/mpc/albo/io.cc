...
Type AlboIo::getShareType(Visibility vis, int owner_rank) const {
  if (vis == VIS_PUBLIC) {
...
std::vector<NdArrayRef> AlboIo::toShares(const NdArrayRef& raw, Visibility vis,
  ...
  if (vis == VIS_PUBLIC) {
    ...
    if (owner_rank >= 0 && owner_rank <= 2) {
      ...
      // TODO: not safe.
      auto zero1 = ring_zeros(field, raw.shape());
      auto zero2 = ring_zeros(field, raw.shape());

      std::vector<NdArrayRef> shares;
      for (std::size_t i = 0; i < 3; i++) {
        shares.push_back(makeAShare(raw, zero1, zero2, field));
      }
      return shares;
    }
  }
...
std::vector<NdArrayRef> AlboIo::makeBitSecret(const PtBufferView& in) const {
  ...
  for (size_t idx = 0; idx < numel; idx++) {
    ...
    _s0[idx][0] = 0;
    _s0[idx][1] = r0[idx] & 0x1;
    _s0[idx][2] = r1[idx] & 0x1;

    _s1[idx][0] = 0;
    _s1[idx][1] = r1[idx] & 0x1;
    _s1[idx][2] = r2 & 0x1;
...
NdArrayRef AlboIo::fromShares(const std::vector<NdArrayRef>& shares) const {
...
std::unique_ptr<AlboIo> makeAlboIo(FieldType field, size_t npc) {
  SPU_ENFORCE(npc == 3U, "albo is only for 3pc.");
  registerTypes();
  return std::make_unique<AlboIo>(field, npc);
}
