...
TEST(AShrTy, Simple) {
  registerTypes();
  {
    ...
    EXPECT_EQ(ty.toString(), "albo.AShr<FM32>");
...
TEST(BShrTy, Simple) {
  ...
  // albo::BShr constructor with field and nbits.
  {
    ...
    EXPECT_EQ(ty.toString(), "albo.BShr<PT_U8,7>");
    ...
    EXPECT_EQ(cty.toString(), "albo.BShr<PT_U8,7>");
  }
}
