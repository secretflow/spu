...
template <typename ShareT, size_t num_shares>
NdArrayRef bit_interleave(const NdArrayRef& in) {
  constexpr std::array<uint128_t, 6> kSwapMasks = {{
      yacl::MakeUint128(0x2222222222222222, 0x2222222222222222),  // 4bit
      yacl::MakeUint128(0x0C0C0C0C0C0C0C0C, 0x0C0C0C0C0C0C0C0C),  // 8bit
      yacl::MakeUint128(0x00F000F000F000F0, 0x00F000F000F000F0),  // 16bit
  ...
  DISPATCH_UINT_PT_TYPES(in_ty->getBacktype(), [&]() {
    ...
    pforeach(0, in.numel(), [&](int64_t idx) {
      ...
      for (int k = Log2Ceil(in_nbits) - 2; k >= 0; k--) {
        ...
        int shift = 1ull << k;
        for (size_t i = 1; i < num_shares; i++)
          _out[idx][i] = (_out[idx][i] & keep) ^
                         ((_out[idx][i] >> shift) & move) ^
                         ((_out[idx][i] & move) << shift);
      }
    });
  });
  return out;
...
NdArrayRef lshift_fixed_bitwidth(const NdArrayRef& in, size_t shift) {
  ...
  const size_t out_nbits =
      std::min(in_nbits + shift, SizeOf(in_ty->getBacktype()));
...
template <typename ShareT, size_t num_shares>
std::pair<NdArrayRef, NdArrayRef> unpack_2_bitvec(const NdArrayRef& in,
                                                  size_t lo_nbits = 0) {
  const auto* in_ty = in.eltype().as<ShareT>();
...
/**
 * Pack the input into a specific container, apply the function, and unpack the
 * result. This is used to compress multiple items with non-standard bitwidth,
 * i.e. 48 bits, into one target_nbits element.
 */
template <typename ShareT, size_t num_shares, typename OutShareT,
          size_t out_num_shares, typename UnaryOp>
NdArrayRef bitwise_vmap(const NdArrayRef& in, size_t target_nbits, UnaryOp func,
                        size_t in_nbits = 0) {
  const auto* in_ty = in.eltype().as<ShareT>();
  ...
  size_t compress_numel =
      numel / compress_ratio + ((numel % compress_ratio) > 0);
  Shape compress_shape = {1, static_cast<int64_t>(compress_numel)};
  NdArrayRef compress(makeType<ShareT>(compress_btype, compress_nbits),
                      compress_shape);
  NdArrayRef out(makeType<OutShareT>(in_ty->getBacktype(), in_nbits),
                 in.shape());
    ...
    return DISPATCH_UINT_PT_TYPES(compress_btype, [&]() {
      ...
      pforeach(0, compress_numel, [&](int64_t idx) {
        ...
        for (size_t s = 0; s < num_shares; s++) {
          ...
          _cc = 0;  // NdArrayRef's buffer is not empty. We should clear it
                    // manually.
          compress_el_t _c;
          for (size_t cr = 0; cr < compress_ratio; cr++) {
            if (loc + cr < static_cast<size_t>(numel))
              _c = _in[loc + cr][s] & in_mask;
            else
              _c = 0;
...
/**
 * Serialize the input into a byte array, apply the function, and deserialize
 * the result. This is used to compress single item with non-standard bitwidth,
 * i.e. an element with 48 bits.
 */
template <typename ShareT, size_t num_shares, typename OutShareT,
          size_t out_num_shares, typename UnaryOp>
NdArrayRef bitwise_vmap_by_byte(const NdArrayRef& in, UnaryOp func,
                                size_t in_nbits = 0) {
  const auto* in_ty = in.eltype().as<ShareT>();
  in_nbits = in_nbits == 0 ? in_ty->nbits() : in_nbits;
...
void AddRounds(KernelEvalContext* ctx, size_t rounds, bool reduce_spu = true,
               bool reduce_yacl = true);
void SubRounds(KernelEvalContext* ctx, size_t rounds, bool reduce_spu = true,
               bool reduce_yacl = true);
