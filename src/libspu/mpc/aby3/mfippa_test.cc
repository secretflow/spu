...
int main() {
  ...
  utils::simulate(3, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    auto x_mss = aby3::ResharingRss2Mrss(&kectx, MyUnwrapValue(x_s));
    auto y_mss = aby3::ResharingRss2Mrss(&kectx, MyUnwrapValue(y_s));
    ...
    // and test
    {
      ...
      if (lctx.get()->Rank() == 0)
        std::cout << "sharing sent: " << GetComm.comm - comm << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "sharing latency: " << GetComm.latency - latency
                  << std::endl;
      auto z_p = b2p(sctx.get(), z_s);
      auto zours_ass =
          aby3::RssAnd2NoComm(&kectx, MyUnwrapValue(x_s), MyUnwrapValue(y_s));
      auto zours_rss = aby3::ResharingAss2Rss(&kectx, zours_ass);
      auto zours_p = b2p(sctx.get(), MyWrapValue(zours_rss));

      auto xy_rss = aby3::MrssAnd2NoComm(&kectx, x_mss, y_mss);
      auto xy_p = b2p(sctx.get(), MyWrapValue(xy_rss));

      if (lctx.get()->Rank() == 0) printResult(z_p.data(), "x & y, spu");
      if (lctx.get()->Rank() == 0)
        printResult(zours_p.data(), "x & y, RssAnd2NoComm");
      if (lctx.get()->Rank() == 0)
        printResult(xy_p.data(), "x & y, MrssAnd2Comm");

      if (lctx.get()->Rank() == 0)
        checkOutput(z_p.data(), zours_p.data(), "and2-rss-based");
      if (lctx.get()->Rank() == 0)
        checkOutput(z_p.data(), xy_p.data(), "and2-mss-based");
    }
    ...
    //   auto a_mss = aby3::ResharingRss2Mrss(&kectx, MyUnwrapValue(a_s));
    //   auto b_mss = aby3::ResharingRss2Mrss(&kectx, MyUnwrapValue(b_s));
    //   auto res_ass = aby3::MrssAnd4NoComm(&kectx, x_mss, y_mss, a_mss,
    //   b_mss); auto res_rss = aby3::ResharingAss2Rss(&kectx, res_ass); auto
    //   res_ours_p = b2p(sctx.get(), MyWrapValue(res_rss));
    //   // auto xy_rss = aby3::MrssAnd2NoComm(&kectx, x_mss, y_mss);
    //   // auto xy_rss_p = OpenRef(xy_rss);
    //   // auto a_rss = aby3::ResharingMrss2Rss(&kectx, a_mss);
    ...
    //   if (lctx.get()->Rank() == 0) printResult(res_ours_p.data(), "and4,
    //   ours");
    //   // if (lctx.get()->Rank() == 0) printResult(xy_rss_p.data(), "and4,
    //   step1 ");
    //   // if (lctx.get()->Rank() == 0) printResult(a_rss_p.data(), "and4,
    //   step2 ");
    //   // if (lctx.get()->Rank() == 0) printResult(xya_rss_p.data(), "and4,
    //   step3 ");

    //   if (lctx.get()->Rank() == 0) checkOutput(res_p.data(),
    //   res_ours_p.data(), "and4");
    // }
    ...
    //   auto z_s = and_bb(sctx.get(), x_s, y_s);
    //   auto z_p = b2p(sctx.get(), z_s);
    //   auto zours_ass = aby3::RssAnd2NoComm(&kectx, MyUnwrapValue(x_s),
    //   MyUnwrapValue(y_s)); auto zours_rss_2 = aby3::ResharingAss2Rss(&kectx,
    //   zours_ass); auto zours_mss_2 = aby3::ResharingRss2Mrss(&kectx,
    //   zours_rss_2); auto zours_rss_3 = aby3::ResharingMrss2Rss(&kectx,
    //   zours_mss_2); auto zours_ass_3 = aby3::ResharingRss2Ass(&kectx,
    //   zours_rss_3); auto zours_mss_3 = aby3::ResharingAss2Mrss(&kectx,
    //   zours_ass_3); auto zours_rss_4 = aby3::ResharingMrss2Rss(&kectx,
    //   zours_mss_3); auto zours_p2 = b2p(sctx.get(),
    //   MyWrapValue(zours_rss_4));

    //   if (lctx.get()->Rank() == 0) printResult(zours_p2.data(), "resharing");
    //   if (lctx.get()->Rank() == 0) checkOutput(z_p.data(), zours_p2.data(),
    //   "resharing");
    // }
    ...
    // msb test
    {
      ...
      if (lctx.get()->Rank() == 0)
        std::cout << "msb sent: " << GetComm.comm - comm << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "msb latency: " << GetComm.latency - latency << std::endl;
      ...
      if (lctx.get()->Rank() == 0)
        std::cout << "msb match count rate: "
                  << static_cast<double>(match_count) / (N * M) << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "msb positive count rate: "
                  << static_cast<double>(positive_count) / (N * M) << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "ours msb positive count rate: "
                  << static_cast<double>(ours_positive_count) / (N * M)
                  << std::endl;
    }
    ...
    // A2B test.
    {
      ...
      if (lctx.get()->Rank() == 0)
        checkOutput(x_r_spu.data(), x_r_ours.data(), "a2b");
    }
  });
}