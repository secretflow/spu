...
class CuckooHashToPermV : public CuckooHashToPermKernel {
 public:
  ...
  NdArrayRef proc(KernelEvalContext* ctx, const NdArrayRef& e_1,
                  const NdArrayRef& e_2, size_t num_hash, size_t scale_factor,
    ...
    // Define the first num_hash rows of out as the permutation pi_0, generated
    // by P0 based on num_hash hash functions, with each permutation of size
    // n_1. The last row is the permutation pi_1, generated by P1 based on
    // cuckoo hash, with a size of num_perm_1.
    const uint64_t n = std::max(n_1, num_perm_1);
    NdArrayRef out(
        makeType<Pub2kTy>(FieldType::FM64),
        Shape{static_cast<int64_t>(num_hash + 1), static_cast<int64_t>(n)});
    NdArrayView<uint64_t> _out(out);
    for (size_t i = 0; i < (num_hash + 1) * n; ++i) {
      _out[i] = 0;
    }
    ...
    if (isOwner(ctx, e_1.eltype())) {
      ...
      for (int64_t j = 0; j < n_1; ++j) {
        ...
        for (size_t i = 0; i < num_hash; ++i) {
          // pi_0[i][j] = e_1_hash.GetHash(i) % num_perm_1;
          _out[(i * n) + j] = e_1_hash.GetHash(i) % num_perm_1;
        }
      }
      ...
      // The definition of permutation pi_1 satisfies pi_1(j)=i, where
      // e_2[i]=t[j]
      int64_t tmp = n_2;
      for (int64_t i = 0; i < num_perm_1; ++i) {
        const auto& bin = cuckoo_index.bins()[i];
        if (!bin.IsEmpty()) {
          // pi_1[i] = bin.InputIdx();
          _out[((num_hash)*n) + i] = bin.InputIdx();
        } else {
          // pi_1[i] = tmp;
          _out[((num_hash)*n) + i] = tmp;
          tmp = tmp + 1;
        }
      }
    }
