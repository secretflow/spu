...
// static_cast<uint128_t>(16813041318660192066ull), kShape);
     ...
     */
    {
...
#ifdef ALKAID_USE_BITWIDTH_16
        auto msbx_s_ref =
            aby3np::MsbA2BForBitwidth16(&kectx_aby3, MyUnwrapValue(x_ari_aby3));
      ...
      // MyUnwrapValue(msb_a2b(sctx_aby3.get(), x_ari));

      spu::NdArrayRef msbx_ours_s;
      for (int i = 0; i < EXP_TIMES; i++) {
#ifdef ALKAID_USE_BITWIDTH_16
        msbx_ours_s = oursnp::MsbA2BMultiFanInForBitwidth16(
            &kectx_ours, MyUnwrapValue(x_ari_ours));
#else
...
#ifdef ALKAID_USE_BITWIDTH_16
        auto x_a_ref =
            aby3np::B2AForBitwidth16(&kectx_aby3, MyUnwrapValue(x_sb_aby3));
...
#ifdef ALKAID_USE_BITWIDTH_16
        auto x_b_ref =
            aby3np::A2BForBitwidth16(&kectx_aby3, MyUnwrapValue(x_ari_aby3));
     ...
     */
    {
      ...
      auto duration_spu = std::chrono::duration_cast<std::chrono::microseconds>(
          b2a_spu - b2a_start);
      if (lctx.get()->Rank() == 0)
        std::cout << "------------------------ PPA, spu" << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "ppa micro seconds: " << duration_spu.count() << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "ppa comm: " << GetComm(aby3).comm - comm << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "ppa latency: " << GetComm(aby3).latency - latency
                  << std::endl;
...
#ifdef ALKAID_USE_BITWIDTH_16
        x_a_ours =
            oursnp::PPATestForBitwidth16(&kectx_ours, MyUnwrapValue(x_sb_ours));
#else
      ...
      if (lctx.get()->Rank() == 0) OfflineRecorder::StopRecorder();
      if (lctx.get()->Rank() == 0)
        std::cout << "------------------------ PPA, ours" << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "ppa micro seconds: " << duration_ours.count()
                  << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "ppa comm: " << GetComm(ours).comm - comm << std::endl;
      if (lctx.get()->Rank() == 0)
