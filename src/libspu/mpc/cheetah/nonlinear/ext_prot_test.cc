...
std::tuple<NdArrayRef, NdArrayRef, NdArrayRef> makeInput(size_t bits,
                                                         SignType sign,
                                                         FieldType field,
                                                         const Shape& shape) {
...
TEST_P(RingExtendProtocolTest, Basic) {
  ...
  inp[0].set_fxp_bits(SizeOf(src_field) * 8);
  inp[1].set_fxp_bits(SizeOf(src_field) * 8);
...
class RingExtendGeneralProtocolTest
...
INSTANTIATE_TEST_SUITE_P(
    Cheetah, RingExtendGeneralProtocolTest,
    testing::Combine(testing::ValuesIn(bw_values),   // (bw_x, bw_out)
                     testing::Values(false, true),   // signed_arith
                     testing::ValuesIn(sign_values)  // sign
                     ));

TEST_P(RingExtendGeneralProtocolTest, Work) {
  size_t kWorldSize = 2;
  Shape shape = {kBenchSize};
  const auto numel = shape.numel();
  ...
  NdArrayRef x;
  NdArrayRef xshr[2];
  std::tie(x, xshr[0], xshr[1]) = makeInput(m, sign, src_field, shape);
  x.set_fxp_bits(m);
  ...
  utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
    ...
    out[rank] = prot.Compute(xshr[rank], meta);
  ...
  const auto field_0 = out[0].eltype().as<Ring2k>()->field();
  const auto field_1 = out[1].eltype().as<Ring2k>()->field();
  EXPECT_EQ(field_0, dst_field);
  EXPECT_EQ(field_1, dst_field);

  EXPECT_EQ(out[0].fxp_bits(), n);
  EXPECT_EQ(out[1].fxp_bits(), n);

  auto expected = ring_add(xshr[0], xshr[1]);
  auto got = ring_add(out[0], out[1]);
  MaskItInplace(expected, m);
  MaskItInplace(got, n);
  ...
  DISPATCH_ALL_FIELDS(src_field, [&]() {
    ...
    DISPATCH_ALL_FIELDS(dst_field, [&]() {
      ...
      if (signed_arith) {
        for (int64_t i = 0; i < shape.numel(); ++i) {
          EXPECT_EQ(ToSignType(expS[i], m), ToSignType(gotS[i], n));
        }
      } else {
        for (int64_t i = 0; i < shape.numel(); ++i) {
          ASSERT_EQ(expU[i], gotU[i]);
