...
void SIMDBatchMMProt::PopulateMatrixRepsIndexMap(
  ...
  for (size_t i = 0; i < row_size; i++) {
    ...
    matrix_reps_index_map[row_size | i] =
        seal::util::reverse_bits(index2, logn);
    // Next primitive root
    pos *= gen;
    pos &= (m - 1);
  }
...
// SIMDMulProt::EncodeSingle + matrix position arrangement from
// seal::BatchEncoder
void SIMDBatchMMProt::EncodeSingle(absl::Span<const uint64_t> array,
                                   RLWEPt& out) const {
...
// SIMDMulProt::DecodeSingle + matrix position arrangement from
// seal::BatchEncoder
void SIMDBatchMMProt::DecodeSingle(const RLWEPt& poly,
...
void SIMDBatchMMProt::SymEncrypt(absl::Span<const RLWEPt> polys,
                                 ...
                                 bool save_seed, absl::Span<RLWECt> out) const {
...
Shape2D SIMDBatchMMProt::ComputeInShape(const Meta& meta) {
  ...
  Shape2D in_shape = {static_cast<int64_t>(eff_row_size),
                      static_cast<int64_t>(block_size)};
  return in_shape;
}

size_t SIMDBatchMMProt::ComputeInputCtNum(const Meta& meta,
                                          Shape2D in_shape) const {
...
size_t SIMDBatchMMProt::ComputeWeightPtNum(const Meta& meta,
                                           Shape2D in_shape) const {
  size_t block_size = in_shape[1];
  size_t num_row_blocks = CeilDiv((size_t)meta.dims[1], block_size);
...
size_t SIMDBatchMMProt::ComputeOutputCtNum(const Meta& meta,
                                           Shape2D in_shape) const {
  size_t block_size = in_shape[1];
  size_t num_col_blocks = CeilDiv((size_t)meta.dims[2], block_size);
  size_t input_groups = CeilDiv((uint64_t)in_shape[0], simd_lane_);
...
void SIMDBatchMMProt::PrepareWeightVector(
    const Meta& meta, Shape2D in_shape, absl::Span<const uint64_t> weight,
  ...
  size_t num_row_blocks = CeilDiv((size_t)meta.dims[1], block_size);
  size_t num_col_blocks = CeilDiv((size_t)meta.dims[2], block_size);
  // input_groups > 1 if and only if eff_row_size > simd_lane_ <-> meta.dims[0]
  // > simd_lane_
  size_t input_groups = CeilDiv((uint64_t)in_shape[0], simd_lane_);
  ...
  // Use weight from different batch to align with different input in
  // {eff_row_size}

  SPU_ENFORCE_EQ(weight.size(), meta.batch * meta.dims[1] * meta.dims[2]);
  if ((uint64_t)in_shape[0] <= row_size_) {
    // Compute baby_step
    size_t baby_step = absl::bit_ceil(static_cast<uint64_t>(
    ...
    SPU_ENFORCE_EQ(weight_vec.size(), num_row_blocks *
                                          CeilDiv(num_col_blocks, (size_t)2) *
                                          block_size * simd_lane_);
    ...
    yacl::parallel_for(0, job_number, [&](int64_t bgn, int64_t end) {
      for (int64_t job_id = bgn; job_id < end; ++job_id) {
        ...
        size_t cb2 =
            job_id % CeilDiv(num_col_blocks,
                             (size_t)2);  // for each block pair of 2 cols
        size_t block_start = (rb * CeilDiv(num_col_blocks, (size_t)2) + cb2) *
                             block_size * simd_lane_;
        ...
        for (size_t d = 0; d < block_size; ++d) {  // for each block diag
          ...
          for (size_t k = 0; k < block_size; ++k) {  // for each diag element
            ...
            size_t src_row_idx1 =
                rb * block_size + (k + d - diag_step + block_size) % block_size;
            // c = cb2 * block_size + c_in_block
            size_t src_col_idx1 = (2 * cb2) * block_size +
                                  (k - diag_step + block_size) % block_size;

            // check boundary
            if (src_row_idx1 < (size_t)meta.dims[1] &&
                src_col_idx1 < (size_t)meta.dims[2]) {
              for (size_t r = 0; r < (size_t)in_shape[0]; ++r) {
                ...
                uint64_t value =
                    weight[batch_idx * meta.dims[1] * meta.dims[2] +
                           src_row_idx1 * meta.dims[2] + src_col_idx1];
            ...
            size_t src_col_idx2 = (2 * cb2 + 1) * block_size +
                                  (k - diag_step + block_size) % block_size;
            if (src_row_idx2 < (size_t)meta.dims[1] &&
                src_col_idx2 < (size_t)meta.dims[2]) {
              for (size_t r = 0; r < (size_t)in_shape[0]; ++r) {
                size_t batch_idx = r / meta.dims[0];
                if (batch_idx >= meta.batch) {
                  break;
                ...
                uint64_t value =
                    weight[batch_idx * meta.dims[1] * meta.dims[2] +
                           src_row_idx2 * meta.dims[2] + src_col_idx2];
                weight_vec[diag_start + k * in_shape[0] + r + row_size_] =
                    value;
              }
            }
            // else pad 0
    ...
    SPU_ENFORCE_EQ(weight_vec.size(),
                   input_groups * num_row_blocks * num_col_blocks * simd_lane_);
    ...
    for (size_t i = 0; i < input_groups; ++i) {  // for each group of input
      ...
      for (size_t rb = 0; rb < num_row_blocks; ++rb) {
        for (size_t cb = 0; cb < num_col_blocks; ++cb) {  // for each block
          size_t block_start =
              ((i * num_row_blocks + rb) * num_col_blocks + cb) * simd_lane_;

          // for (size_t r = 0; r < (size_t)in_shape[0]; ++r) { // for each
          // input col element
          for (size_t r = 0; r < simd_lane_;
               ++r) {  // for each input col element
            ...
            if (src_row_idx < meta.batch * meta.dims[1] &&
                src_col_idx < (size_t)meta.dims[2]) {
...
NdArrayRef SIMDBatchMMProt::PrepareWeightVector(
    const Meta& meta, Shape2D in_shape, const NdArrayRef& weight) const {
  ...
  SPU_ENFORCE_EQ((size_t)weight.numel(),
                 (size_t)meta.batch * meta.dims[1] * meta.dims[2]);
  ...
  DISPATCH_ALL_FIELDS(field, [&]() {
    ...
    if ((uint64_t)in_shape[0] <= row_size_) {
      ...
      SPU_ENFORCE_EQ(input_groups, (size_t)1);
      weight_vec = ring_zeros(
          field, {static_cast<int64_t>(num_row_blocks *
                                       CeilDiv(num_col_blocks, (size_t)2) *
                                       block_size * simd_lane_)});
        ...
        for (int64_t job_id = bgn; job_id < end; ++job_id) {
          ...
          for (size_t d = 0; d < block_size; ++d) {  // for each block diag
            ...
            for (size_t k = 0; k < block_size; ++k) {  // for each diag element
              size_t src_row_idx1 =
                  rb * block_size +
                  (k + d - diag_step + block_size) % block_size;
              size_t src_col_idx1 = (2 * cb2) * block_size +
                                    (k - diag_step + block_size) % block_size;
              ...
              if (src_row_idx1 < (size_t)meta.dims[1] &&
                ...
                for (size_t r = 0; r < (size_t)in_shape[0]; ++r) {
                  ...
                  uint64_t value =
                      weight.at<uT>(batch_idx * meta.dims[1] * meta.dims[2] +
                                    src_row_idx1 * meta.dims[2] + src_col_idx1);
              ...
              if (src_row_idx2 < (size_t)meta.dims[1] &&
                ...
                for (size_t r = 0; r < (size_t)in_shape[0]; ++r) {
                  ...
                  uint64_t value =
                      weight.at<uT>(batch_idx * meta.dims[1] * meta.dims[2] +
                                    src_row_idx2 * meta.dims[2] + src_col_idx2);
                  weight_vec.at<uT>(diag_start + k * in_shape[0] + r +
                                    row_size_) = value;
                }
              }
            }
      ...
      SPU_ENFORCE_EQ(block_size, (size_t)1);
      weight_vec = ring_zeros(
          field, {static_cast<int64_t>(input_groups * num_row_blocks *
                                       num_col_blocks * simd_lane_)});
          ...
          for (size_t cb = 0; cb < num_col_blocks; ++cb) {  // for each block
            ...
            for (size_t r = 0; r < simd_lane_;
              ...
              if (src_row_idx < meta.batch * meta.dims[1] &&
                  src_col_idx < (size_t)meta.dims[2]) {
                uint64_t value =
                    weight.at<uT>(src_row_idx * meta.dims[2] + src_col_idx);
  ...
  for (size_t i = 0; i < input_groups; ++i) {     // for each group of input
    for (size_t b = 0; b < number_blocks; ++b) {  // for each block
      ...
      for (size_t c = 0; c < block_size; ++c) {  // for each col of the block
        ...
        for (size_t r = 0; r < std::min((size_t)in_shape[0], simd_lane_);
             ++r) {  // for each row of the block
          size_t src_row_idx = i * simd_lane_ + r;
          size_t src_col_idx = b * block_size + c;

          if (src_row_idx < meta.batch * meta.dims[0] &&
              src_col_idx < (size_t)meta.dims[1]) {
            input_vec[block_start + col_start + r] =
                input[src_row_idx * meta.dims[1] + src_col_idx];

            // if eff_row_size <= row_size_, we copy input into the second half
            // of simd poly
...
NdArrayRef SIMDBatchMMProt::PrepareInputVector(const Meta& meta,
                                               Shape2D in_shape,
  ...
  SPU_ENFORCE_EQ((size_t)input.numel(),
                 (size_t)meta.batch * meta.dims[0] * meta.dims[1]);

  const auto field = eltype.as<Ring2k>()->field();
  NdArrayRef input_vec = ring_zeros(
      field, {static_cast<int64_t>(input_groups * number_blocks * simd_lane_)});
...
void SIMDBatchMMProt::ParseResult(const Meta& meta, Shape2D in_shape,
  ...
  SPU_ENFORCE_EQ(res_mat.size(), static_cast<uint64_t>(
                                     meta.batch * meta.dims[0] * meta.dims[2]));
  if ((size_t)in_shape[0] <= row_size_) {
    // input_groups == 1
    SPU_ENFORCE_EQ(input_groups, (size_t)1);
    SPU_ENFORCE_EQ(ans_poly.size(),
                   CeilDiv(num_col_blocks, (size_t)2) * simd_lane_);

    for (size_t cb2 = 0; cb2 < CeilDiv(num_col_blocks, (size_t)2);
         ++cb2) {  // for each block pair of 2 cols
      size_t block_start = cb2 * simd_lane_;

      for (size_t c = 0; c < block_size; ++c) {  // for each col of the block
        size_t col_start = c * in_shape[0];

        for (size_t r = 0; r < (size_t)in_shape[0];
             ++r) {  // for each row of the block
          size_t res_row_idx = r;
          size_t res_col_idx1 = (2 * cb2) * block_size + c;
          size_t res_col_idx2 = (2 * cb2 + 1) * block_size + c;

          if (res_row_idx < (size_t)meta.batch * meta.dims[0] &&
              res_col_idx1 < (size_t)meta.dims[2]) {
          ...
          if (res_row_idx < (size_t)meta.batch * meta.dims[0] &&
              res_col_idx2 < (size_t)meta.dims[2]) {
    ...
    for (size_t i = 0; i < input_groups; ++i) {  // for each group of input
      for (size_t cb = 0; cb < num_col_blocks; ++cb) {  // for each block
        ...
        // for (size_t r = 0; r < (size_t)in_shape[0]; ++r) { // for each row of
        // the block
        for (size_t r = 0; r < simd_lane_; ++r) {  // for each row of the block
          ...
          if (res_row_idx < (size_t)meta.batch * meta.dims[0] &&
              res_col_idx < (size_t)meta.dims[2]) {
...
NdArrayRef SIMDBatchMMProt::ParseResult(const Meta& meta, Shape2D in_shape,
  ...
  NdArrayRef res_mat = ring_zeros(
      eltype.as<Ring2k>()->field(),
  ...
  DISPATCH_ALL_FIELDS(eltype.as<Ring2k>()->field(), [&]() {
    ...
    if ((size_t)in_shape[0] <= row_size_) {
      SPU_ENFORCE_EQ(input_groups, (size_t)1);
      SPU_ENFORCE_EQ((size_t)ans_poly.numel(),
                     CeilDiv(num_col_blocks, (size_t)2) * simd_lane_);
      ...
      SPU_ENFORCE_EQ((size_t)ans_poly.numel(),
                     input_groups * num_col_blocks * simd_lane_);
      SPU_ENFORCE_EQ(block_size, (size_t)1);
      for (size_t i = 0; i < input_groups; ++i) {  // for each group of input
        for (size_t cb = 0; cb < num_col_blocks; ++cb) {  // for each block
          size_t block_start = (i * num_col_blocks + cb) * simd_lane_;
...
void SIMDBatchMMProt::BatchMatMatMul(const Meta& meta, Shape2D in_shape,
                                     absl::Span<const RLWECt> lhs_input,
                                     absl::Span<const RLWEPt> rhs_weight,
  ...
  if ((size_t)in_shape[0] <= row_size_) {
    ...
    SPU_ENFORCE_EQ(
        rhs_weight.size(),
        num_row_blocks * CeilDiv(num_col_blocks, (size_t)2) * block_size);
    ...
    yacl::parallel_for(0, num_row_blocks * baby_step,
                       [&](size_t bgn, size_t end) {
                         for (size_t idx = bgn; idx < end; ++idx) {
                           ...
                           if (s == 0) {
                             ...
                             CATCH_SEAL_ERROR(evaluator.rotate_rows(
                                 lhs_input[rb], s * in_shape[0], gal_keys,
    ...
    std::vector<RLWECt> tmp_out(CeilDiv(num_col_blocks, (size_t)2) *
                                (block_size / baby_step));

    yacl::parallel_for(
        0, CeilDiv(num_col_blocks, (size_t)2) * (block_size / baby_step),
        [&](size_t bgn, size_t end) {
          for (size_t job_id = bgn; job_id < end;
               ++job_id) {  // for each block pair of 2 cols
            size_t cb2 = job_id / (block_size / baby_step);
            size_t gs_id = job_id % (block_size / baby_step);
            // for (size_t cb2 = bgn; cb2 < end; ++cb2) { // for each block pair
            // of 2 cols for (size_t gs_id = 0; gs_id < (block_size /
            // baby_step); ++gs_id) { // for each giant step
            RLWECt res;  // zero

            seal::util::encrypt_zero_asymmetric(
                public_key, context, lhs_input[0].parms_id(),
                lhs_input[0].is_ntt_form(), res);

            for (size_t bs_id = 0; bs_id < baby_step;
                 ++bs_id) {  // for each baby step

              for (size_t rb = 0; rb < num_row_blocks;
                   ++rb) {  // for each row block / input ct
                size_t rotated_input_idx = rb * baby_step + bs_id;
                size_t rhs_weight_idx =
                    (rb * CeilDiv(num_col_blocks, (size_t)2) + cb2) *
                        block_size +
                    gs_id * baby_step + bs_id;
                RLWECt tmp;

                evaluator.multiply_plain(rotated_input[rotated_input_idx],
                                         rhs_weight[rhs_weight_idx], tmp);
                evaluator.add_inplace(res, tmp);
              }
            }
            evaluator.rotate_rows_inplace(res, gs_id * baby_step * in_shape[0],
                                          gal_keys);
    ...
    yacl::parallel_for(
        0, CeilDiv(num_col_blocks, (size_t)2), [&](size_t bgn, size_t end) {
          for (size_t cb2 = bgn; cb2 < end;
               ++cb2) {  // for each block pair of 2 cols
            // output ct must be encrypted zero
            seal::util::encrypt_zero_asymmetric(
                public_key, context, lhs_input[0].parms_id(),
                lhs_input[0].is_ntt_form(), out[cb2]);
            for (size_t gs_id = 0; gs_id < (block_size / baby_step);
                 ++gs_id) {  // for each giant step
    ...
    SPU_ENFORCE_EQ(rhs_weight.size(),
                   input_groups * num_row_blocks * num_col_blocks);
    ...
    // No rotation needed
    yacl::parallel_for(
        0, input_groups * num_col_blocks, [&](size_t bgn, size_t end) {
          for (size_t idx = bgn; idx < end; ++idx) {  // for each output block
            ...
            for (size_t rb = 0; rb < num_row_blocks;
              ...
              size_t rhs_weight_idx = i * num_row_blocks * num_col_blocks +
                                      rb * num_col_blocks + cb;
              RLWECt tmp;
              evaluator.multiply_plain(lhs_input[lhs_input_idx],
                                       rhs_weight[rhs_weight_idx], tmp);
...
void SIMDBatchMMProt::ReshareOutputInplace(
    absl::Span<RLWECt> ct, absl::Span<const uint64_t> share_mask,
    const RLWEPublicKey& public_key, const seal::SEALContext& context) {
