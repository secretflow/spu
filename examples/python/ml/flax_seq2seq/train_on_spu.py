...
import models
from examples.python.ml.flax_seq2seq.input_pipeline import CharacterTable as CTable

from examples.python.ml.flax_seq2seq.input_pipeline import get_sequence_lengths
from examples.python.ml.flax_seq2seq.input_pipeline import mask_sequences
...
def train_and_evaluate(workdir: str) -> train_state.TrainState:
    ...
    for step in range(FLAGS.num_train_steps):
        batch = ppd.device("P1")(ctable.get_batch)(FLAGS.batch_size)
        state, metrics = ppd.device("SPU")(train_step)(state, batch, rng, ctable.eos_id)

        if step % FLAGS.decode_frequency == 0:
            writer.write_scalars(step, ppd.get(metrics))
            batch = ctable.get_batch(5)
            decode_batch(ppd.get(state), batch, rng, ctable)
