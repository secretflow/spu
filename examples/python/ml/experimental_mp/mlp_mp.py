...
# > bazel run -c opt //examples/python/ml/haiku_mp:mlp_mp_test
...
Modified from deepmind/dm-haiku: https://github.com/deepmind/dm-haiku/blob/main/examples/mnist.py
"""
...
# Used for designated precision configuration
class MyLinear(hk.Linear):
    def __init__(
        self,
        output_size: int,
        with_bias: bool = True,
    ...
    def __call__(
        ...
        *,
        ...
        return super().__call__(inputs, precision=precision)
...
def main():
    # mixed-precision haiku

    low_policy = jmp.get_policy("p=f32,c=f16,o=f16")
    high_policy = jmp.get_policy("p=f32,c=f32,o=f32")

    hk.mixed_precision.set_policy(hk.Linear, low_policy)
    hk.mixed_precision.set_policy(MyLinear, high_policy)
    ...
    print(jax.make_jaxpr(evaluate)(state.params, next(train_dataset)))
