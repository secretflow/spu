...
import haiku as hk
from examples.python.ml.haiku_mp import dataset
import jax
...
flags.DEFINE_string('mp_policy', 'p=f16,c=f16,o=f16', help='')
...
def main(argv):
  ...
  # hack
  # batch = np.random.random(size=(224, 224, 3))
  ...
  with time_activity('train'):
    for step_num in range(num_train_steps):
      ...
      with jax.profiler.StepTraceAnnotation('train', step_num=step_num):
        batch = next(train_dataset)
        # hack
        # batch = np.random.random(size=(FLAGS.train_device_batch_size, 224, 224, 3))
