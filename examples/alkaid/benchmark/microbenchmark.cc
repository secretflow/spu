...
// static_cast<uint128_t>(16813041318660192066ull), kShape);
...
#ifdef ALKAID_USE_BITWIDTH_16
    MyUnwrapValue(x_sb_aby3).eltype() =
        spu::makeType<aby3np::BShrTy>(aby3np::calcBShareBacktype(16), 16);
    MyUnwrapValue(y_sb_aby3).eltype() =
        spu::makeType<aby3np::BShrTy>(aby3np::calcBShareBacktype(16), 16);
    MyUnwrapValue(x_sb_ours).eltype() =
        spu::makeType<oursnp::BShrTy>(oursnp::calcBShareBacktype(16), 16);
    MyUnwrapValue(y_sb_ours).eltype() =
        spu::makeType<oursnp::BShrTy>(oursnp::calcBShareBacktype(16), 16);
#endif
     ...
     */
    {
      ...
      if (lctx.get()->Rank() == 0)
        std::cout << "Mult size: " << N * M << std::endl;
      ...
      auto x_test_mss =
          make_p(sctx_aby3.get(), static_cast<uint128_t>(1ull << 7), newkShape);
      auto xb_test_mss = p2b(sctx_ours.get(), x_test_mss);
      auto xb_test_rss =
          oursnp::ResharingMrss2Rss(&kectx_ours, MyUnwrapValue(xb_test_mss));

      spu::NdArrayRef x_b_ours;
      for (int i = 0; i < EXP_TIMES; i++) {
        auto xb_res_ass =
            oursnp::RssAnd2NoComm(&kectx_ours, xb_test_rss, xb_test_rss);
      ...
      auto duration_ours =
          std::chrono::duration_cast<std::chrono::microseconds>(offline_ours -
                                                                offline_start);
      if (lctx.get()->Rank() == 0)
        std::cout << "------------------------ offline, ours" << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "offline micro seconds: " << duration_ours.count()
                  << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "offline comm: " << GetComm(ours).comm - comm << std::endl;
      if (lctx.get()->Rank() == 0)
        std::cout << "offline latency: " << GetComm(ours).latency - latency
                  << std::endl;
      if (lctx.get()->Rank() == 0) OfflineRecorder::StopRecorder();
    }
    ...
    //   if (lctx.get()->Rank() == 0) std::cout << "Prepare input." <<
    //   std::endl;
    ...
    //   for (int i = 0; i < EXP_TIMES; i++) {
    //     #ifdef ALKAID_USE_BITWIDTH_16
    //     auto msbx_s_ref = aby3np::MsbA2BForBitwidth16(&kectx_aby3,
    //     MyUnwrapValue(x_ari_aby3)); msbx_s = MyWrapValue(msbx_s_ref); #else
    ...
    //   auto duration_spu =
    //   std::chrono::duration_cast<std::chrono::microseconds>(msb_spu -
    //   msb_start); if (lctx.get()->Rank() == 0) std::cout <<
    //   "------------------------ MSB, spu" << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "msb micro seconds: " <<
    //   duration_spu.count() << std::endl; if (lctx.get()->Rank() == 0)
    //   std::cout << "msb comm: " << GetComm(aby3).comm - comm << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "msb latency: " <<
    ...
    //   // MyUnwrapValue(x_ari_test).eltype() =
    //   spu::makeType<aby3::AShrTy>(field);
    //   // auto msbx_ours_s = MyUnwrapValue(msb_a2b(sctx_aby3.get(), x_ari));

    //   spu::NdArrayRef msbx_ours_s;
    //   for (int i = 0; i < EXP_TIMES; i++) {
    //     #ifdef ALKAID_USE_BITWIDTH_16
    //     msbx_ours_s = oursnp::MsbA2BMultiFanInForBitwidth16(&kectx_ours,
    //     MyUnwrapValue(x_ari_ours)); #else msbx_ours_s =
    //     oursnp::MsbA2BMultiFanIn(&kectx_ours, MyUnwrapValue(x_ari_ours));
    //     #endif
    //   }
    ...
    //   msb_spu); if (lctx.get()->Rank() == 0) OfflineRecorder::StopRecorder();
    //   if (lctx.get()->Rank() == 0) std::cout << "------------------------
    //   MSB, ours" << std::endl; if (lctx.get()->Rank() == 0) std::cout << "msb
    //   micro seconds: " << duration_ours.count() << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "msb comm: " <<
    ...
    //   if (lctx.get()->Rank() == 0) printResult(msb_p_ours.data(), "msb,
    //   ours"); if (lctx.get()->Rank() == 0) checkOutput(msb_p_spu.data(),
    //   msb_p_ours.data(), "msb");
    // }
    ...
    //   for (int i = 0; i < EXP_TIMES; i++) {
    //     #ifdef ALKAID_USE_BITWIDTH_16
    //     auto x_a_ref = aby3np::B2AForBitwidth16(&kectx_aby3,
    //     MyUnwrapValue(x_sb_aby3)); x_a = MyWrapValue(x_a_ref); #else x_a =
    //     b2a(sctx_aby3.get(), x_sb_aby3); #endif
    //   }
    ...
    //   "------------------------ B2A, spu" << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "b2a micro seconds: " <<
    //   duration_spu.count() << std::endl; if (lctx.get()->Rank() == 0)
    //   std::cout << "b2a comm: " << GetComm(aby3).comm - comm << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "b2a latency: " <<
    //   GetComm(aby3).latency - latency << std::endl;
    ...
    //     MyUnwrapValue(x_sb_ours)); #else x_a_ours =
    //     oursnp::B2AMultiFanIn(&kectx_ours, MyUnwrapValue(x_sb_ours)); #endif
    //   }
    ...
    //   if (lctx.get()->Rank() == 0) std::cout << "------------------------
    //   B2A, ours" << std::endl; if (lctx.get()->Rank() == 0) std::cout << "b2a
    //   micro seconds: " << duration_ours.count() << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "b2a comm: " <<
    //   GetComm(ours).comm - comm << std::endl; if (lctx.get()->Rank() == 0)
    ...
    //   auto x_p_ours = OpenRef(oursnp::A2BMultiFanIn(&kectx_ours, x_a_ours),
    //   ours);
    //   // if (lctx.get()->Rank() == 0) printResult(msbx_p.data(), "msb, spu");
    //   // if (lctx.get()->Rank() == 0) printResult(msbx_ours_p.data(), "msb,
    //   ours"); if (lctx.get()->Rank() == 0) checkOutput(x_p_spu.data(),
    //   x_p_ours.data(), "b2a");
    // }
    ...
    //   for (int i = 0; i < EXP_TIMES; i++) {
    //     #ifdef ALKAID_USE_BITWIDTH_16
    //     auto x_b_ref = aby3np::A2BForBitwidth16(&kectx_aby3,
    //     MyUnwrapValue(x_ari_aby3)); x_b = MyWrapValue(x_b_ref); #else x_b =
    //     a2b(sctx_aby3.get(), x_ari_aby3); #endif
    //   }

    //   auto a2b_spu = std::chrono::high_resolution_clock::now();
    //   auto duration_spu =
    //   std::chrono::duration_cast<std::chrono::microseconds>(a2b_spu -
    //   a2b_start); if (lctx.get()->Rank() == 0) std::cout <<
    //   "------------------------ A2B, spu" << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "a2b micro seconds: " <<
    //   duration_spu.count() << std::endl; if (lctx.get()->Rank() == 0)
    //   std::cout << "a2b comm: " << GetComm(aby3).comm - comm << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "a2b latency: " <<
    //   GetComm(aby3).latency - latency << std::endl;
    ...
    //   for (int i = 0; i < EXP_TIMES; i++) {
    //     #ifdef ALKAID_USE_BITWIDTH_16
    //     x_b_ours = oursnp::A2BMultiFanInForBitwidth16(&kectx_ours,
    //     MyUnwrapValue(x_ari_ours)); #else x_b_ours =
    //     oursnp::A2BMultiFanIn(&kectx_ours, MyUnwrapValue(x_ari_ours)); #endif
    //   }

    //   auto a2b_ours = std::chrono::high_resolution_clock::now();
    //   auto duration_ours =
    //   std::chrono::duration_cast<std::chrono::microseconds>(a2b_ours -
    //   a2b_spu); if (lctx.get()->Rank() == 0) OfflineRecorder::StopRecorder();
    //   if (lctx.get()->Rank() == 0) std::cout << "------------------------
    //   A2B, ours" << std::endl; if (lctx.get()->Rank() == 0) std::cout << "a2b
    //   micro seconds: " << duration_ours.count() << std::endl; if
    //   (lctx.get()->Rank() == 0) std::cout << "a2b comm: " <<
    //   GetComm(ours).comm - comm << std::endl; if (lctx.get()->Rank() == 0)
    ...
    //     MyUnwrapValue(x_sb_ours)); #else x_a_ours =
    //     oursnp::PPATest(&kectx_ours, MyUnwrapValue(x_sb_ours)); #endif
    //   }
    ...
    //   // Check output.
    //   // auto x_p_spu = OpenRef(MyUnwrapValue(a2b(sctx_aby3.get(), x_a)),
    //   aby3);
    ...
    //   MyUnwrapValue(input10)); auto input11_mss =
    //   oursnp::ResharingRss2Mrss(&kectx, MyUnwrapValue(input11)); auto
    //   input12_mss = oursnp::ResharingRss2Mrss(&kectx,
    //   MyUnwrapValue(input12)); auto input13_mss =
    //   oursnp::ResharingRss2Mrss(&kectx, MyUnwrapValue(input13)); auto
    //   input30_mss = oursnp::ResharingRss2Mrss(&kectx,
    //   MyUnwrapValue(input30)); auto input31_mss =
    //   oursnp::ResharingRss2Mrss(&kectx, MyUnwrapValue(input31)); auto
    //   input32_mss = oursnp::ResharingRss2Mrss(&kectx,
    //   MyUnwrapValue(input32)); auto input33_mss =
    //   oursnp::ResharingRss2Mrss(&kectx, MyUnwrapValue(input33)); auto
    //   input50_mss = oursnp::ResharingRss2Mrss(&kectx,
    //   MyUnwrapValue(input50)); auto input51_mss =
    //   oursnp::ResharingRss2Mrss(&kectx, MyUnwrapValue(input51)); auto
    //   input52_mss = oursnp::ResharingRss2Mrss(&kectx,
    //   MyUnwrapValue(input52)); auto input53_mss =
    //   oursnp::ResharingRss2Mrss(&kectx, MyUnwrapValue(input53));
